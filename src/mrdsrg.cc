...
void MRDSRG::build_density(){
    ...
    outfile->Printf("\n  If this is not desired, go to mrdsrg.cc build_density() around line 190.");
...
//    return Edsrg_sa[0][0];
    ...
    for(int n = 0; n < nentry; ++n){
        ...
        for(int i = 0; i < nstates; ++i){
            outfile->Printf("\n     %3d     %3s    %2d   %20.12f",
                            multi, irrep_symbol[irrep].c_str(), i, Edsrg_vec[0][n][i]);
        }
        outfile->Printf("\n    %s", dash1.c_str());
    }
    ...
    for(int n = 0; n < nentry; ++n){
        ...
        for(int i = 0; i < nstates; ++i){
            outfile->Printf("\n     %3d     %3s    %2d   %20.12f*",
                            multi, irrep_symbol[irrep].c_str(), i, Edsrg_vec[Edsrg_vec.size()-1][n][i]);
        }
        outfile->Printf("\n    %s", dash1.c_str());
    }
...
//    double h0 = 0.0;
//    BlockedTensor h1 = BTF_->build(tensor_type_,"hole1",spin_cases({"cc"}));
...
//        F_.block(block).citerate([&](const std::vector<size_t>& i,const double& value){
//            if(i[0] == i[1]){
//                h0 -= value;
//            }
//        });

//        h1.block(block).citerate([&](const std::vector<size_t>& i,const double& value){
//            if(i[0] == i[1]){
//                h0 += value;
//            }
//        });
...
//        V_.block(block).citerate([&](const std::vector<size_t>& i,const double& value){
//            if(i[0] == i[2] && i[1] == i[3]){
//                h0 += 0.5 * value;
//            }
//        });
...
//            }
//        });
//        H_.block(block).citerate([&](const std::vector<size_t>& i,const double& value){
//            if (i[0] == i[1]){
//                Etest1 += 0.5 * value;
//            }
//        });

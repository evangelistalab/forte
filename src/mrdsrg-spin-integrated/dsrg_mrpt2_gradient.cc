...
void DSRG_MRPT2::set_xi() {
    ...
    double eee;
    eee += 0.5 * V_["evxy"] * T1_["ue"] * Lambda2_["xyuv"];
    eee += V_["eVxY"] * T1_["ue"] * Lambda2_["xYuV"];
    eee += 0.5 * V_["EVXY"] * T1_["UE"] * Lambda2_["XYUV"];
    eee += V_["vExY"] * T1_["UE"] * Lambda2_["xYvU"];

    std::cout << "tested energy(xi) = " << std::setprecision(12) << eee << std::endl;
}

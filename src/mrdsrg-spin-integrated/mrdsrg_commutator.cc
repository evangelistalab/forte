...
void MRDSRG::H2_T1_C0_DF(BlockedTensor& B, BlockedTensor& T1, const double& alpha, double& C0) {
    ...
    forte::timer t("H2_T1_C0_DF");

    TIME_LINE(temp = ambit::BlockedTensor::build(tensor_type_, "temp", {"aaaa"}));
    TIME_LINE(temp["uvxy"] += B["gex"] * B["gvy"] * T1["ue"]);
    TIME_LINE(temp["uvxy"] -= B["gey"] * B["gvx"] * T1["ue"]);
    TIME_LINE(temp["uvxy"] -= B["gum"] * B["gvy"] * T1["mx"]);
    TIME_LINE(temp["uvxy"] += B["guy"] * B["gvm"] * T1["mx"]);
    ...
    TIME_LINE(temp = ambit::BlockedTensor::build(tensor_type_, "temp", {"aAaA"}));
    TIME_LINE(temp["uVxY"] += B["gex"] * B["gVY"] * T1["ue"]);
    TIME_LINE(temp["uVxY"] += B["gux"] * B["gEY"] * T1["VE"]);
    TIME_LINE(temp["uVxY"] -= B["gum"] * B["gVY"] * T1["mx"]);
    TIME_LINE(temp["uVxY"] -= B["gux"] * B["gVM"] * T1["MY"]);
...
void MRDSRG::H2_T1_C1_DF(BlockedTensor& B, BlockedTensor& T1, const double& alpha,
    ...
    forte::timer t("H2_T1_C1_DF");

    TIME_LINE(C1["qp"] += alpha * T1["ma"] * B["gqp"] * B["gam"]);
    TIME_LINE(C1["qp"] -= alpha * T1["ma"] * B["gqm"] * B["gap"]);
    TIME_LINE(C1["qp"] += alpha * T1["xe"] * Gamma1_["yx"] * B["gqp"] * B["gey"]);
    TIME_LINE(C1["qp"] -= alpha * T1["xe"] * Gamma1_["yx"] * B["gqy"] * B["gep"]);
    TIME_LINE(C1["qp"] -= alpha * T1["mu"] * Gamma1_["uv"] * B["gqp"] * B["gvm"]);
    TIME_LINE(C1["qp"] += alpha * T1["mu"] * Gamma1_["uv"] * B["gqm"] * B["gvp"]);
    TIME_LINE(C1["qp"] += alpha * T1["MA"] * B["gqp"] * B["gAM"]);
    TIME_LINE(C1["qp"] += alpha * T1["XE"] * Gamma1_["YX"] * B["gqp"] * B["gEY"]);
    TIME_LINE(C1["qp"] -= alpha * T1["MU"] * Gamma1_["UV"] * B["gqp"] * B["gVM"]);

    TIME_LINE(C1["QP"] += alpha * T1["ma"] * B["gam"] * B["gQP"]);
    TIME_LINE(C1["QP"] += alpha * T1["xe"] * Gamma1_["yx"] * B["gey"] * B["gQP"]);
    TIME_LINE(C1["QP"] -= alpha * T1["mu"] * Gamma1_["uv"] * B["gvm"] * B["gQP"]);
    TIME_LINE(C1["QP"] += alpha * T1["MA"] * B["gQP"] * B["gAM"]);
    TIME_LINE(C1["QP"] -= alpha * T1["MA"] * B["gQM"] * B["gAP"]);
    TIME_LINE(C1["QP"] += alpha * T1["XE"] * Gamma1_["YX"] * B["gQP"] * B["gEY"]);
    TIME_LINE(C1["QP"] -= alpha * T1["XE"] * Gamma1_["YX"] * B["gQY"] * B["gEP"]);
...
void MRDSRG::H2_T1_C2_DF(BlockedTensor& B, BlockedTensor& T1, const double& alpha,
    ...
    forte::timer t("H2_T1_C2_DF");

    TIME_LINE(C2["irpq"] += alpha * T1["ia"] * B["gap"] * B["grq"]);
    TIME_LINE(C2["irpq"] -= alpha * T1["ia"] * B["gaq"] * B["grp"]);
    TIME_LINE(C2["ripq"] += alpha * T1["ia"] * B["grp"] * B["gaq"]);
    TIME_LINE(C2["ripq"] -= alpha * T1["ia"] * B["grq"] * B["gap"]);
    TIME_LINE(C2["rsaq"] -= alpha * T1["ia"] * B["gri"] * B["gsq"]);
    TIME_LINE(C2["rsaq"] += alpha * T1["ia"] * B["grq"] * B["gsi"]);
    TIME_LINE(C2["rspa"] -= alpha * T1["ia"] * B["grp"] * B["gsi"]);
    TIME_LINE(C2["rspa"] += alpha * T1["ia"] * B["gri"] * B["gsp"]);
...
void MRDSRG::H2_T2_C2_DF(BlockedTensor& B, BlockedTensor& T2, const double& alpha,
    ...
    forte::timer t("H2_T2_C2_DF");

    // particle-particle contractions
    forte::timer pp("H2_T2_C2 pp");
    if (foptions_->get_bool("DSRG_DF_BATCH")) {

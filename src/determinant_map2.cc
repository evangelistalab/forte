...
DeterminantMap2::DeterminantMap2(std::vector<STLDeterminant>& dets) {
...
DeterminantMap2::DeterminantMap2(const std::vector<STLDeterminant>& dets) {
    // The dimension of the wavefunction
    wfn_size_ = dets.size();
...
DeterminantMap2::DeterminantMap2(STLDeterminant& det) {
    wfn_[det] = 0;
    wfn_size_ = wfn_.size();
}

DeterminantMap2::DeterminantMap2() { wfn_size_ = wfn_.size(); }

DeterminantMap2::DeterminantMap2(detmap& wfn) : wfn_(wfn) { wfn_size_ = wfn.size(); }

const detmap& DeterminantMap2::wfn_hash() const { return wfn_; }

detmap& DeterminantMap2::wfn_hash() { return wfn_; }

void DeterminantMap2::clear() {
    wfn_.clear();
    wfn_size_ = wfn_.size();
}

std::vector<STLDeterminant> DeterminantMap2::determinants() const {
    std::vector<STLDeterminant> space(wfn_size_);
...
size_t DeterminantMap2::size() const { return wfn_.size(); }

void DeterminantMap2::add(const STLDeterminant& det) {
    wfn_[det] = wfn_size_;
    wfn_size_ = wfn_.size();
}

STLDeterminant DeterminantMap2::get_det(const size_t value) const {
...
bool DeterminantMap2::has_det(const STLDeterminant& det) const {
    bool has = false;
...
void DeterminantMap2::subspace(DeterminantMap2& dets, SharedMatrix evecs,
    ...
    for (size_t I = 0; I < dim; ++I) {
        const STLDeterminant& detI = dets.get_det(det_weights[I].second);
        this->add(detI);
...
void DeterminantMap2::copy(DeterminantMap2& dets) {
    this->clear();
    wfn_ = dets.wfn_;
    wfn_size_ = dets.size();
}

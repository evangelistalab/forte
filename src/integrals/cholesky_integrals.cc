...
double CholeskyIntegrals::aptei_aa(size_t p, size_t q, size_t r, size_t s) {
    ...
    vpqrsalphaC =
        C_DDOT(nthree_, &(ThreeIntegral_->pointer()[p * aptei_idx_ + r][0]), 1,
               &(ThreeIntegral_->pointer()[q * aptei_idx_ + s][0]), 1);
    vpqrsalphaE =
        C_DDOT(nthree_, &(ThreeIntegral_->pointer()[p * aptei_idx_ + s][0]), 1,
...
ambit::Tensor
CholeskyIntegrals::three_integral_block(const std::vector<size_t>& A,
...
void CholeskyIntegrals::gather_integrals() {
    ...
    Timer timer;
    std::shared_ptr<CholeskyERI> Ch(
        new CholeskyERI(std::shared_ptr<TwoBodyAOInt>(integral->eri()),
    ...
    if (options_.get_str("DF_INTS_IO") == "LOAD") {
        ...
        if (psio->exists(file_unit)) {
            ...
            psio->read_entry(file_unit, "length", (char*)&nthree_,
                             sizeof(long int));
            SharedMatrix L_tri =
                SharedMatrix(new Matrix("Partial Cholesky", nthree_, ntri));
            ...
            SharedMatrix L_ao = SharedMatrix(
                new Matrix("Partial Cholesky", nthree_, nbf * nbf));
...
void CholeskyIntegrals::transform_integrals() {
    ...
    ambit::Tensor Cpq_tensor =
        ambit::Tensor::build(tensor_type, "C_sorted", {nso_, nmo_});
    ...
    ThreeIntegral("L,p,q") =
        ThreeIntegral_ao("L,m,n") * Cpq_tensor("m,p") * Cpq_tensor("n,q");
...
void CholeskyIntegrals::make_fock_matrix(SharedMatrix gamma_aM,
                                         SharedMatrix gamma_bM) {
    TensorType tensor_type = CoreTensor;
    ambit::Tensor ThreeIntegralTensor = ambit::Tensor::build(
        tensor_type, "ThreeIndex", {ncmo_, ncmo_, nthree_});
    ambit::Tensor gamma_a =
        ambit::Tensor::build(tensor_type, "Gamma_a", {ncmo_, ncmo_});
    ambit::Tensor gamma_b =
        ambit::Tensor::build(tensor_type, "Gamma_b", {ncmo_, ncmo_});
    ambit::Tensor fock_a =
        ambit::Tensor::build(tensor_type, "Fock_a", {ncmo_, ncmo_});
    ambit::Tensor fock_b =
        ambit::Tensor::build(tensor_type, "Fock_b", {ncmo_, ncmo_});
    ...
    std::memcpy(&gamma_a.data()[0], gamma_aM->pointer()[0],
                sizeof(double) * ncmo_ * ncmo_);
    std::memcpy(&gamma_b.data()[0], gamma_bM->pointer()[0],
                sizeof(double) * ncmo_ * ncmo_);
    std::memcpy(&fock_a.data()[0], one_electron_integrals_a,
                sizeof(double) * ncmo_ * ncmo_);
    std::memcpy(&fock_b.data()[0], one_electron_integrals_b,
                sizeof(double) * ncmo_ * ncmo_);
    ...
    fock_a("p,q") += ThreeIntegralTensor("p,q,Q") *
                     ThreeIntegralTensor("r,s,Q") * gamma_a("r,s");
    fock_a("p,q") -= ThreeIntegralTensor("p,r,Q") *
                     ThreeIntegralTensor("q,s,Q") * gamma_a("r,s");
    fock_a("p,q") += ThreeIntegralTensor("p,q,Q") *
                     ThreeIntegralTensor("r,s,Q") * gamma_b("r,s");

    fock_b("p,q") += ThreeIntegralTensor("p,q,Q") *
                     ThreeIntegralTensor("r,s,Q") * gamma_b("r,s");
    fock_b("p,q") -= ThreeIntegralTensor("p,r,Q") *
                     ThreeIntegralTensor("q,s,Q") * gamma_b("r,s");
    fock_b("p,q") += ThreeIntegralTensor("p,q,Q") *
                     ThreeIntegralTensor("r,s,Q") * gamma_a("r,s");
    ...
    //});
    std::memcpy(fock_matrix_a, &fock_a.data()[0],
                sizeof(double) * ncmo_ * ncmo_);
    std::memcpy(fock_matrix_b, &fock_b.data()[0],
                sizeof(double) * ncmo_ * ncmo_);
}
...
void CholeskyIntegrals::resort_three(std::shared_ptr<Matrix>& threeint,
...
void CholeskyIntegrals::set_tei(size_t, size_t, size_t, size_t, double, bool,
                                bool) {
    outfile->Printf(

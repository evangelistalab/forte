...
double DressedQuantity::contract_with_rdms(RDMs reference) {
    ...
    if (max_rdm_level >= 1 and max_body_ >= 1) {
        out += a_("uv") * reference.g1a()("vu");
        out += b_("uv") * reference.g1b()("vu");
    }

    if (max_rdm_level >= 2 and max_body_ >= 2) {
        out += 0.25 * aa_("uvxy") * reference.g2aa()("xyuv");
        out += 0.25 * bb_("uvxy") * reference.g2bb()("xyuv");
        out += ab_("uvxy") * reference.g2ab()("xyuv");
    }

    if (max_rdm_level >= 3 and max_body_ >= 3) {
        out += (1.0 / 36.0) * aaa_("uvwxyz") * reference.g3aaa()("xyzuvw");
        out += (1.0 / 36.0) * bbb_("uvwxyz") * reference.g3bbb()("xyzuvw");
        out += 0.25 * aab_("uvwxyz") * reference.g3aab()("xyzuvw");
        out += 0.25 * abb_("uvwxyz") * reference.g3abb()("xyzuvw");
    }

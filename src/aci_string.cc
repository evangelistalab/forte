...
std::vector<int> ACIString::get_occupation()
{
	...
	if(ref_type == "RHF" or ref_type == "RKS" or ref_type == "ROHF"){
		...
		//	outfile->Printf("\n  Electron removed from %d, out of %d", labeled_orb_en[noalpha_ - k].second.second, nactel_);
		...
		//		outfile->Printf("\n  Electron removed from %d, out of %d", labeled_orb_en_alfa[noalpha_ - k].second.second, nactel_);
		...
		//		outfile->Printf("\n  Electron removed from %d, out of %d", labeled_orb_en_beta[nobeta_ - k].second.second, nactel_);
...
//  P_space_map_[bs_det] = 1;
    ...
    for (cycle = 0; cycle < max_cycle_; ++cycle){
		...
		if(spin_complete_){
			...
			if( !quiet_mode_) outfile->Printf("\n  %s: %zu determinants","Spin-complete dimension of the P space",str_to_det_.size());
        }else if( !quiet_mode_ ){
        ...
        if( str_to_det_.size() < 200){
            ...
            for( size_t I = 0; I < psize; ++I){
                STLBitsetDeterminant det( alfa_str_[std::get<0>(str_to_det_[I])][std::get<1>(str_to_det_[I])], beta_str_[std::get<2>(str_to_det_[I])][std::get<3>(str_to_det_[I])]  );
                P_space_[I] = det;
            }
            sparse_solver.diagonalize_hamiltonian(P_space_,P_evals,P_evecs,num_ref_roots,wavefunction_multiplicity_,diag_method_);
        }else{
            // Fix this eventually
            sparse_solver.diagonalize_hamiltonian(P_space_,P_evals,P_evecs,num_ref_roots,wavefunction_multiplicity_,DLString);
        }
...
bool ACIString::check_convergence(std::vector<std::vector<double>>& energy_history,SharedVector evals)
{
    int nroot = evals->dim();
...
void ACIString::prune_q_space(std::vector<STLBitsetDeterminant>& large_space,std::vector<STLBitsetDeterminant>& pruned_space,
...
pVector<std::pair<double, double>, std::pair<size_t,double>> ACIString::compute_spin(std::vector<STLBitsetDeterminant> space,
																					  SharedMatrix evecs,
																					  int nroot)
{
	double norm;
...
void ACIString::wfn_analyzer(std::vector<STLBitsetDeterminant> det_space, SharedMatrix evecs, int nroot)
{
...
void ACIString::full_spin_transform( std::vector< STLBitsetDeterminant > det_space, SharedMatrix cI, int nroot )
{
	Timer timer;
...
double ACIString::compute_spin_contamination( std::vector<STLBitsetDeterminant> space, SharedMatrix evecs, int nroot)
{
	auto spins = compute_spin(space, evecs, nroot);
...
std::vector<double> ACIString::davidson_correction( std::vector<STLBitsetDeterminant> P_dets,  SharedVector P_evals, 
...
void ACIString::set_max_rdm( int rdm )
{
	rdm_level_ = rdm;
}
...
void ACIString::compute_H_expectation_val( const std::vector<STLBitsetDeterminant> space, SharedVector& evals, const SharedMatrix evecs, int nroot, DiagonalizationMethod diag_method)
{

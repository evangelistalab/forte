...
public:
     ...
     */
    DSRG_MRPT(Reference reference, boost::shared_ptr<Wavefunction> wfn, Options& options,
...
protected:
    ...
    void H2_T2_C0(BlockedTensor& H2, BlockedTensor& T2, const double& alpha, double& C0, const bool& stored);

    /// Compute zero-body term of commutator [H2, T2] involving only L1
    void H2_T2_C0_L1(BlockedTensor& H2, BlockedTensor& T2, const double& alpha, double& C0, const bool& stored);
    ...
    /// Compute zero-body term of commutator [H2, T2] involving only L3, intermediate of aaaaaa
    double H2_T2_C0_L3_a(BlockedTensor& H2, BlockedTensor& T2);
    /// Compute zero-body term of commutator [H2, T2] involving only L3, intermediate of aaav
    double H2_T2_C0_L3_v(BlockedTensor& H2, BlockedTensor& T2);

    /// Compute zero-body term of commutator [V, T2] involving only L1 and ccvv of V
    double V_T2_C0_L1_ccvv(const std::vector<std::vector<size_t>>& small_core_mo);
    /// Compute zero-body term of commutator [V, T2] involving only L1 and cavv of V
    double V_T2_C0_L1_cavv(const std::vector<std::vector<size_t>>& small_core_mo);
    /// Compute zero-body term of commutator [V, T2] involving only L1 and ccav of V

...
bool pairComp64(const std::pair<double, STLBitsetDeterminant> E1,
                const std::pair<double, STLBitsetDeterminant> E2) {
    return E1.first < E2.first;
}

AdaptiveCI64::AdaptiveCI64(SharedWavefunction ref_wfn, Options& options,
...
AdaptiveCI64::~AdaptiveCI64() {}

void AdaptiveCI64::set_fci_ints(std::shared_ptr<FCIIntegrals> fci_ints) {
    fci_ints_ = fci_ints;
    nuclear_repulsion_energy_ = molecule_->nuclear_repulsion_energy();
    set_ints_ = true;
}

void AdaptiveCI64::set_aci_ints(SharedWavefunction ref_wfn, std::shared_ptr<ForteIntegrals> ints) {
...
double AdaptiveCI64::compute_energy() {
...
void AdaptiveCI64::diagonalize_final_and_compute_rdms() {
...
DeterminantHashVec AdaptiveCI64::get_wavefunction() { return final_wfn_; }

void AdaptiveCI64::print_final(DeterminantHashVec& dets, SharedMatrix& PQ_evecs,
                               SharedVector& PQ_evals) {
...
void AdaptiveCI64::default_find_q_space(DeterminantHashVec& P_space, DeterminantHashVec& PQ_space,
...
#pragma omp parallel
    ...
    std::sort(sorted_dets.begin(), sorted_dets.end(), pairComp64);
...
void AdaptiveCI64::find_q_space(DeterminantHashVec& P_space, DeterminantHashVec& PQ_space,
...
double AdaptiveCI64::average_q_values(int nroot, std::vector<double>& C1, std::vector<double>& E2) {
...
double AdaptiveCI64::root_select(int nroot, std::vector<double>& C1, std::vector<double>& E2) {
    double select_value;
...
void AdaptiveCI64::get_excited_determinants2(int nroot, SharedMatrix evecs,
...
void AdaptiveCI64::get_excited_determinants(int nroot, SharedMatrix evecs,
                                            DeterminantHashVec& P_space,
                                            det_hash<std::vector<double>>& V_hash) {
...
void AdaptiveCI64::get_core_excited_determinants(SharedMatrix evecs, DeterminantHashVec& P_space,
                                                 det_hash<std::vector<double>>& V_hash) {
    size_t max_P = P_space.size();
    const det_hashvec& P_dets = P_space.wfn_hash();
    int nroot = 1;
...
bool AdaptiveCI64::check_convergence(std::vector<std::vector<double>>& energy_history,
                                     SharedVector evals) {
    int nroot = evals->dim();
    int ref = 0;
...
void AdaptiveCI64::prune_q_space(DeterminantHashVec& PQ_space, DeterminantHashVec& P_space,
...
bool AdaptiveCI64::check_stuck(std::vector<std::vector<double>>& energy_history,
                               SharedVector evals) {
    bool stuck = false;
    int nroot = evals->dim();
    if (cycle_ < 4) {
...
std::vector<std::pair<double, double>> AdaptiveCI64::compute_spin(DeterminantHashVec& space,
                                                                  WFNOperator& op,
                                                                  SharedMatrix evecs, int nroot) {
...
/*
void AdaptiveCI64::wfn_analyzer(DeterminantMap& det_space, SharedMatrix evecs, int nroot) {
...
*/
std::vector<std::tuple<double, int, int>> AdaptiveCI64::sym_labeled_orbitals(std::string type) {
...
void AdaptiveCI64::print_wfn(DeterminantHashVec& space, WFNOperator& op, SharedMatrix evecs,
                             int nroot) {
...
void AdaptiveCI64::full_spin_transform(DeterminantHashVec& det_space, SharedMatrix cI, int nroot) {
    //	Timer timer;
...
double AdaptiveCI64::compute_spin_contamination(DeterminantHashVec& space, WFNOperator& op,
                                                SharedMatrix evecs, int nroot) {
...
void AdaptiveCI64::save_dets_to_file(DeterminantHashVec& space, SharedMatrix evecs) {
...
std::vector<double> AdaptiveCI64::davidson_correction(std::vector<STLBitsetDeterminant>& P_dets,
...
void AdaptiveCI64::set_max_rdm(int rdm) {
    rdm_level_ = rdm;
    set_rdm_ = true;
}

Reference AdaptiveCI64::reference() {
...
// void AdaptiveCI64::compute_H_expectation_val( const
...
/*
void AdaptiveCI64::convert_to_string(const std::vector<STLBitsetDeterminant>& space) {
    size_t space_size = space.size();
    size_t nalfa_str = 0;
    size_t nbeta_str = 0;
...
int AdaptiveCI64::root_follow(DeterminantHashVec& P_ref, std::vector<double>& P_ref_evecs,
                              DeterminantHashVec& P_space, SharedMatrix P_evecs,
                              int num_ref_roots) {
...
void AdaptiveCI64::compute_aci(DeterminantHashVec& PQ_space, SharedMatrix& PQ_evecs,
                               SharedVector& PQ_evals) {
...
AdaptiveCI64::dl_initial_guess(std::vector<STLBitsetDeterminant>& old_dets,
...
void AdaptiveCI64::compute_rdms(std::shared_ptr<FCIIntegrals> fci_ints, DeterminantHashVec& dets,
...
void AdaptiveCI64::add_bad_roots(DeterminantHashVec& dets) {
    bad_roots_.clear();
...
void AdaptiveCI64::save_old_root(DeterminantHashVec& dets, SharedMatrix& PQ_evecs, int root) {
...
void AdaptiveCI64::compute_multistate(SharedVector& PQ_evals) {
...
DeterminantHashVec AdaptiveCI64::approximate_wfn(DeterminantHashVec& PQ_space, SharedMatrix& evecs,
...
void AdaptiveCI64::upcast_reference(DeterminantHashVec& ref) {
...
void AdaptiveCI64::add_external_excitations(DeterminantHashVec& ref) {
...
void AdaptiveCI64::spin_analysis() {
...
void AdaptiveCI64::update_sigma() { sigma_ = options_.get_double("ACI_RELAX_SIGMA"); }
}
} // EndNamespaces

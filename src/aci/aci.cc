...
void set_ACI_options(ForteOptions &foptions) {
  ...
  foptions.add_double("ACI_PRESCREEN_THRESHOLD", 1e-12,
                      "The SD space prescreening threshold");
  ...
  foptions.add_str("ACI_EXCITED_ALGORITHM", "ROOT_ORTHOGONALIZE",
  ...
  foptions.add_str("ACI_SIZE_CORRECTION", "",
                   "Perform size extensivity correction");
  ...
  foptions.add_int("ACI_PREITERATIONS", 0,
                   "Number of iterations to run SA-ACI before SS-ACI");
  ...
  foptions.add_bool("ACI_SAVE_FINAL_WFN", false,
                    "Print final wavefunction to file");
  /*- Print the P space? -*/
  foptions.add_bool("ACI_PRINT_REFS", false, "Print the P space");
  /*- Set the initial guess space size for DL solver -*/
  foptions.add_int("DL_GUESS_SIZE", 100,
                   "Set the initial guess space size for DL solver");
  /*- Number of guess vectors for Sparse CI solver -*/
  foptions.add_int("N_GUESS_VEC", 10,
                   "Number of guess vectors for Sparse CI solver");
  foptions.add_double("ACI_NO_THRESHOLD", 0.02,
  ...
  foptions.add_bool("ACI_FIRST_ITER_ROOTS", false,
                    "Compute all roots on first iteration?");
  foptions.add_bool("ACI_PRINT_WEIGHTS", false,
  ...
  foptions.add_bool("SAVE_FINAL_WFN", false,
  ...
  foptions.add_bool("UNPAIRED_DENSITY", false,
                    "Compute unpaired electron density");
}
...
void AdaptiveCI::set_aci_ints(SharedWavefunction ref_wfn,
                              std::shared_ptr<ForteIntegrals> ints) {
  ints_ = ints;
  shallow_copy(ref_wfn);
  reference_wavefunction_ = ref_wfn;
...
void AdaptiveCI::startup() {
  ...
  if (options_.get_str("ACI_INITIAL_SPACE") == "HF" or
    ...
    CI_Reference ref(reference_wavefunction_, options_, mo_space_info_, det,
                     multiplicity_, twice_ms_, wavefunction_symmetry_);
...
double AdaptiveCI::compute_energy() {
  ...
  if (ex_alg_ == "ROOT_COMBINE") {
    ...
    sparse_solver.diagonalize_hamiltonian_map(full_space, op_c, PQ_evals,
                                              PQ_evecs, nroot_, multiplicity_,
                                              diag_method_);
  }
  ...
  if (options_.get_bool("MRPT2")) {
    MRPT2 pt(reference_wavefunction_, options_, ints_, mo_space_info_,
             final_wfn_, PQ_evecs, PQ_evals);
  ...
  if (ex_alg_ == "ROOT_COMBINE") {
    ...
    DeterminantMap approx =
        approximate_wfn(final_wfn_, PQ_evecs, external_wfn_, new_evecs);
  ...
  return PQ_evals->get(options_.get_int("ACI_ROOT")) +
...
void AdaptiveCI::diagonalize_final_and_compute_rdms() {
  ...
  sparse_solver.set_guess_dimension(options_.get_int("DL_GUESS_SIZE"));
  sparse_solver.set_spin_project_full(false);
  sparse_solver.diagonalize_hamiltonian_map(final_wfn_, op_, final_evals,
                                            final_evecs, nroot_, multiplicity_,
                                            diag_method_);
...
#pragma omp parallel
  {
    ...
    size_t start = (tid < (max_P % ntd)) ? tid * bin_size
...
void AdaptiveCI::print_wfn(DeterminantMap &space, SharedMatrix evecs,
  ...
  for (int n = 0; n < nroot; ++n) {
    ...
    outfile->Printf("\n\n  Spin state for root %zu: S^2 = %5.6f, S = %5.3f, %s",
                    n, root_spin_vec_[n].first, root_spin_vec_[n].second,
                    state_label.c_str());
  }
  outfile->Flush();
}
      ...
      for (int p = 0; p < nactpi_[h]; ++p) {
        for (int q = 0; q < nactpi_[h]; ++q) {
          ...
          if ((occ >= no_thresh) and (occ <= (2.0 - no_thresh))) {
            ...
            oshell[p] +=
                (NO_A->get(h, p, q)) * (NO_A->get(h, p, q)) * (2 - occ) * occ;
          }
        }
      }
...
void AdaptiveCI::compute_aci(DeterminantMap &PQ_space, SharedMatrix &PQ_evecs,
  ...
  for (cycle = 0; cycle < max_cycle_; ++cycle) {
    ...
    Timer diag;
    sparse_solver.diagonalize_hamiltonian_map(P_space, op_, P_evals, P_evecs,
                                              num_ref_roots, multiplicity_,
                                              diag_method_);
    if (!quiet_mode_)
    ...
    if (follow and num_ref_roots > 1 and (cycle >= pre_iter_) and cycle > 0) {
      ref_root_ =
          root_follow(P_ref, P_ref_evecs, P_space, P_evecs, num_ref_roots);
    }
    ...
    if (sigma_method == "HZ") {
      ...
      outfile->Printf("\n  Time spent building strings      %1.6f s",
                      str.get());
    ...
    sparse_solver.diagonalize_hamiltonian_map(PQ_space, op_, PQ_evals, PQ_evecs,
                                              num_ref_roots, multiplicity_,
                                              diag_method_);
    ...
    if (follow and (num_ref_roots > 1) and (cycle >= pre_iter_)) {
      ref_root_ =
          root_follow(P_ref, P_ref_evecs, PQ_space, PQ_evecs, num_ref_roots);
    }
  ...
  for (int i = 0; i < nroot; ++i) {
    ...
    for (size_t I = 0, max_I = state.size(); I < max_I; ++I) {
      if (dets.has_det(state[I].first)) {
        ...
        bad_root.push_back(
            std::make_pair(dets.get_idx(state[I].first), state[I].second));
        nadd++;
      }
    }
...
DeterminantMap AdaptiveCI::approximate_wfn(DeterminantMap &PQ_space,
                                           SharedMatrix &evecs,

...
class SortedStringList {
  public:
    ...
    const std::vector<Determinant>& sorted_half_dets() const;

    const std::pair<size_t, size_t>& range(Determinant d) const;
    ...
    size_t find(const Determinant& d, size_t& first, size_t& last) const;

  protected:
    int nmo_ = 0;
    size_t num_dets_ = 0;
    DetSpinType zero_spin_type_;
    std::vector<Determinant> sorted_half_dets_;

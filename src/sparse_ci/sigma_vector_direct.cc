...
void SigmaVectorDirect::compute_sigma_aa_fast_search_group_ui64(SharedVector sigma,
                                                                SharedVector b) {
    timer energy_timer("SigmaVectorDirect:sigma_aa");
    for (size_t I = 0; I < size_; ++I)
        temp_sigma_[I] = 0.0;
    ...
    for (const auto& Ib : sorted_half_dets) {
        compute_aa_coupling_compare_group_ui64(Ib, b);
    }
...
void SigmaVectorDirect::compute_sigma_bb_fast_search_group_ui64(SharedVector sigma,
                                                                SharedVector b) {
    timer energy_timer("SigmaVectorDirect:sigma_bb");
    for (size_t I = 0; I < size_; ++I)
        temp_sigma_[I] = 0.0;
    ...
    for (const auto& Ia : sorted_half_dets) {
        compute_bb_coupling_compare_group_ui64(Ia, b);
    }
...
void SigmaVectorDirect::compute_aa_coupling_compare_group_ui64(const UI64Determinant::bit_t& Ib,
                                                               const SharedVector& b) {
    ...
    for (size_t posI = first_I; posI < last_I; ++posI) {
        ...
        size_t addI = b_sorted_string_list_ui64_.add(posI);
        ...
        for (size_t posJ = first_I; posJ < last_I; ++posJ) {
            ...
            if (ndiff == 2) {
                double h_ia = slater_rules_single_alpha(Ib, Ia, Ja, fci_ints_);
                temp_sigma_[posJ] += h_ia * b_I;
#if SIGMA_VEC_DEBUG
                count_aa++;
#endif
            } else if (ndiff == 4) {
                double h_ia = slater_rules_double_alpha_alpha(Ia, Ja, fci_ints_);
                temp_sigma_[posJ] += h_ia * b_I;
#if SIGMA_VEC_DEBUG
                count_aaaa++;
#endif
...
void SigmaVectorDirect::compute_bb_coupling_compare_group_ui64(const UI64Determinant::bit_t& Ia,
                                                               const SharedVector& b) {
    ...
    for (size_t posI = first_I; posI < last_I; ++posI) {
        ...
        for (size_t posJ = first_I; posJ < last_I; ++posJ) {
            ...
            if (ndiff == 2) {
                double h_ia = slater_rules_single_beta(Ia, Ib, Jb, fci_ints_);
                temp_sigma_[posJ] += h_ia * b_I;
#if SIGMA_VEC_DEBUG
                count_bb++;
#endif
            } else if (ndiff == 4) {
                double h_ia = slater_rules_double_beta_beta(Ib, Jb, fci_ints_);
                temp_sigma_[posJ] += h_ia * b_I;
#if SIGMA_VEC_DEBUG
                count_bbbb++;
#endif

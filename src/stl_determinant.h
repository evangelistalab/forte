...
class STLDeterminant {
  public:
    ...
    STLDeterminant(const std::vector<int>& occupation);
    /// Construct the determinant from an occupation vector that
    /// specifies the alpha and beta strings.  occupation = [Ia,Ib]
    STLDeterminant(const std::vector<bool>& occupation);
    ...
    /// Equal operator
    bool operator==(const STLDeterminant& lhs) const;
    /// Less than operator
    bool operator<(const STLDeterminant& lhs) const;
    /// XOR operator
    STLDeterminant operator^(const STLDeterminant& lhs) const;
    ...
    std::vector<std::pair<STLDeterminant, double>> spin_plus() const;
    /// Apply S- to this determinant
    std::vector<std::pair<STLDeterminant, double>> spin_minus() const;
    /// Compute the matrix element of the S^2 operator between this determinant
    /// and a given one
    double spin2_slow(const STLDeterminant& rhs) const;
    /// Return the eigenvalue of Sz
    double spin_z() const;
    /// Compute the matrix element of the S^2 operator between this determinant
    /// and a given one
    double spin2(const STLDeterminant& rhs) const;
    ...
    /// complete set
    void enforce_spin_completeness(std::vector<STLDeterminant>& det_space);

    struct Hash {
        std::size_t operator()(const psi::forte::STLDeterminant& bs) const {

...
void SOMRDSRG::startup()
{
    ...
    Delta2.iterate([&](const std::vector<size_t>& i,const std::vector<SpinType>& spin,double& value){
        ...
        } else if (spin[0] == AlphaSpin and spin[1] == BetaSpin and spin[2] == AlphaSpin and spin[3] == BetaSpin){
            value = Fa[i[0]] + Fb[i[1]] - Fa[i[2]] - Fb[i[3]];
        } else if (spin[0] == AlphaSpin and spin[1] == BetaSpin and spin[2] == BetaSpin and spin[3] == AlphaSpin){
            value = Fa[i[0]] + Fb[i[1]] - Fb[i[2]] - Fa[i[3]];
        } else if (spin[0] == BetaSpin and spin[1] == AlphaSpin and spin[2] == AlphaSpin and spin[3] == BetaSpin){
            value = Fb[i[0]] + Fa[i[1]] - Fa[i[2]] - Fb[i[3]];
        } else if (spin[0] == BetaSpin and spin[1] == AlphaSpin and spin[2] == BetaSpin and spin[3] == AlphaSpin){
            value = Fb[i[0]] + Fa[i[1]] - Fb[i[2]] - Fa[i[3]];
        } else if (spin[0] == BetaSpin and spin[1] == BetaSpin and spin[2] == BetaSpin and spin[3] == BetaSpin){
    ...
    RInvDelta1.iterate([&](const std::vector<size_t>& i,const std::vector<SpinType>& spin,double& value){
        if (spin[0] == AlphaSpin and spin[1] == AlphaSpin){
    ...
    RInvDelta2.iterate([&](const std::vector<size_t>& i,const std::vector<SpinType>& spin,double& value){
        if (spin[0] == AlphaSpin and spin[1] == AlphaSpin and spin[2] == AlphaSpin and spin[3] == AlphaSpin){
            value = Fa[i[0]] + Fa[i[1]] - Fa[i[2]] - Fa[i[3]];
        } else if (spin[0] == AlphaSpin and spin[1] == BetaSpin and spin[2] == AlphaSpin and spin[3] == BetaSpin){
            value = Fa[i[0]] + Fb[i[1]] - Fa[i[2]] - Fb[i[3]];
...
double SOMRDSRG::compute_energy()
{
    ...
    E0_ = molecule_->nuclear_repulsion_energy() + ints_->frozen_core_energy();
    E0_ += F["qp"] * Gamma1["pq"];
...
//        }
        ...
        T1.citerate([&](const std::vector<size_t>& i,const std::vector<SpinType>& spin,const double& value){
            if (std::fabs(value) > std::fabs(max_T1)) max_T1 = value;
        });

        T2.citerate([&](const std::vector<size_t>& i,const std::vector<SpinType>& spin,const double& value){
            if (std::fabs(value) > std::fabs(max_T2)) max_T2 = value;
        });
...
//        double norm_Hbar2_ex = std::sqrt(0.25 * norm_H2aa * norm_H2aa + norm_H2ab * norm_H2ab + 0.25 * norm_H2bb * norm_H2bb);
        ...
        outfile->Printf("\n    @SO-MR-DSRG %4d %20.12f %11.3e %10.3e %10.3e %7.4f %7.4f %7.4f %7.4f",cycle,energy,delta_energy,0,0,norm_T1,norm_T2,max_T1,max_T2);
...
void SOMRDSRG::H_eq_commutator_C_T(double factor,
                                BlockedTensor& F,
                                BlockedTensor& V,
                                BlockedTensor& T1,
                                BlockedTensor& T2,
                                ...
                                int order)
{
    ...
    H0 += 0.250000 * Eta1["p2,p0"] * Eta1["p3,p1"] * Gamma1["h0,h2"] * Gamma1["h1,h3"] * T2["h2,h3,p2,p3"] * V["p0,p1,h0,h1"];
    ...
    H1["g0,p0"] += -0.500000 * Gamma1["h1,h0"] * Gamma1["h3,h2"] * T2["h0,h2,p0,p1"] * V["g0,p1,h1,h3"];
    H1["g0,p0"] += -0.500000 * Gamma1["a1,a0"] * T2["h0,h1,p0,a1"] * V["g0,a0,h0,h1"];
    H1["g0,p0"] += 1.000000 * Gamma1["a1,a0"] * Gamma1["h1,h0"] * T2["h0,h2,p0,a1"] * V["g0,a0,h1,h2"];
    H1["h0,g0"] += 0.500000 * Gamma1["h2,h1"] * T2["h0,h1,p0,p1"] * V["p0,p1,g0,h2"];
    H1["h0,g0"] += 0.500000 * Gamma1["a1,a0"] * Gamma1["a3,a2"] * T2["h0,h1,a1,a3"] * V["a0,a2,g0,h1"];
    H1["h0,g0"] += -1.000000 * Gamma1["a1,a0"] * Gamma1["h2,h1"] * T2["h0,h1,p0,a1"] * V["p0,a0,g0,h2"];

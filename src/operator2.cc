...
WFNOperator2::build_H_sparse(const DeterminantMap2& wfn) {
    Timer build;
    size_t size = wfn.size();
    std::vector<std::pair<std::vector<size_t>, std::vector<double>>> H_sparse(size);
    size_t n_nonzero = 0;

    const std::vector<STLDeterminant>& dets = wfn.determinants();
                    ...
                    for( auto& aaaJ_mo : aa_cre_list_[aaJ_add] ){
                        ...
                        if ((p != r) and (q != s) and (p != s) and (q != r)){
                            ...
                            const double HIJ = sign_pq * sign_rs *
                                STLDeterminant::fci_ints_->tei_aa(p,q,r,s);
                ...
                for( auto& bbJ_mo_sign : bb_ann_list_[J] ){
                    ...
                    for( auto& bbbJ_mo : bb_cre_list_[bbJ_add] ){
                        ...
                        if ((p != r) and (q != s) and (p != s) and (q != r)){
                            ...
                            const double HIJ = sign_pq * sign_rs *
                                STLDeterminant::fci_ints_->tei_bb(p,q,r,s);
                ...
                for( auto& abJ_mo_sign : ab_ann_list_[J] ){
                    ...
                    for( auto& abbJ_mo : ab_cre_list_[abJ_add] ){
                        ...
                        if ( (p != r) and (q != s) ){
                            ...
                            const double HIJ = sign_pq * sign_rs *
                                STLDeterminant::fci_ints_->tei_ab(p,q,r,s);
                            ids.resize( id+1 );
                            H_vals.resize( id+1 );
                            ids[id] = I;
                            H_vals[id] = HIJ;
...
double WFNOperator2::s2(DeterminantMap2& wfn, SharedMatrix& evecs, int root) {
    double S2 = 0.0;
    const stldet_hash<size_t>& wfn_map = wfn.wfn_hash();

    for (auto& det : wfn_map) {
        // Compute diagonal
        // PhiI = PhiJ
        STLDeterminant PhiI = det.first;
...
void WFNOperator2::add_singles(DeterminantMap2& wfn) {

    stldet_hash<size_t>& wfn_map = wfn.wfn_hash();

    DeterminantMap2 singles;
    // Loop through determinants, generate singles and add them to the wfn
    // Alpha excitations
    for (auto& I : wfn_map) {
        STLDeterminant det = I.first;
...
void WFNOperator2::add_doubles(DeterminantMap2& wfn) {
    const stldet_hash<size_t>& wfn_map = wfn.wfn_hash();

    DeterminantMap2 external;
...
void WFNOperator2::build_strings(DeterminantMap2& wfn) {
    beta_strings_.clear();
    ...
    const stldet_hash<size_t>& wfn_map = wfn.wfn_hash();
    {
        stldet_hash<size_t> beta_str_hash;
        size_t nbeta = 0;
        for (auto& I : wfn_map) {
            // Grab mutable copy of determinant
            STLDeterminant detI = I.first;
            detI.zero_spin(0);

            stldet_hash<size_t>::iterator it = beta_str_hash.find(detI);
            size_t b_add;
    ...
    {
        stldet_hash<size_t> alfa_str_hash;
        size_t nalfa = 0;
        for (auto& I : wfn_map) {
            // Grab mutable copy of determinant
            STLDeterminant detI = I.first;
            ...
            stldet_hash<size_t>::iterator it = alfa_str_hash.find(detI);
            size_t a_add;
    ...
    for (auto& I : wfn_map) {
        ...
        for (int i = 0, nalfa = aocc.size(); i < nalfa; ++i) {
            int ii = aocc[i];
            STLDeterminant ann_det(detI);
            ann_det.set_alfa_bit(ii, false);

            size_t a_add;
            stldet_hash<size_t>::iterator it = alfa_str_hash.find(ann_det);
            if (it == alfa_str_hash.end()) {
                a_add = naalpha;
...
void WFNOperator2::op_s_lists(DeterminantMap2& wfn) {
    ...
    for (size_t b = 0, max_b = beta_strings_.size(); b < max_b; ++b) {
        ...
        stldet_hash<int> map_a_ann;
        for (size_t I = 0, maxI = c_dets.size(); I < maxI; ++I) {
            size_t index = c_dets[I];
            const STLDeterminant& detI = dets[index];
            const std::vector<int>& aocc = detI.get_alfa_occ();
            int noalfa = aocc.size();

            for (int i = 0; i < noalfa; ++i) {
                int ii = aocc[i];
                STLDeterminant detJ(detI);
...
void WFNOperator2::tp_s_lists(DeterminantMap2& wfn) {
    ...
    Timer aa;
    {
        for (size_t b = 0, max_b = beta_strings_.size(); b < max_b; ++b) {
            ...
            stldet_hash<int> map_aa_ann;
            std::vector<size_t> c_dets = beta_strings_[b];
            size_t max_I = c_dets.size();
            for (size_t I = 0; I < max_I; ++I) {
                size_t idx = c_dets[I];
                STLDeterminant detI = dets[idx];
                ...
                for (int i = 0; i < noalfa; ++i) {
                    for (int j = i + 1; j < noalfa; ++j) {
                        ...
                        stldet_hash<int>::iterator it = map_aa_ann.find(detJ);
                        size_t detJ_add;
            ...
            for (size_t I = 0; I < max_I; ++I) {
                ...
                for (int i = 0, ij = 0; i < nobeta; ++i) {
                    for (int j = i + 1; j < nobeta; ++j, ++ij) {
                        ...
                        stldet_hash<int>::iterator it = map_bb_ann.find(detJ);
                        size_t detJ_add;
                        // Add detJ to map if it isn't there
        ...
        for (size_t a = 0, max_a = alpha_a_strings_.size(); a < max_a; ++a) {
            ...
            for (size_t I = 0; I < max_I; ++I) {
                ...
                for (size_t j = 0; j < nobeta; ++j) {
                    ...
                    stldet_hash<int>::iterator it = map_ab_ann.find(detJ);
                    size_t detJ_add;
...
void WFNOperator2::clear_op_s_lists() {
    a_list_.clear();
    b_list_.clear();
}

void WFNOperator2::clear_tp_s_lists() {
    aa_list_.clear();
    bb_list_.clear();
    ab_list_.clear();
}

void WFNOperator2::three_s_lists(DeterminantMap2& wfn) {
                ...
                for (int i = 0; i < noalfa; ++i) {
                    for (int j = i + 1; j < noalfa; ++j) {
                        for (int k = j + 1; k < noalfa; ++k) {
                            ...
                            stldet_hash<int>::iterator it = map_aaa.find(detJ);
                            size_t detJ_add;
                            // Add detJ to map if it isn't there
                            if (it == map_aaa.end()) {
                                detJ_add = naa_ann;
    ...
    /// AAB coupling
    {
        ...
        for (auto& I : wfn_map) {
            ...
            for (int i = 0, nbeta = bocc.size(); i < nbeta; ++i) {
                ...
                size_t b_add;
                stldet_hash<size_t>::iterator it = beta_str_hash.find(ann_det);
                if (it == beta_str_hash.end()) {
                    b_add = nabeta;
            ...
            for (int I = 0; I < max_I; ++I) {
                ...
                for (int i = 0, jk = 0; i < noalfa; ++i) {
                    for (int j = i + 1; j < noalfa; ++j, ++jk) {
                        ...
                        stldet_hash<int>::iterator hash_it = aab_ann_map.find(detJ);
                        size_t detJ_add;
            ...
            for (int I = 0; I < max_I; ++I) {
                ...
                for (int j = 0, jk = 0; j < nobeta; ++j) {
                    for (int k = j + 1; k < nobeta; ++k, ++jk) {
                        ...
                        stldet_hash<int>::iterator hash_it = abb_ann_map.find(detJ);
                        size_t detJ_add;
                ...
                for (int i = 0; i < nobeta; ++i) {
                    for (int j = i + 1; j < nobeta; ++j) {
                        for (int k = j + 1; k < nobeta; ++k) {
                            ...
                            stldet_hash<int>::iterator hash_it = bbb_ann_map.find(detJ);
                            size_t detJ_add;

...
DSRG_MRPT2::DSRG_MRPT2(Reference reference, SharedWavefunction ref_wfn,
...
void DSRG_MRPT2::startup() {
    ...
    BTF_->add_composite_mo_space("g", "pqrs",
    ...
    if (relax_ref_ != "NONE") {
        ...
        if (options_.get_bool("FORM_HBAR3")) {
            Hbar3_ = BTF_->build(tensor_type_, "3-body Hbar",
                                 spin_cases({"aaaaaa"}));
        }
    }
...
void DSRG_MRPT2::print_summary() {
    ...
    if (internal_amp_) {
        calculation_info_string.push_back(
            {"internal_amp", options_.get_str("INTERNAL_AMP")});
        calculation_info_string.push_back(
...
void DSRG_MRPT2::compute_t2() {
    ...
    if (print_denom) {
        ...
        myfile << acore_mos_.size() + aactv_mos_.size() << " "
               ...
               << aactv_mos_.size() + avirt_mos_.size() << " \n";
        T2_.iterate([&](const std::vector<size_t>& i,
                        const std::vector<SpinType>& spin, double&) {
            if ((spin[0] == AlphaSpin) and (spin[1] == AlphaSpin)) {
                double D = dsrg_source_->compute_renormalized_denominator(
                    Fa_[i[0]] + Fa_[i[1]] - Fa_[i[2]] - Fa_[i[3]]);
                D *= 1.0 +
                     dsrg_source_->compute_renormalized(Fa_[i[0]] + Fa_[i[1]] -
                                                        Fa_[i[2]] - Fa_[i[3]]);
                myfile << i[0] << " " << i[1] << " " << i[2] << " " << i[3]
                       << " " << D << " \n";
            }
        });
    }
...
double DSRG_MRPT2::E_VT2_6() {
    ...
    // aaa
    BlockedTensor temp =
        ambit::BlockedTensor::build(tensor_type_, "temp", {"aaaaaa"});
...
void DSRG_MRPT2::H1_T1_C1(BlockedTensor& H1, BlockedTensor& T1,
...
void DSRG_MRPT2::H1_T2_C1(BlockedTensor& H1, BlockedTensor& T2,
                          const double& alpha, BlockedTensor& C1) {
    Timer timer;
...
void DSRG_MRPT2::H2_T1_C1(BlockedTensor& H2, BlockedTensor& T1,
                          const double& alpha, BlockedTensor& C1) {
    Timer timer;
...
void DSRG_MRPT2::H2_T2_C1(BlockedTensor& H2, BlockedTensor& T2,
                          const double& alpha, BlockedTensor& C1) {
    Timer timer;
    BlockedTensor temp;
    ...
    C1["ik"] +=
        0.5 * alpha * T2["ijux"] * Gamma1_["xy"] * Gamma1_["uv"] * H2["vykj"];
    C1["IK"] +=
        0.5 * alpha * T2["IJUX"] * Gamma1_["XY"] * Gamma1_["UV"] * H2["VYKJ"];
    ...
    C1["kw"] -=
        0.5 * alpha * T2["vywb"] * Eta1_["uv"] * Eta1_["xy"] * H2["kbux"];
    C1["KW"] -=
        0.5 * alpha * T2["VYWB"] * Eta1_["UV"] * Eta1_["XY"] * H2["KBUX"];
...
void DSRG_MRPT2::H1_T2_C2(BlockedTensor& H1, BlockedTensor& T2,
...
void DSRG_MRPT2::H2_T1_C2(BlockedTensor& H2, BlockedTensor& T1,
                          const double& alpha, BlockedTensor& C2) {
    Timer timer;
...
void DSRG_MRPT2::H2_T2_C2(BlockedTensor& H2, BlockedTensor& T2,
                          const double& alpha, BlockedTensor& C2) {
    Timer timer;
    ...
    BlockedTensor temp =
        ambit::BlockedTensor::build(tensor_type_, "temp", {"aaaa"});
...
void DSRG_MRPT2::H2_T2_C3(BlockedTensor& H2, BlockedTensor& T2,
...
DSRG_MRPT2::diagonalize_Fock_diagblocks(BlockedTensor& U) {
...
ambit::Tensor DSRG_MRPT2::separate_tensor(ambit::Tensor& tens,
...
void DSRG_MRPT2::print_amp_summary(
...
void DSRG_MRPT2::print_intruder(
    const std::string& name,
    const std::vector<std::pair<std::vector<size_t>, double>>& list) {
    int rank = name.size();

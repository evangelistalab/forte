...
/**
 */
std::vector<StringSubstitution>& StringLists::get_alfa_vvoo_list(size_t p, size_t q, size_t r, size_t s, int h)
{
...
/**
 */
std::vector<StringSubstitution>& StringLists::get_beta_vvoo_list(size_t p, size_t q, size_t r, size_t s, int h)
{
    boost::tuple<size_t,size_t,size_t,size_t,int> pqrs_pair(p,q,r,s,h);
 ...
 * that is: |J> = ± a^{+}_p a_q a^{+}_r a_s |I>. p,q,r, and s are absolute indices.
...
void StringLists::make_vvoo(GraphPtr graph,VVOOList& list,int p, int q,int r, int s)
{
    ...
    bool overlap = false;
    if((a[0] == a[1]) || (a[1] == a[2]) || (a[2] == a[3])) overlap = true;
    ...
    if(k >= 0 and n >= 0){
        ...
        for(int h = 0; h < nirrep_; ++h){
            ...
            do{
                ...
                if(graph->sym(I) == h){
                    // Copy I to J
                    for (int i = 0; i < ncmo_ ; ++i) J[i] = I[i];
                    short sign = 1;
                    // Apply a^{+}_p a^{+}_q a_s a_r to I
                    for(int i = s; i < r; ++i) if(J[i]) sign *= -1;
                    J[r] = false;
                    J[s] = false;
                    if(!J[q]){ // q = 0
                        J[q] = true;
                        for(int i = 0; i < q; ++i) if(J[i]) sign *= -1;
                        if(!J[p]){ // p = 0
                            J[p] = true;
                            for(int i = 0; i < p; ++i) if(J[i]) sign *= -1;
                            // Add the sting only of irrep(I) is h
                            list[pqrs_pair].push_back(StringSubstitution(sign,graph->rel_add(I),graph->rel_add(J)));
                        }
                    }
                }
 ...
 * indices.
 */
void StringLists::make_VOVO(GraphPtr graph,VOVOList& list,
                            int p, int q,int r, int s)
{
    ...
    for(int h = 0; h < nirrep_; ++h){
        ...
        for(int i = n - k; i < n; ++i) I[i] = true;  // 1
        do{
            if(graph->sym(I) == h){
                // Copy I to J
                for (int i = 0; i < ncmo_ ; ++i) J[i] = I[i];
                ...
                if(J[s]){  // s = 1
                    J[s] = false;
                    for(int i = 0; i < s; ++i) if(J[i]) sign *= -1;
                    if(!J[r]){ // r = 0
                        J[r] = true;
                        for(int i = 0; i < r; ++i) if(J[i]) sign *= -1;
                        if(J[q]){ // q = 1
                            J[q] = false;
                            for(int i = 0; i < q; ++i) if(J[i]) sign *= -1;
                            if(!J[p]){ // p = 0
 ...
 */
std::vector<StringSubstitution>& StringLists::get_alfa_vovo_list(size_t p, size_t q, size_t r, size_t s, int h)
{
    boost::tuple<size_t,size_t,size_t,size_t,int> pqrs_pair(p,q,r,s,h);
 ...
 * that is: J = ± a^{+}_p a_q I. p and q are absolute indices.
 */
std::vector<StringSubstitution>& StringLists::get_beta_vovo_list(size_t p, size_t q, size_t r, size_t s, int h)
{
    boost::tuple<size_t,size_t,size_t,size_t,int> pqrs_pair(p,q,r,s,h);

...
double test_bitset_determinant2_copy_constructor(int nmo,int nset,int repeat)
{
...
{
    std::vector<STLBitsetDeterminant> det_vec;
    std::unordered_map<STLBitsetDeterminant,double,STLBitsetDeterminant::Hash> det_hash;
    for (int n = 0; n < repeat; ++n){
        det_vec.push_back(random_bitset_determinant2(nmo,nset));
    }
    auto start = chrono::steady_clock::now();
...
std::vector<std::pair<std::string,double>>  run_tests(int nmo,int nset, int nrepeat){
    std::vector<std::pair<std::string,double>> test_results;

    outfile->Printf("\n\n\n  ==> nmo: %d, nset: %d, nrepeat: %d <==\n",nmo,nset,nrepeat);

    // Test generation
    test_results.push_back(std::make_pair("DynamicBitsetDeterminant generate",
                                          test_bitset_determinant_generate(nmo,nset,nrepeat)));
    test_results.push_back(std::make_pair("STLBitsetDeterminant generate",
                                          test_bitset_determinant2_generate(nmo,nset,nrepeat)));

    test_results.push_back(std::make_pair("DynamicBitsetDeterminant place in map",
                                          test_bitset_determinant_place_in_map(nmo,nset,nrepeat)));

    // Test placing in hash
    test_results.push_back(std::make_pair("DynamicBitsetDeterminant place in hash",
                                          test_bitset_determinant_place_in_hash(nmo,nset,nrepeat)));
    if (nmo <= 256){
        test_results.push_back(std::make_pair("STLBitsetDeterminant place in hash",
                                              test_bitset_determinant2_place_in_hash(nmo,nset,nrepeat)));
    }
    if (nmo <= 64){
        test_results.push_back(std::make_pair("bitset_128 place in hash",
                                              test_bitset_128_place_in_hash(nmo,nset,nrepeat)));
    }
    if (nmo <= 128){
        test_results.push_back(std::make_pair("bitset_256 place in hash",
                                              test_bitset_256_place_in_hash(nmo,nset,nrepeat)));
    }

    // Test placing in vector
    test_results.push_back(std::make_pair("DynamicBitsetDeterminant place in vector",
                                          test_bitset_determinant_place_in_vector(nmo,nset,nrepeat)));
    test_results.push_back(std::make_pair("STLBitsetDeterminant place in vector",
                                          test_bitset_determinant2_place_in_vector(nmo,nset,nrepeat)));
    test_results.push_back(std::make_pair("bitset_256 place in vector",
    ...
    for (auto kv : test_results){
        ...
        outfile->Printf("\n%-s%s%.3f ns/operation",kv.first.c_str(),filler.c_str(),value);
    }

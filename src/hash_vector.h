...
public:
    ...
    explicit HashVector( const std::unordered_set<Key, Hash_2>& other );
    template<class Value, class Hash_2>
    explicit HashVector( const std::unordered_map<Key, Value, Hash_2>& other );
    template<class Value, class Hash_2>
    explicit HashVector( const std::unordered_map<Key, Value, Hash_2>& other, std::vector<Value>& values );
    HashVector( const HashVector<Key, Hash>& other );
    ...
    template<class Hash_2>
    std::vector<size_t> merge( const HashVector<Key, Hash_2>& source );
    template<class Value, class Hash_2>
    void merge( const HashVector<Key, Hash_2>& source, const std::vector<Value>& src_values, std::vector<Value>& values );
    template<class Value, class Hash_2>
    void merge( const HashVector<Key, Hash_2>& source, const std::vector<Value>& src_values, std::vector<Value>& values, const std::function<Value(Value, Value)>& f, const Value& default_value, bool change_this = true );
    std::vector<size_t> merge( const std::vector<Key>& source );
    template<class Value>
    void merge( const std::vector<Key>& source, const std::vector<Value>& src_values, std::vector<Value>& values );
    template<class Value>
    void merge( const std::vector<Key>& source, const std::vector<Value>& src_values, std::vector<Value>& values, const std::function<Value(Value, Value)>& f, const Value& default_value, bool change_this = true );
    template<class Value>
    void merge( const std::vector<std::pair<Key, Value> >& source, std::vector<Value>& values );
    template<class Value>
    void merge( const std::vector<std::pair<Key, Value> >& source, std::vector<Value>& values, const std::function<Value(Value, Value)>& f, const Value& default_value, bool change_this = true );
    template<class Hash_2>
    void merge( const std::unordered_set<Key, Hash_2>& source );
    template<class Value, class Hash_2>
    void merge( const std::unordered_map<Key, Value, Hash_2>& source, std::vector<Value>& values );
    template<class Value, class Hash_2>
    void merge( const std::unordered_map<Key, Value, Hash_2>& source, std::vector<Value>& values, const std::function<Value(Value, Value)>& f, const Value& default_value, bool change_this = true );
    ...
    template<class Value>
    std::unordered_map<Key, Value, Hash> toUnordered_map( const std::vector<Value> &values );
...
const size_t HashVector<Key, Hash>::npos;

namespace std {
    template<class Key, class Hash>
    void swap( HashVector<Key, Hash>& a, HashVector<Key, Hash>& b ) {
        a.swap(b);
    }
}

template<class Key, class Hash>
HashVector<Key, Hash>::HashVector()
{
    this->clear();
}

template<class Key, class Hash>
HashVector<Key, Hash>::HashVector( size_t count ) {
    this->clear();
    this->reserve(count);
}

template<class Key, class Hash>
HashVector<Key, Hash>::HashVector( const std::vector<Key> &other ) {
    this->clear();
...
template<class Key, class Hash> template<class Hash_2>
...
template<class Key, class Hash> template<class Value, class Hash_2>
HashVector<Key, Hash>::HashVector( const std::unordered_map<Key, Value, Hash_2>& other ) {
    this->clear();
    this->reserve(other.size());
...
template<class Key, class Hash> template<class Value, class Hash_2>
HashVector<Key, Hash>::HashVector( const std::unordered_map<Key, Value, Hash_2>& other, std::vector<Value>& values ) {
    this->clear();
    this->reserve(other.size());
    values.clear();
...
template<class Key, class Hash>
HashVector<Key, Hash>::HashVector( const HashVector<Key, Hash>& other ) :
    vec(other.vec),
...
template<class Key, class Hash>
void HashVector<Key, Hash>::double_buckets() {
    ...
    if (current_size >= num_bucket) {
        ...
        for (size_t i = 0; i < current_size; ++i) {
            ...
            if (new_bucket_index != old_bucket_index && this->begin_index[new_bucket_index] == npos) {
                size_t pre_index = npos;
...
template<class Key, class Hash>
void HashVector<Key, Hash>::half_buckets() {
    size_t new_num_bucket = num_bucket >> 1;
...
template<class Key, class Hash>
inline void HashVector<Key, Hash>::update_current_max_load_size() {
    this->current_max_load_size = max_load * num_bucket;
}

template<class Key, class Hash>
const Key& HashVector<Key, Hash>::operator[]( size_t pos ) const {
    return this->vec[pos].value;
}

template<class Key, class Hash>
size_t HashVector<Key, Hash>::find( const Key& key ) const {
    size_t bucket_index = Hash()(key) % num_bucket;
    if (this->begin_index[bucket_index] != npos) {
        size_t temp_index = this->begin_index[bucket_index];
        while (temp_index != npos) {
...
template<class Key, class Hash>
size_t HashVector<Key, Hash>::size() const {
    return this->vec.size();
}

template<class Key, class Hash>
size_t HashVector<Key, Hash>::max_size() const {
    size_t vec_max_size = vec.max_size();
    return vec_max_size < npos ? vec_max_size : npos - 1;
}

template<class Key, class Hash>
size_t HashVector<Key, Hash>::capacity() const {
    return this->vec.capacity();
}

template<class Key, class Hash>
void HashVector<Key, Hash>::clear() {
    this->vec.clear();
...
template<class Key, class Hash>
size_t HashVector<Key, Hash>::add(const Key &key ) {
...
std::pair<size_t, size_t> HashVector<Key, Hash>::erase_by_key( const Key& key ) {
    ...
    std::tie(key_bucket_index, key_pre_index, key_index) = find_detail_by_key(key);
    if (key_index == npos) return std::make_pair(npos, npos);
...
template<class Key, class Hash> template<class Value, class Hash_2>
void HashVector<Key, Hash>::merge( const HashVector<Key, Hash_2>& source, const std::vector<Value>& src_values, std::vector<Value>& values ) {
...
template<class Key, class Hash> template<class Value, class Hash_2>
void HashVector<Key, Hash>::merge(const HashVector<Key, Hash_2>& source, const std::vector<Value>& src_values, std::vector<Value>& values, const std::function<Value(Value, Value)>& f, const Value& default_value, bool change_this ) {
    size_t original_size = current_size;
    size_t merge_size = source.size();
    if (change_this) {
...
template<class Key, class Hash> template<class Value>
void HashVector<Key, Hash>::merge( const std::vector<Key>& source, const std::vector<Value>& src_values, std::vector<Value>& values ) {
    size_t original_size = current_size;
    size_t merge_size = source.size();
    for (size_t i = 0; i < merge_size; ++i) {
        size_t index = this->add(source[i]);
...
template<class Key, class Hash> template<class Value>
void HashVector<Key, Hash>::merge(const std::vector<Key>& source, const std::vector<Value>& src_values, std::vector<Value>& values, const std::function<Value(Value, Value)>& f, const Value& default_value, bool change_this ) {
    size_t original_size = current_size;
    size_t merge_size = source.size();
    if (change_this) {
        std::vector<bool> intersects(original_size, false);
...
template<class Key, class Hash> template<class Value>
void HashVector<Key, Hash>::merge( const std::vector<std::pair<Key, Value> >& source, std::vector<Value>& values ) {
    size_t original_size = current_size;
...
template<class Key, class Hash> template<class Value>
void HashVector<Key, Hash>::merge( const std::vector<std::pair<Key, Value> >& source, std::vector<Value>& values, const std::function<Value(Value, Value)>& f, const Value& default_value, bool change_this ) {
    size_t original_size = current_size;
    if (change_this) {
        std::vector<bool> intersects(original_size, false);
        for (std::pair<Key, Value> kv : source) {
...
template<class Key, class Hash> template<class Value, class Hash_2>
void HashVector<Key, Hash>::merge( const std::unordered_map<Key, Value, Hash_2>& source, std::vector<Value>& values ) {
    size_t original_size = current_size;
    for (std::pair<Key, Value> kv : source) {
        size_t index = this->add(kv.first);
        if (index < original_size) {
...
template<class Key, class Hash> template<class Value, class Hash_2>
void HashVector<Key, Hash>::merge( const std::unordered_map<Key, Value, Hash_2>& source, std::vector<Value>& values, const std::function<Value(Value, Value)>& f, const Value& default_value, bool change_this ) {
    size_t original_size = current_size;
    if (change_this) {
        std::vector<bool> intersects(original_size, false);
        for (std::pair<Key, Value> kv : source) {
...
template<class Key, class Hash>
void HashVector<Key, Hash>::swap( HashVector<Key, Hash>& other ) {
    vec.swap(other.vec);
...
template<class Key, class Hash>
size_t HashVector<Key, Hash>::bucket_count() const {
    return this->num_bucket;
}

template<class Key, class Hash>
size_t HashVector<Key, Hash>::max_bucket_count() const {
    size_t begin_index_max_size = begin_index.max_size();
    return begin_index_max_size < npos ? begin_index_max_size : npos - 1;
}

template<class Key, class Hash>
size_t HashVector<Key, Hash>::bucket_size( size_t n ) const {
    if (n >= this->num_bucket)
        return npos;
//    if (this->begin_index[n] == npos)
//        return 0;
...
template<class Key, class Hash>
size_t HashVector<Key, Hash>::bucket( const Key& key ) const {
    size_t key_bucket_index, key_pre_index, key_index;
    std::tie(key_bucket_index, key_pre_index, key_index) = find_detail_by_key(key);
    if (key_index == npos)
        return npos;
...
template<class Key, class Hash>
float HashVector<Key, Hash>::load_factor() const {
    return ((float) current_size)/num_bucket;
}

template<class Key, class Hash>
float HashVector<Key, Hash>::max_load_factor() const {
    return this->max_load;
}

template<class Key, class Hash>
void HashVector<Key, Hash>::max_load_factor( float ml ) {
    this->max_load = ml;
    update_current_max_load_size();
...
template<class Key, class Hash>
void HashVector<Key, Hash>::reserve( size_t count ) {
...
template<class Key, class Hash>
void HashVector<Key, Hash>::shrink_to_fit() {
...
template<class Key, class Hash>
std::vector<size_t> HashVector<Key, Hash>::optimize() {
...
template<class Key, class Hash>
std::vector<Key> HashVector<Key, Hash>::toVector() {
    std::vector<Key> keys;
...
template<class Key, class Hash> template<class Value>
std::unordered_map<Key, Value, Hash> HashVector<Key, Hash>::toUnordered_map( const std::vector<Value> &values ) {
...
template<class Key, class Hash>
void HashVector<Key, Hash>::print_out() const {
    std::cout << "\tBEGIN Printing the HashVector: " << std::endl;
    std::cout << "\tBucket\tcount\tbegin_index" << std::endl;
    for (size_t i = 0; i < this->begin_index.size(); ++i) {
        if (this->begin_index[i] == npos)
            std::cout << "\t" << i << "\t" << this->bucket_size(i) << "\tEND" << std::endl;
        else
            std::cout << "\t" << i << "\t" << this->bucket_size(i) << "\t" << this->begin_index[i] << std::endl;
    }
    std::cout << "\t" << "vec:";
    for (CINode<Key> temp : this->vec) {
        if (temp.next == npos)
            std::cout << " (" << temp.value.getValue() << ", END" << ")";
        else
            std::cout << " (" << temp.value.getValue() << ", " << temp.next << ")";
    }
    std::cout << "\t" << std::endl;
    std::cout << "\t" << "Load factor: " << this->load_factor() << std::endl;
    std::cout << "\t" << "BeginIndex capacity: " << this->begin_index.capacity() << std::endl;
    std::cout << "\t" << "vec        capacity: " << this->vec.capacity() << std::endl;
    std::cout << "\tEND Printing the HashVector." << std::endl << std::endl;
}

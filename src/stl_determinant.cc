...
STLDeterminant::STLDeterminant() {}

STLDeterminant::STLDeterminant(const std::vector<int>& occupation) {
    for (int p = 0; p < 2 * nmo_; ++p)
        bits_[p] = occupation[p];
}

STLDeterminant::STLDeterminant(const std::vector<bool>& occupation) {
    for (int p = 0; p < 2 * nmo_; ++p)
        bits_[p] = occupation[p];
}
...
STLDeterminant STLDeterminant::operator^(const STLDeterminant& lhs) const {
    STLDeterminant ndet(bits_ ^ lhs.bits());
    return ndet;
}

const bit_t& STLDeterminant::bits() const { return bits_; }

bool STLDeterminant::get_alfa_bit(int n) const { return bits_[n]; }

bool STLDeterminant::get_beta_bit(int n) const { return bits_[n + nmo_]; }

void STLDeterminant::set_alfa_bit(int n, bool value) { bits_[n] = value; }

void STLDeterminant::set_beta_bit(int n, bool value) { bits_[n + nmo_] = value; }

std::vector<bool> STLDeterminant::get_alfa_bits_vector_bool() {
...
std::vector<bool> STLDeterminant::get_beta_bits_vector_bool() {
    std::vector<bool> result(nmo_);
    for (int n = 0; n < nmo_; ++n) {
...
const std::vector<bool> STLDeterminant::get_alfa_bits_vector_bool() const {
    std::vector<bool> result(nmo_);
    for (int n = 0; n < nmo_; ++n) {
        result[n] = bits_[n];
    }
...
const std::vector<bool> STLDeterminant::get_beta_bits_vector_bool() const {
    std::vector<bool> result(nmo_);
    for (int n = 0; n < nmo_; ++n) {
        result[n] = bits_[nmo_ + n];
    }
...
std::vector<int> STLDeterminant::get_alfa_occ() {
    std::vector<int> occ;
    for (int p = 0; p < nmo_; ++p) {
        if (bits_[p])
            occ.push_back(p);
...
std::vector<int> STLDeterminant::get_beta_occ() {
    std::vector<int> occ;
    for (int p = 0; p < nmo_; ++p) {
        if (bits_[nmo_ + p])
            occ.push_back(p);
...
std::vector<int> STLDeterminant::get_alfa_vir() {
    std::vector<int> vir;
    for (int p = 0; p < nmo_; ++p) {
        if (not bits_[p])
            vir.push_back(p);
...
std::vector<int> STLDeterminant::get_beta_vir() {
    std::vector<int> vir;
    for (int p = 0; p < nmo_; ++p) {
        if (not bits_[nmo_ + p])
            vir.push_back(p);
...
std::vector<int> STLDeterminant::get_alfa_occ() const {
    std::vector<int> occ;
    for (int p = 0; p < nmo_; ++p) {
        if (bits_[p])
            occ.push_back(p);
...
std::vector<int> STLDeterminant::get_beta_occ() const {
    std::vector<int> occ;
    for (int p = 0; p < nmo_; ++p) {
        if (bits_[nmo_ + p])
            occ.push_back(p);
...
std::vector<int> STLDeterminant::get_alfa_vir() const {
    std::vector<int> vir;
    for (int p = 0; p < nmo_; ++p) {
        if (not bits_[p])
            vir.push_back(p);
...
std::vector<int> STLDeterminant::get_beta_vir() const {
    std::vector<int> vir;
    for (int p = 0; p < nmo_; ++p) {
        if (not bits_[nmo_ + p])
            vir.push_back(p);
...
double STLDeterminant::create_alfa_bit(int n) {
    if (bits_[n])
        return 0.0;
    bits_[n] = true;
...
double STLDeterminant::create_beta_bit(int n) {
    if (bits_[nmo_ + n])
        return 0.0;
    bits_[nmo_ + n] = true;
...
double STLDeterminant::destroy_alfa_bit(int n) {
    if (not bits_[n])
        return 0.0;
    bits_[n] = false;
   // return SlaterSign(bits_, n);
...
double STLDeterminant::destroy_beta_bit(int n) {
    if (not bits_[nmo_ + n])
        return 0.0;
    bits_[nmo_ + n] = false;
    //return SlaterSign(bits_, nmo_ + n);
...
void STLDeterminant::zero_spin(bool spin) {
    for (int p = 0; p < nmo_; ++p) {
        bits_[p + (spin * nmo_)] = false;
    }
}

void STLDeterminant::print() const {
    outfile->Printf("\n  |");
    for (int p = 0; p < nmo_; ++p) {
 ...
 */
std::string STLDeterminant::str() const {
    std::string s;
    s += "|";
...
double STLDeterminant::slater_sign_a(int n) const {
    double sign = 1.0;
...
double STLDeterminant::slater_sign_b(int n) const {
    double sign = 1.0;
    for (int i = 0; i < n; ++i) { // This runs up to the operator before n
        if (bits_[nmo_ + i])
            sign *= -1.0;
...
double STLDeterminant::single_excitation_a(int i, int a) {
    bits_[i] = false;
    bits_[a] = true;
...
double STLDeterminant::double_excitation_ab(int i, int j, int a, int b) {
    bits_[i] = false;
    bits_[nmo_ + j] = false;
    bits_[nmo_ + b] = true;
    bits_[a] = true;
...
double STLDeterminant::double_excitation_bb(int i, int j, int a, int b) {
    bits_[nmo_ + i] = false;
    bits_[nmo_ + j] = false;
    bits_[nmo_ + b] = true;
    bits_[nmo_ + a] = true;
...
std::vector<std::pair<STLDeterminant, double>> STLDeterminant::spin_plus() const {
    std::vector<std::pair<STLDeterminant, double>> res;
    for (int i = 0; i < nmo_; ++i) {
        if ((not ALFA(i)) and BETA(i)) {
            double sign = this->slater_sign_a(i) * this->slater_sign_b(i);
            STLDeterminant new_det(*this);
...
std::vector<std::pair<STLDeterminant, double>> STLDeterminant::spin_minus() const {
    std::vector<std::pair<STLDeterminant, double>> res;
    for (int i = 0; i < nmo_; ++i) {
...
double STLDeterminant::spin_z() const {
    int n = 0;
    for (int i = 0; i < nmo_; ++i) {
        if (ALFA(i))
            n++;
...
double STLDeterminant::spin2_slow(const STLDeterminant& rhs) const {
    double s2 = 0.0;
    if (rhs == *this) {
        double sz = spin_z();
        s2 += sz * (sz + 1.0);
...
double STLDeterminant::spin2(const STLDeterminant& rhs) const {
    const bit_t& I = bits_;
...
/*
double STLDeterminant::SlaterSign(int n) {
    double sign = 1.0;
    for (int i = 0; i < n; ++i) { // This runs up to the operator before n
        if (bits_[i])
            sign *= -1.0;
...
double STLDeterminant::SlaterSign(const bit_t& I, int m, int n) {
    double sign = 1.0;
    for (int i = m + 1; i < n; ++i) {
        if (I[i])
            sign *= -1.0;
...
double STLDeterminant::SlaterSign(const bit_t& bits, int i, int j, int a, int b) {
    if ((((i < a) && (j < a) && (i < b) && (j < b)) == true) ||
        (((i < a) || (j < a) || (i < b) || (j < b)) == false)) {
        if ((i < j) ^ (a < b)) {
...
*/
void STLDeterminant::enforce_spin_completeness(std::vector<STLDeterminant>& det_space) {
    ...
    for (size_t I = 0, det_size = det_space.size(); I < det_size; ++I) {
        const STLDeterminant& det = det_space[I];

...
void AdaptiveCI::startup() {
    ...
    if (psi::Options_.get_bool("ACI_STREAMLINE_Q") == true) {
...
double AdaptiveCI::compute_energy() {
    ...
    if (psi::Options_.get_bool("ACI_ADD_SINGLES")) {
    ...
    if ((psi::Options_.get_int("ACI_MAX_RDM") >= 3 or (rdm_level_ >= 3)) and
        !(psi::Options_.get_bool("ACI_DIRECT_RDMS"))) {
        outfile->Printf("\n  Computing 3-list...    ");
        local_timer l3;
    ...
    if (ex_alg_ == "ROOT_COMBINE") {
        ...
        //    op1.op_lists(approx);
        if (!(psi::Options_.get_bool("ACI_DIRECT_RDMS"))) {
            op_.clear_op_lists();
            op_.clear_tp_lists();
    ...
    // printf( "\n%1.5f\n", aci_elapse.get());
    if (psi::Options_.get_bool("ACI_SPIN_ANALYSIS") and !(psi::Options_.get_bool("ACI_RELAXED_SPIN"))) {
        spin_analysis();
    }

    // Save final wave function to a file
    if (psi::Options_.get_bool("ACI_SAVE_FINAL_WFN")) {
    ...
    return PQ_evals->get(psi::Options_.get_int("ACI_ROOT")) + nuclear_repulsion_energy_ +
...
void AdaptiveCI::diagonalize_final_and_compute_rdms() {
    ...
    if (psi::Options_.get_bool("ACI_DIRECT_RDMS") == false) {
        op_.clear_op_s_lists();
        op_.clear_tp_s_lists();
        op_.op_lists(final_wfn_);
        op_.tp_lists(final_wfn_);
...
void AdaptiveCI::print_final(DeterminantHashVec& dets, psi::SharedMatrix& PQ_evecs,
    ...
    //   if(psi::Options_.get_bool("DETERMINANT_HISTORY")){
...
void AdaptiveCI::default_find_q_space(DeterminantHashVec& P_space, DeterminantHashVec& PQ_space,
    ...
    if (!quiet_mode_) {
        outfile->Printf("\n  %s: %zu determinants", "psi::Dimension of the SD space", V_hash.size());
        outfile->Printf("\n  %s: %f s\n", "Time spent building the external space (default)",
                        build.get());
    }
...
std::vector<std::pair<double, double>> AdaptiveCI::compute_spin(DeterminantHashVec& space,
                                                                WFNOperator& op, psi::SharedMatrix evecs,
                                                                int nroot) {
...
//void AdaptiveCI::full_spin_transform(DeterminantHashVec& det_space, psi::SharedMatrix cI, int nroot) {
    //	local_timer timer;
...
void AdaptiveCI::compute_aci(DeterminantHashVec& PQ_space, psi::SharedMatrix& PQ_evecs,
    ...
    for (cycle = 0; cycle < max_cycle_; ++cycle) {
        ...
        if (psi::Options_.get_bool("ACI_BATCHED_SCREENING")) {
...
void AdaptiveCI::compute_rdms(std::shared_ptr<FCIIntegrals> fci_ints, DeterminantHashVec& dets,
    ...
    if (psi::Options_.get_bool("ACI_DIRECT_RDMS")) {
        ...
        if (rdm_level_ >= 1) {
            ...
            if (psi::Options_.get_bool("ACI_PRINT_NO")) {
                print_nos();
            }
        }
        if (rdm_level_ >= 2) {
    ...
    if (psi::Options_.get_bool("ACI_TEST_RDMS")) {
...
void AdaptiveCI::spin_analysis() {
    ...
    if (psi::Options_.get_str("SPIN_BASIS") == "IAO") {
        ...
        Cainv->invert();
        psi::SharedMatrix iao_coeffs = Matrix::doublet(Cainv, iao_orbs, false, false);
        ...
        if (psi::Options_.get_bool("PI_ACTIVE_SPACE")) {
        ...
        psi::SharedMatrix Ca_new = Matrix::doublet(CA, Ua_full, false, false);
        psi::SharedMatrix Cb_new = Matrix::doublet(CB, Ub_full, false, false);
    ...
    if (psi::Options_.get_bool("SPIN_MAT_TO_FILE")) {
        std::ofstream file;

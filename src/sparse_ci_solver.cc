        ...
        for( size_t adet = 0; adet < nastr; ++adet){
            ...
            for( size_t a = 0, maxa = a_list.size(); a < maxa; ++a){
                ...
                for( size_t j = 0; j < nobeta_; ++j){
                    ...
                    ab_ann[i*noalfa_ + j] =  std::make_tuple(detJ_add,(sign > 0.5 ) ? (ii+1) : (-ii-1),jj);
                    detB.set_beta_bit(jj,true);  
                }

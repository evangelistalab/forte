...
AtomicOrbitalHelper::AtomicOrbitalHelper(SharedMatrix CMO, SharedVector eps_occ, SharedVector eps_vir, double laplace_tolerance, int shift)
: CMO_(CMO), eps_rdocc_(eps_occ), eps_virtual_(eps_vir), laplace_tolerance_(laplace_tolerance), shift_(shift)
{
    LaplaceDenominator laplace(eps_rdocc_, eps_virtual_, laplace_tolerance_);
    Occupied_Laplace_ = laplace.denominator_occ();
    Virtual_Laplace_  = laplace.denominator_vir();
        ...
        for(size_t mu = 0; mu < nbf_; mu++){
            for(size_t nu = 0; nu < nbf_; nu++){
                ...
                for(size_t a = 0; a < nvir_; a++){
                    value_vir += CMO_->get(mu, nrdocc_ + shift_ + a) * CMO_->get(nu, nrdocc_  + shift_ + a) * Virtual_Laplace_->get(w, a);
                }

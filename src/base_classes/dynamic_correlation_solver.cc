...
namespace forte {
DynamicCorrelationSolver::DynamicCorrelationSolver(RDMs rdms, std::shared_ptr<SCFInfo> scf_info,
            ...
            for (int v1 = o_max + 1; v1 < n_gas; ++v1) {
                for (int v2 = o_max + 1; v2 < n_gas; ++v2) {
                    t2_internals_.push_back(
                        {gas_spaces[o1], gas_spaces[o2], gas_spaces[v1], gas_spaces[v2], false});
                }
            }
        }
    }

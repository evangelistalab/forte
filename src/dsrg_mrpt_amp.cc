...
void DSRG_MRPT::compute_T_1st(BlockedTensor& V, BlockedTensor& T2, BlockedTensor& F, BlockedTensor& T1){
...
void DSRG_MRPT::compute_T2_1st(BlockedTensor& V, BlockedTensor& T2){
    ...
    T2.iterate([&](const std::vector<size_t>& i,const std::vector<SpinType>&,double& value){
        if (std::fabs(value) > 1.0e-15){
            value *= dsrg_source_->compute_renormalized_denominator(Fdiag_[i[0]] + Fdiag_[i[1]] - Fdiag_[i[2]] - Fdiag_[i[3]]);
            T2norm_ += value * value;
...
void DSRG_MRPT::compute_T1_1st(BlockedTensor& F, BlockedTensor& T2, BlockedTensor& T1){
    ...
    // contribution from temp and T2 (extra work because we do not have aaaa of T2)
        ...
        T2.block(block).citerate([&](const std::vector<size_t>& i, const double& value){
            if(fabs(value) > 1.0e-15){
                ...
                if((idx0 <= idx1) && (idx2 <= idx3)){
                    ...
                    std::sort(t2_idx_pair.begin(), t2_idx_pair.end(), rsort_pair_second<std::vector<size_t>, double>());
                    ...
                    std::sort(lt2_.begin(), lt2_.end(), rsort_pair_second<std::vector<size_t>, double>());
                }
            }
        });
    }
    ...
    for(const std::string& block: T1.block_labels()){
        T1.block(block).citerate([&](const std::vector<size_t>& i, const double& value){
            if(fabs(value) > 1.0e-15){
                ...
                std::sort(t1_idx_pair.begin(), t1_idx_pair.end(), rsort_pair_second<std::vector<size_t>, double>());
                ...
                std::sort(lt1_.begin(), lt1_.end(), rsort_pair_second<std::vector<size_t>, double>());
            }
        });
    }

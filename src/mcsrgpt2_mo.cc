...
void MCSRGPT2_MO::startup(Options& options) {
    ...
    if (sourcemap.find(source_) == sourcemap.end()) {
        outfile->Printf("\n  Source operator %s is not available.",
                        source_.c_str());
    ...
    if (taylor_threshold_ <= 0) {
        throw PSIEXCEPTION(
            "TAYLOR_THRESHOLD must be an integer greater than 0.");
    }
    ...
    if (options.get_str("CORR_LEVEL") == "SRG_PT2") {
        ...
        if (boost::starts_with(t_algorithm, "DSRG")) {
            outfile->Printf("\n  Form T amplitudes using %s formalism.",
                            t_algorithm.c_str());
...
double MCSRGPT2_MO::ElementRH(const string& source, const double& D,
                              const double& V) {
    if (fabs(V) < 1.0e-12)
        return 0.0;
        ...
        for (size_t i = 0; i < nh_; ++i) {
            ...
            for (size_t a = 0; a < npt_; ++a) {
                ...
                for (size_t u = 0; u < na_; ++u) {
                    ...
                    for (size_t x = 0; x < na_; ++x) {
                        size_t nx = idx_a_[x];
                        value_a += (Fa_[nx][nx] - Fa_[nu][nu]) *
                                   T2aa_[i][u][a][x] * Da_[nx][nu];
                        value_a += (Fb_[nx][nx] - Fb_[nu][nu]) *
                                   T2ab_[i][u][a][x] * Db_[nx][nu];
                        value_b += (Fa_[nx][nx] - Fa_[nu][nu]) *
                                   T2ab_[u][i][x][a] * Da_[nx][nu];
                        value_b += (Fb_[nx][nx] - Fb_[nu][nu]) *
                                   T2bb_[i][u][a][x] * Db_[nx][nu];
                    }
                }
                value_a += Fa_[ni][na];
                value_b += Fb_[ni][na];
            ...
            for (size_t j = 0; j < nh_; ++j) {
                ...
                for (size_t a = 0; a < npt_; ++a) {
                    ...
                    for (size_t b = 0; b < npt_; ++b) {
                        size_t nb = idx_p_[b];
                        double Daa = Fa_[ni][ni] + Fa_[nj][nj] - Fa_[na][na] -
                                     Fa_[nb][nb];
                        double Dab = Fa_[ni][ni] + Fb_[nj][nj] - Fa_[na][na] -
                                     Fb_[nb][nb];
                        double Dbb = Fb_[ni][ni] + Fb_[nj][nj] - Fb_[na][na] -
                                     Fb_[nb][nb];
        ...
        for (size_t q = 0; q < na_; ++q) {
            ...
            for (size_t r = 0; r < na_; ++r) {
                ...
                for (size_t s = 0; s < na_; ++s) {
                    size_t ns = idx_a_[s];
                    Eref_ += 0.25 * integral_->aptei_aa(np, nq, nr, ns) *
                             L2aa_[p][q][r][s];
                    Eref_ += 0.25 * integral_->aptei_bb(np, nq, nr, ns) *
                             L2bb_[p][q][r][s];
                    Eref_ +=
                        integral_->aptei_ab(np, nq, nr, ns) * L2ab_[p][q][r][s];
                }
            }
        }
    }
...
double MCSRGPT2_MO::ElementT(const string& source, const double& D,
                             const double& V) {
    if (fabs(V) < 1.0e-12)
        return 0.0;
    switch (sourcemap[source]) {
    ...
    case EMP2: {
        ...
        if (fabs(Z) < pow(0.1, taylor_threshold_)) {
            ...
            return (1 - exp(-1.0 * s_ * pow(fabs(RD / V), expo_delta_))) * V /
                   D;
        }
    }
    case LAMP: {
    ...
    default: {
        ...
        if (fabs(Z) < pow(0.1, taylor_threshold_)) {
            return Taylor_Exp(Z, taylor_order_, expo_delta_) *
                   pow(s_, 1 / expo_delta_) * V;
        } else {
        ...
        for (size_t j = 0; j < nh_; ++j) {
            ...
            for (size_t a = 0; a < npt_; ++a) {
                ...
                for (size_t b = 0; b < npt_; ++b) {
                    ...
                    double Dab =
                        Fa_[ni][ni] + Fb_[nj][nj] - Fa_[na][na] - Fb_[nb][nb];
                    double Dbb =
                        Fb_[ni][ni] + Fb_[nj][nj] - Fb_[na][na] - Fb_[nb][nb];
        ...
        for (size_t a = 0; a < npt_; ++a) {
            ...
            for (size_t u = 0; u < na_; ++u) {
                ...
                for (size_t x = 0; x < na_; ++x) {
                    ...
                    scalar_a += (Fa_[nx][nx] - Fa_[nu][nu]) *
                                T2aa_[i][u][a][x] * Da_[nx][nu];
                    scalar_a += (Fb_[nx][nx] - Fb_[nu][nu]) *
                                T2ab_[i][u][a][x] * Db_[nx][nu];
                    scalar_b += (Fa_[nx][nx] - Fa_[nu][nu]) *
                                T2ab_[u][i][x][a] * Da_[nx][nu];
                    scalar_b += (Fb_[nx][nx] - Fb_[nu][nu]) *
                                T2bb_[i][u][a][x] * Db_[nx][nu];
                }
            }
...
inline bool
ReverseSortT2(const tuple<double, size_t, size_t, size_t, size_t>& lhs,
              const tuple<double, size_t, size_t, size_t, size_t>& rhs) {
    return fabs(get<0>(rhs)) < fabs(get<0>(lhs));
}

void MCSRGPT2_MO::Check_T2(const string& x, const d4& M, double& Norm,
                           double& MaxT, Options& options) {
    timer_on("Check T2");
    ...
    vector<tuple<double, size_t, size_t, size_t, size_t>> Large(
        ntamp, make_tuple(0.0, 0, 0, 0, 0));
    double value = 0.0;
    size_t count = 0;
    for (size_t i = 0; i < nh_; ++i) {
        size_t ni = idx_h_[i];
    ...
    // Print
    outfile->Printf("\n");
    outfile->Printf("\n  ==> Largest T2 amplitudes for spin case %s: <==",
                    x.c_str());
    if (x == "AA")
        outfile->Printf("\n");
    if (x == "AB")
    ...
    if (x == "BB")
        outfile->Printf("\n   %3c %3c %3c %3c %9c  %3c %3c %3c %3c %9c  %3c "
                        ...
                        '_', '_', '_', ' ');
    outfile->Printf(
        "\n   %3c %3c %3c %3c %9c  %3c %3c %3c %3c %9c  %3c %3c %3c %3c %9c",
    ...
    for (size_t n = 0; n != ntamp; ++n) {
        if (n % 3 == 0)
            outfile->Printf("\n  ");
        outfile->Printf("[%3zu %3zu %3zu %3zu] %8.5f ", get<1>(Large[n]),
                        get<2>(Large[n]), get<3>(Large[n]), get<4>(Large[n]),
                        get<0>(Large[n]));
    }
    outfile->Printf("\n  "
                    "----------------------------------------------------------"
                    "----------------------");
    outfile->Printf(
        "\n  Norm of T2%s vector: (nonzero elements: %12zu) %25.15lf.",
        x.c_str(), count, Norm);
    outfile->Printf("\n  "
                    "----------------------------------------------------------"
                    "----------------------");
    outfile->Printf("\n");
    outfile->Printf("\n  ==> T2 intruder states analysis for spin case %s: <==",
                    x.c_str());
    outfile->Printf("\n  "
                    "----------------------------------------------------------"
                    "-------------------------------");
...
void MCSRGPT2_MO::Check_T1(const string& x, const d2& M, double& Norm,
                           double& MaxT, Options& options) {
    timer_on("Check T1");
    int ntamp = options.get_int("NTAMP");
    double intruder = options.get_double("INTRUDER_TAMP");
    ...
    // Print
    outfile->Printf("\n");
    outfile->Printf("\n  ==> Largest T1 amplitudes for spin case %s: <==",
                    x.c_str());
    if (x == "A")
        outfile->Printf("\n");
    if (x == "B")
    ...
    for (size_t n = 0; n != ntamp; ++n) {
        if (n % 3 == 0)
            outfile->Printf("\n  ");
        outfile->Printf("[%3zu %3c %3zu %3c] %8.5f ", get<1>(Large[n]), ' ',
                        get<2>(Large[n]), ' ', get<0>(Large[n]));
    }
    outfile->Printf("\n  "
                    "----------------------------------------------------------"
                    "----------------------");
    outfile->Printf(
        "\n  Norm of T1%s vector: (nonzero elements: %12zu) %26.15lf.",
        x.c_str(), count, Norm);
    outfile->Printf("\n  "
                    "----------------------------------------------------------"
                    "----------------------");
    outfile->Printf("\n");
    outfile->Printf("\n  ==> T1 intruder states analysis for spin case %s: <==",
                    x.c_str());
    outfile->Printf("\n  "
                    "----------------------------------------------------------"
                    "-----------");
    ...
    for (size_t n = 0; n != Max.size(); ++n) {
        ...
        double up = t2 * down;
        outfile->Printf(
            "\n  [%3zu %3c %3zu %3c] = %7.4f = %7.4f / (%7.4f - %7.4f = %7.4f)",
            i, ' ', a, ' ', t2, up, fi, fa, down);
    }
    outfile->Printf("\n  "
                    "----------------------------------------------------------"
                    "-----------");
        ...
        for (size_t a = 0; a < npt_; ++a) {
            ...
            if (std::find(idx_a_.begin(), idx_a_.end(), ni) != idx_a_.end() &&
                ...
                if (std::fabs(Da) < small_threshold) {
                    smallD1.push_back(
                        std::make_pair(std::vector<size_t>{ni, na}, Da));
                }
            }
        }
    }
    ...
    if (smallD1.size() == 0) {
        ...
        std::string title = indent +
                            str(boost::format("%=9s    %=15s    %=15s\n") %
                                "Indices" % "Denominator" % "Original Denom.") +
                            indent + dash;
        outfile->Printf("\n%s", title.c_str());
        ...
        for (size_t j = i; j < nh_; ++j) {
            ...
            for (size_t a = 0; a < npt_; ++a) {
                ...
                for (size_t b = a; b < npt_; ++b) {
                    ...
                    if ((sym_ncmo_[ni] ^ sym_ncmo_[nj] ^ sym_ncmo_[na] ^
                         sym_ncmo_[nb]) != 0)
                        continue;

                    // cannot be all active
                    if (std::find(idx_a_.begin(), idx_a_.end(), ni) !=
                            idx_a_.end() &&
                        std::find(idx_a_.begin(), idx_a_.end(), nj) !=
                            idx_a_.end() &&
                        std::find(idx_a_.begin(), idx_a_.end(), na) !=
                            idx_a_.end() &&
                        std::find(idx_a_.begin(), idx_a_.end(), nb) !=
                            idx_a_.end()) {
                        continue;
                    } else {
                        double Daa = Fa_[ni][ni] + Fa_[nj][nj] - Fa_[na][na] -
                        ...
                        for (size_t c = 0; c < npt_; ++c) {
                            ...
                            for (size_t d = 0; d < npt_; ++d) {
                                size_t nd = idx_p_[d];
                                Daa -=
                                    0.5 * integral_->aptei_aa(nc, nd, na, nb) *
                                    (1.0 - Da_[na][nc]) * (1.0 - Da_[nb][nd]);
                                Dab -= integral_->aptei_ab(nc, nd, na, nb) *
                                       (1.0 - Da_[na][nc]) *
                                       (1.0 - Db_[nb][nd]);
                            }
                        }
                        ...
                        for (size_t v = 0; v < na_; ++v) {
                            ...
                            for (size_t y = 0; y < na_; ++y) {
                                size_t ny = idx_a_[y];
                                Daa += 0.5 *
                                       integral_->aptei_aa(nv, ny, na, nb) *
                                       Da_[na][nv] * Da_[nb][ny];
                                Dab += integral_->aptei_ab(nv, ny, na, nb) *
                                       Da_[na][nv] * Db_[nb][ny];
                            }
                        }
                        ...
                        for (size_t k = 0; k < nh_; ++k) {
                            ...
                            for (size_t l = 0; l < nh_; ++l) {
                                size_t nl = idx_h_[l];
                                Daa -= 0.5 *
                                       integral_->aptei_aa(ni, nj, nk, nl) *
                                       Da_[nk][ni] * Da_[nl][nj];
                                Dab -= integral_->aptei_ab(ni, nj, nk, nl) *
                                       Da_[nk][ni] * Db_[nl][nj];
                            }
                        }
                        ...
                        for (size_t u = 0; u < na_; ++u) {
                            ...
                            for (size_t x = 0; x < na_; ++x) {
                                size_t nx = idx_a_[x];
                                Daa +=
                                    0.5 * integral_->aptei_aa(ni, nj, nu, nx) *
                                    (1.0 - Da_[nu][ni]) * (1.0 - Da_[nx][nj]);
                                Dab += integral_->aptei_ab(ni, nj, nu, nx) *
                                       (1.0 - Da_[nu][ni]) *
                                       (1.0 - Db_[nx][nj]);
                            }
                        }
                        ...
                        for (size_t k = 0; k < nh_; ++k) {
                            ...
                            Daa -= integral_->aptei_aa(na, ni, nk, na) *
                                   Da_[nk][ni];
                            Daa -= integral_->aptei_aa(nb, ni, nk, nb) *
                                   Da_[nk][ni];
                            Daa -= integral_->aptei_aa(na, nj, nk, na) *
                                   Da_[nk][nj];
                            Daa -= integral_->aptei_aa(nb, nj, nk, nb) *
                                   Da_[nk][nj];

                            Dab -= integral_->aptei_aa(na, ni, nk, na) *
                                   Da_[nk][ni];
                            Dab += integral_->aptei_ab(ni, nb, nk, nb) *
                                   Da_[nk][ni];
                            Dab += integral_->aptei_ab(na, nj, na, nk) *
                                   Db_[nk][nj];
                            Dab -= integral_->aptei_bb(nb, nj, nk, nb) *
                                   Da_[nk][nj];
                        }
                        ...
                        for (size_t y = 0; y < na_; ++y) {
                            ...
                            Daa += integral_->aptei_aa(ny, ni, ni, na) *
                                   Da_[na][ny];
                            Daa += integral_->aptei_aa(ny, ni, ni, nb) *
                                   Da_[nb][ny];
                            Daa += integral_->aptei_aa(ny, nj, nj, na) *
                                   Da_[na][ny];
                            Daa += integral_->aptei_aa(ny, nj, nj, nb) *
                                   Da_[nb][ny];

                            Dab -= integral_->aptei_aa(ny, ni, ni, na) *
                                   Da_[na][ny];
                            Dab += integral_->aptei_ab(ni, ny, ni, nb) *
                                   Db_[nb][ny];
                            Dab += integral_->aptei_ab(ny, nj, na, nj) *
                                   Da_[na][ny];
                            Dab -= integral_->aptei_bb(ny, nj, nj, nb) *
                                   Db_[nb][ny];
                        }

                        if (std::fabs(Daa) < small_threshold && ni != nj &&
                            na != nb) {
                            smallD2aa.push_back(std::make_pair(
                                std::vector<size_t>{ni, nj, na, nb}, Daa));
                        }
                        if (std::fabs(Dab) < small_threshold) {
                            smallD2ab.push_back(std::make_pair(
                                std::vector<size_t>{ni, nj, na, nb}, Dab));
                        }
                    }
                }
            }
    ...
    for (size_t m = 0; m < nc_; ++m) {
        ...
        for (size_t n = 0; n < nc_; ++n) {
            ...
            for (size_t u = 0; u < na_; ++u) {
                ...
                for (size_t v = 0; v < na_; ++v) {
                    size_t nv = idx_a_[v];
                    if ((sym_ncmo_[nm] ^ sym_ncmo_[nn] ^ sym_ncmo_[nu] ^
                         sym_ncmo_[nv]) != 0)
                        continue;

                    double Daa =
                        Fa_[nm][nm] + Fa_[nn][nn] - Fa_[nu][nu] - Fa_[nv][nv];
                    double Dab =
                        Fa_[nm][nm] + Fb_[nn][nn] - Fa_[nu][nu] - Fb_[nv][nv];
                    ...
                    for (size_t x = 0; x < na_; ++x) {
                        size_t nx = idx_a_[x];
                        Daa +=
                            Da_[nu][nx] * integral_->aptei_aa(nx, nv, nu, nv);
                        Dab +=
                            Da_[nu][nx] * integral_->aptei_ab(nx, nv, nu, nv);
                    }

                    if (std::fabs(Daa) < small_threshold) {
                        smallD2aa.push_back(std::make_pair(
                            std::vector<size_t>{nm, nn, nu, nv}, Daa));
                    }
                    if (std::fabs(Dab) < small_threshold) {
                        smallD2ab.push_back(std::make_pair(
                            std::vector<size_t>{nm, nn, nu, nv}, Dab));
                    }
                }
            }
        }
    ...
    for (size_t x = 0; x < na_; ++x) {
        ...
        for (size_t y = 0; y < na_; ++y) {
            ...
            for (size_t e = 0; e < nv_; ++e) {
                ...
                for (size_t f = 0; f < nv_; ++f) {
                    size_t nf = idx_v_[f];
                    if ((sym_ncmo_[nx] ^ sym_ncmo_[ny] ^ sym_ncmo_[ne] ^
                         sym_ncmo_[nf]) != 0)
                        continue;

                    double Daa =
                        Fa_[nx][nx] + Fa_[ny][ny] - Fa_[ne][ne] - Fa_[nf][nf];
                    double Dab =
                        Fa_[nx][nx] + Fb_[ny][ny] - Fa_[ne][ne] - Fb_[nf][nf];
                    ...
                    for (size_t u = 0; u < na_; ++u) {
                        size_t nu = idx_a_[u];
                        Daa -=
                            Da_[nu][nx] * integral_->aptei_aa(nx, ny, nu, ny);
                        Dab -=
                            Da_[nu][nx] * integral_->aptei_ab(nx, ny, nu, ny);
                    }

                    if (std::fabs(Daa) < small_threshold) {
                        smallD2aa.push_back(std::make_pair(
                            std::vector<size_t>{nx, ny, ne, nf}, Daa));
                    }
                    if (std::fabs(Dab) < small_threshold) {
                        smallD2ab.push_back(std::make_pair(
                            std::vector<size_t>{nx, ny, ne, nf}, Dab));
                    }
                }
            }
        }
    ...
    for (size_t m = 0; m < nc_; ++m) {
        ...
        for (size_t y = 0; y < na_; ++y) {
            ...
            for (size_t e = 0; e < nv_; ++e) {
                ...
                for (size_t v = 0; v < na_; ++v) {
                    size_t nv = idx_a_[v];
                    if ((sym_ncmo_[nm] ^ sym_ncmo_[ny] ^ sym_ncmo_[ne] ^
                         sym_ncmo_[nv]) != 0)
                        continue;

                    double Daa =
                        Fa_[nm][nm] + Fa_[ny][ny] - Fa_[ne][ne] - Fa_[nv][nv];
                    double D1 =
                        Fa_[nm][nm] + Fb_[ny][ny] - Fa_[ne][ne] - Fb_[nv][nv];
                    double D2 =
                        Fa_[nm][nm] + Fb_[ny][ny] - Fb_[ne][ne] - Fa_[nv][nv];
                    double D3 =
                        Fb_[nm][nm] + Fa_[ny][ny] - Fa_[ne][ne] - Fb_[nv][nv];

                    for (size_t u = 0; u < na_; ++u) {
                        size_t nu = idx_a_[u];
                        Daa +=
                            Da_[nu][ny] * integral_->aptei_aa(ny, nv, nu, nv);
                        Daa -=
                            Da_[nv][nu] * integral_->aptei_aa(nu, ny, nv, ny);
                    ...
                    if (std::fabs(Daa) < small_threshold) {
                        smallD2aa.push_back(std::make_pair(
                            std::vector<size_t>{nm, ny, ne, nv}, Daa));
                    }
                    if (std::fabs(D1) < small_threshold) {
                        smallD2ab.push_back(std::make_pair(
                            std::vector<size_t>{nm, ny, ne, nv}, D1));
                    }
                    if (std::fabs(D2) < small_threshold) {
                        smallD2ab.push_back(std::make_pair(
                            std::vector<size_t>{nm, ny, nv, ne}, D2));
                    }
                    if (std::fabs(D3) < small_threshold) {
                        smallD2ab.push_back(std::make_pair(
                            std::vector<size_t>{ny, nm, ne, nv}, D3));
                    }
                }
            }
        }
    ...
    for (size_t x = 0; x < na_; ++x) {
        ...
        for (size_t y = 0; y < na_; ++y) {
            ...
            for (size_t z = 0; z < na_; ++z) {
                ...
                for (size_t e = 0; e < nv_; ++e) {
                    size_t ne = idx_v_[e];
                    if ((sym_ncmo_[nx] ^ sym_ncmo_[ny] ^ sym_ncmo_[nz] ^
                         sym_ncmo_[ne]) != 0)
                        continue;

                    double Daa =
                        Fa_[nx][nx] + Fa_[ny][ny] - Fa_[nz][nz] - Fa_[ne][ne];
                    double Dab =
                        Fa_[nx][nx] + Fb_[ny][ny] - Fa_[nz][nz] - Fb_[ne][ne];
                    ...
                    for (size_t u = 0; u < na_; ++u) {
                        ...
                        Daa -=
                            Da_[nu][nx] * integral_->aptei_aa(nx, ny, nu, ny);
                        Daa +=
                            Da_[nu][nx] * integral_->aptei_aa(nx, nz, nu, nz);
                        Daa -=
                            Da_[nz][nu] * integral_->aptei_aa(nx, nu, nx, nz);
                        Daa +=
                            Da_[nu][ny] * integral_->aptei_aa(ny, nz, nu, nz);
                        Daa -=
                            Da_[nz][nu] * integral_->aptei_aa(ny, nu, ny, nz);
                        ...
                        Dab -=
                            Da_[nz][nu] * integral_->aptei_aa(nx, nu, nx, nz);
                        Dab +=
                            Db_[nu][ny] * integral_->aptei_ab(nz, ny, nz, nu);
                        Dab -=
                            Da_[nz][nu] * integral_->aptei_ab(nu, ny, nz, ny);
                    }

                    if (std::fabs(Daa) < small_threshold) {
                        smallD2aa.push_back(std::make_pair(
                            std::vector<size_t>{nx, ny, nz, ne}, Daa));
                    }
                    if (std::fabs(Dab) < small_threshold) {
                        smallD2ab.push_back(std::make_pair(
                            std::vector<size_t>{nx, ny, nz, ne}, Dab));
                    }
                }
            }
        }
    ...
    for (size_t m = 0; m < nc_; ++m) {
        ...
        for (size_t w = 0; w < na_; ++w) {
            ...
            for (size_t u = 0; u < na_; ++u) {
                ...
                for (size_t v = 0; v < na_; ++v) {
                    size_t nv = idx_a_[v];
                    if ((sym_ncmo_[nm] ^ sym_ncmo_[nw] ^ sym_ncmo_[nu] ^
                         sym_ncmo_[nv]) != 0)
                        continue;

                    double Daa =
                        Fa_[nm][nm] + Fa_[nw][nw] - Fa_[nu][nu] - Fa_[nv][nv];
                    double Dab =
                        Fa_[nm][nm] + Fb_[nw][nw] - Fa_[nu][nu] - Fb_[nv][nv];
                    ...
                    for (size_t x = 0; x < na_; ++x) {
                        ...
                        Daa +=
                            Da_[nu][nx] * integral_->aptei_aa(nx, nv, nu, nv);
                        Daa +=
                            Da_[nx][nw] * integral_->aptei_aa(nu, nw, nu, nx);
                        Daa -=
                            Da_[nu][nx] * integral_->aptei_aa(nx, nw, nu, nw);
                        Daa +=
                            Da_[nx][nw] * integral_->aptei_aa(nw, nv, nx, nv);
                        Daa -=
                            Da_[nv][nx] * integral_->aptei_aa(nx, nw, nv, nw);

                        Dab +=
                            Da_[nu][nx] * integral_->aptei_ab(nx, nv, nu, nv);
                        Dab +=
                            Db_[nx][nw] * integral_->aptei_ab(nu, nw, nu, nx);
                        Dab -=
                            Da_[nu][nx] * integral_->aptei_ab(nx, nw, nu, nw);
                        Dab +=
                            Db_[nx][nw] * integral_->aptei_bb(nw, nv, nx, nv);
                        Dab -=
                            Db_[nv][nx] * integral_->aptei_bb(nx, nw, nv, nw);
                    }

                    if (std::fabs(Daa) < small_threshold) {
                        smallD2aa.push_back(std::make_pair(
                            std::vector<size_t>{nm, nw, nu, nv}, Daa));
                    }
                    if (std::fabs(Dab) < small_threshold) {
                        smallD2ab.push_back(std::make_pair(
                            std::vector<size_t>{nm, nw, nu, nv}, Dab));
                    }
                }
            }
        }
            ...
            for (size_t a = 0; a < npt_; ++a) {
                ...
                for (size_t b = a; b < npt_; ++b) {
                    ...
                    if (std::find(idx_a_.begin(), idx_a_.end(), ni) !=
                        ...
                        out_delta
                            << boost::format("%3d %3d %3d %3d %20.15f\n") % ni %
                                   nj % na % nb % Daa;
                        //                        out_delta <<
...
double MCSRGPT2_MO::compute_energy_dsrg() {
    ...
    outfile->Printf("\n    E([V, T2] C_2^2 * C_4: PP) %2c = %22.15lf", ' ',
                    E8_1);
    outfile->Printf("\n    E([V, T2] C_2^2 * C_4: HH) %2c = %22.15lf", ' ',
                    E8_2);
    outfile->Printf("\n    E([V, T2] C_2^2 * C_4: PH) %2c = %22.15lf", ' ',
                    E8_3);
    ...
    for (size_t i = 0; i < nh_; ++i) {
        ...
        for (size_t j = 0; j < nh_; ++j) {
            ...
            for (size_t a = 0; a < npt_; ++a) {
                ...
                for (size_t b = 0; b < npt_; ++b) {
                    size_t nb = idx_p_[b];
                    E += Fa_dsrg_[nb][nj] * T1a_[i][a] * Da_[nj][ni] *
                         (Delta(na, nb) - Da_[na][nb]);
                    E += Fb_dsrg_[nb][nj] * T1b_[i][a] * Db_[nj][ni] *
                         (Delta(na, nb) - Db_[na][nb]);
                }
            }
        }
    }
...
void MCSRGPT2_MO::E_VT1_FT2(double& EF1, double& EF2, double& EV1,
                            double& EV2) {
    timer_on("[F, T2] & [V, T1]");
    EF1 = 0.0;
    EF2 = 0.0;
            ...
            for (size_t x = 0; x < na_; ++x) {
                ...
                for (size_t y = 0; y < na_; ++y) {
                    ...
                    for (size_t e = 0; e < nv_; ++e) {
                        size_t ne = idx_v_[e];
                        size_t te = e + na_;
                        EV1 += integral_->aptei_aa(nx, ny, ne, nv) *
                               T1a_[u][te] * L2aa_[x][y][u][v];
                        EV1 += integral_->aptei_bb(nx, ny, ne, nv) *
                               T1b_[u][te] * L2bb_[x][y][u][v];
                        EV1 += 2 * integral_->aptei_ab(nx, ny, ne, nv) *
                               T1a_[u][te] * L2ab_[x][y][u][v];
                        EV1 += 2 * integral_->aptei_ab(ny, nx, nv, ne) *
                               T1b_[u][te] * L2ab_[y][x][v][u];

                        EF1 += Fa_dsrg_[ne][nx] * T2aa_[u][v][te][y] *
                               L2aa_[x][y][u][v];
                        EF1 += Fb_dsrg_[ne][nx] * T2bb_[u][v][te][y] *
                               L2bb_[x][y][u][v];
                        EF1 += 2 * Fa_dsrg_[ne][nx] * T2ab_[u][v][te][y] *
                               L2ab_[x][y][u][v];
                        EF1 += 2 * Fb_dsrg_[ne][nx] * T2ab_[v][u][y][te] *
                               L2ab_[y][x][v][u];
                    }
                    ...
                    for (size_t m = 0; m < nc_; ++m) {
                        size_t nm = idx_c_[m];
                        size_t tm = m + na_;
                        EV2 -= integral_->aptei_aa(nm, ny, nu, nv) *
                               T1a_[tm][x] * L2aa_[x][y][u][v];
                        EV2 -= integral_->aptei_bb(nm, ny, nu, nv) *
                               T1b_[tm][x] * L2bb_[x][y][u][v];
                        EV2 -= 2 * integral_->aptei_ab(nm, ny, nu, nv) *
                               T1a_[tm][x] * L2ab_[x][y][u][v];
                        EV2 -= 2 * integral_->aptei_ab(ny, nm, nv, nu) *
                               T1b_[tm][x] * L2ab_[y][x][v][u];

                        EF2 -= Fa_dsrg_[nv][nm] * T2aa_[u][tm][x][y] *
                               L2aa_[x][y][u][v];
                        EF2 -= Fb_dsrg_[nv][nm] * T2bb_[u][tm][x][y] *
                               L2bb_[x][y][u][v];
                        EF2 -= 2 * Fa_dsrg_[nv][nm] * T2ab_[tm][u][y][x] *
                               L2ab_[y][x][v][u];
                        EF2 -= 2 * Fb_dsrg_[nv][nm] * T2ab_[u][tm][x][y] *
                               L2ab_[x][y][u][v];
                    }
                }
            }
            ...
            for (size_t a = 0; a < npt_; ++a) {
                ...
                for (size_t d = 0; d < npt_; ++d) {
                    ...
                    for (size_t c = 0; c < npt_; ++c) {
                        size_t nc = idx_p_[c];
                        C1aa[a][d][k][l] +=
                            integral_->aptei_aa(nk, nl, nc, nd) *
                            (Delta(na, nc) - Da_[na][nc]);
                        C1ab[a][d][k][l] +=
                            integral_->aptei_ab(nk, nl, nc, nd) *
                            (Delta(na, nc) - Da_[na][nc]);
                        C1bb[a][d][k][l] +=
                            integral_->aptei_bb(nk, nl, nc, nd) *
            ...
            for (size_t a = 0; a < npt_; ++a) {
                for (size_t b = 0; b < npt_; ++b) {
                    ...
                    for (size_t d = 0; d < npt_; ++d) {
                        size_t nd = idx_p_[d];
                        C2aa[a][b][k][l] +=
                            C1aa[a][d][k][l] * (Delta(nb, nd) - Da_[nb][nd]);
                        C2ab[a][b][k][l] +=
                            C1ab[a][d][k][l] * (Delta(nb, nd) - Db_[nb][nd]);
                        C2bb[a][b][k][l] +=
                            C1bb[a][d][k][l] * (Delta(nb, nd) - Db_[nb][nd]);
                    }
                }
            }
        }
            ...
            for (size_t d = 0; d < npt_; ++d) {
                ...
                for (size_t a = 0; a < npt_; ++a) {
                    ...
                    for (size_t c = 0; c < npt_; ++c) {
                        size_t nc = idx_p_[c];
                        C1aa[a][d][x][y] +=
                            integral_->aptei_aa(nx, ny, nc, nd) *
                            (Delta(na, nc) - Da_[na][nc]);
                        C1ab[a][d][x][y] +=
                            integral_->aptei_ab(nx, ny, nc, nd) *
                            (Delta(na, nc) - Da_[na][nc]);
                        C1bb[a][d][x][y] +=
                            integral_->aptei_bb(nx, ny, nc, nd) *
                            (Delta(na, nc) - Db_[na][nc]);
                    }
                }
            }
            ...
            for (size_t a = 0; a < npt_; ++a) {
                for (size_t b = 0; b < npt_; ++b) {
                    ...
                    for (size_t d = 0; d < npt_; ++d) {
                        size_t nd = idx_p_[d];
                        C2aa[a][b][x][y] +=
                            C1aa[a][d][x][y] * (Delta(nb, nd) - Da_[nb][nd]);
                        C2ab[a][b][x][y] +=
                            C1ab[a][d][x][y] * (Delta(nb, nd) - Db_[nb][nd]);
                        C2bb[a][b][x][y] +=
                            C1bb[a][d][x][y] * (Delta(nb, nd) - Db_[nb][nd]);
                    }
                }
            }
        }
            ...
            for (size_t a = 0; a < npt_; ++a) {
                for (size_t b = 0; b < npt_; ++b) {
                    for (size_t x = 0; x < na_; ++x) {
                        for (size_t y = 0; y < na_; ++y) {
                            C1aa[a][b][u][v] +=
                                C2aa[a][b][x][y] * L2aa_[x][y][u][v];
                            C1bb[a][b][u][v] +=
                                C2bb[a][b][x][y] * L2bb_[x][y][u][v];
                            C1ab[a][b][u][v] +=
                                C2ab[a][b][x][y] * L2ab_[x][y][u][v];
                        }
                    }
                }
            }
            ...
            for (size_t l = 0; l < nh_; ++l) {
                ...
                for (size_t k = 0; k < nh_; ++k) {
                    ...
                    for (size_t i = 0; i < nh_; ++i) {
                        size_t ni = idx_h_[i];
                        C1aa[u][v][i][l] +=
                            integral_->aptei_aa(nk, nl, nu, nv) * Da_[nk][ni];
                        C1ab[u][v][i][l] +=
                            integral_->aptei_ab(nk, nl, nu, nv) * Da_[nk][ni];
                        C1bb[u][v][i][l] +=
                            integral_->aptei_bb(nk, nl, nu, nv) * Db_[nk][ni];
                    }
                }
            }
        }
            ...
            for (size_t i = 0; i < nh_; ++i) {
                for (size_t j = 0; j < nh_; ++j) {
                    for (size_t u = 0; u < na_; ++u) {
                        for (size_t v = 0; v < na_; ++v) {
                            C1aa[x][y][i][j] +=
                                C2aa[u][v][i][j] * L2aa_[x][y][u][v];
                            C1ab[x][y][i][j] +=
                                C2ab[u][v][i][j] * L2ab_[x][y][u][v];
                            C1bb[x][y][i][j] +=
                                C2bb[u][v][i][j] * L2bb_[x][y][u][v];
                        }
                    }
                }
            }
        ...
        for (size_t v = 0; v < na_; ++v) {
            ...
            for (size_t j = 0; j < nh_; ++j) {
                ...
                for (size_t a = 0; a < npt_; ++a) {
                    ...
                    for (size_t b = 0; b < npt_; ++b) {
                        size_t nb = idx_p_[b];
                        C11[v][a][j][x] += integral_->aptei_aa(nj, nx, nv, nb) *
                                           (Delta(na, nb) - Da_[na][nb]);
                        C12[v][a][j][x] -= integral_->aptei_ab(nx, nj, nv, nb) *
                                           (Delta(na, nb) - Db_[na][nb]);
                        C13[v][a][j][x] += integral_->aptei_bb(nj, nx, nv, nb) *
                                           (Delta(na, nb) - Db_[na][nb]);
                        C14[v][a][j][x] -= integral_->aptei_ab(nj, nx, nb, nv) *
                                           (Delta(na, nb) - Da_[na][nb]);
                        C19[v][a][j][x] += integral_->aptei_ab(nx, nj, nb, nv) *
                                           (Delta(na, nb) - Da_[na][nb]);
                        C110[v][a][j][x] +=
                            integral_->aptei_ab(nj, nx, nv, nb) *
                            (Delta(na, nb) - Db_[na][nb]);
                    }
                }
            }
            ...
            for (size_t x = 0; x < na_; ++x) {
                for (size_t v = 0; v < na_; ++v) {
                    for (size_t i = 0; i < nh_; ++i) {
                        for (size_t a = 0; a < npt_; ++a) {
                            C31[u][v][x][y] +=
                                C21[v][a][i][x] * T2aa_[i][u][a][y];
                            C32[u][v][x][y] +=
                                C22[v][a][i][x] * T2ab_[u][i][y][a];
                            C33[u][v][x][y] +=
                                C23[v][a][i][x] * T2bb_[i][u][a][y];
                            C34[u][v][x][y] +=
                                C24[v][a][i][x] * T2ab_[i][u][a][y];
                            C35[u][v][x][y] +=
                                C21[v][a][i][x] * T2ab_[i][u][a][y];
                            C36[u][v][x][y] +=
                                C22[v][a][i][x] * T2bb_[i][u][a][y];
                            C37[u][v][x][y] +=
                                C24[v][a][i][x] * T2aa_[i][u][a][y];
                            C38[u][v][x][y] +=
                                C23[v][a][i][x] * T2ab_[u][i][y][a];
                            C39[u][v][x][y] -=
                                C29[v][a][i][x] * T2ab_[u][i][a][y];
                            C310[u][v][x][y] -=
                                C210[v][a][i][x] * T2ab_[i][u][y][a];
                        }
                    }
                }
            }
                    ...
                    for (size_t y = 0; y < na_; ++y) {
                        ...
                        for (size_t z = 0; z < na_; ++z) {
                            ...
                            for (size_t i = 0; i < nh_; ++i) {
                                size_t ni = idx_h_[i];
                                // L3aaa & L3bbb
                                E1 += integral_->aptei_aa(ni, nz, nu, nv) *
                                      T2aa_[i][w][x][y] *
                                      L3aaa_[x][y][z][u][v][w];
                                E1 += integral_->aptei_bb(ni, nz, nu, nv) *
                                      T2bb_[i][w][x][y] *
                                      L3bbb_[x][y][z][u][v][w];
                                // L3aab
                                E1 -= 2 * L3aab_[x][z][y][u][v][w] *
                                      T2ab_[i][w][x][y] *
                                      integral_->aptei_aa(ni, nz, nu, nv);
                                // L3aba & L3baa
                                E1 -= 2 * L3aab_[x][y][z][u][w][v] *
                                      T2aa_[i][w][x][y] *
                                      integral_->aptei_ab(ni, nz, nu, nv);
                                E1 += 4 * L3aab_[x][z][y][u][w][v] *
                                      T2ab_[w][i][x][y] *
                                      integral_->aptei_ab(nz, ni, nu, nv);
                                // L3abb & L3bab
                                E1 += 4 * L3abb_[x][y][z][u][v][w] *
                                      T2ab_[i][w][x][y] *
                                      integral_->aptei_ab(ni, nz, nu, nv);
                                E1 -= 2 * L3abb_[z][x][y][u][v][w] *
                                      T2bb_[i][w][x][y] *
                                      integral_->aptei_ab(nz, ni, nu, nv);
                                // L3bba
                                E1 -= 2 * L3abb_[x][y][z][w][u][v] *
                                      T2ab_[w][i][x][y] *
                            ...
                            for (size_t a = 0; a < npt_; ++a) {
                                size_t na = idx_p_[a];
                                // L3aaa & L3bbb
                                E2 += integral_->aptei_aa(nx, ny, nw, na) *
                                      T2aa_[u][v][a][z] *
                                      L3aaa_[x][y][z][u][v][w];
                                E2 += integral_->aptei_bb(nx, ny, nw, na) *
                                      T2bb_[u][v][a][z] *
                                      L3bbb_[x][y][z][u][v][w];
                                // L3aab
                                E2 += 2 * L3aab_[x][z][y][u][v][w] *
                                      T2aa_[u][v][a][z] *
                                      integral_->aptei_ab(nx, ny, na, nw);
                                // L3aba & L3baa
                                E2 -= 4 * L3aab_[x][z][y][u][w][v] *
                                      T2ab_[u][v][z][a] *
                                      integral_->aptei_ab(nx, ny, nw, na);
                                E2 -= 2 * L3aab_[x][y][z][u][w][v] *
                                      T2ab_[u][v][a][z] *
                                      integral_->aptei_aa(nx, ny, nw, na);
                                // L3abb & L3bab
                                E2 -= 4 * L3abb_[x][y][z][u][v][w] *
                                      T2ab_[u][v][a][z] *
                                      integral_->aptei_ab(nx, ny, na, nw);
                                E2 -= 2 * L3abb_[z][x][y][u][v][w] *
                                      T2ab_[u][v][z][a] *
                                      integral_->aptei_bb(nx, ny, nw, na);
                                // L3bba
                                E2 += 2 * L3abb_[x][y][z][w][u][v] *
                                      T2bb_[u][v][a][z] *
                                      integral_->aptei_ab(nx, ny, nw, na);
                            }
                        }
            ...
            for (size_t j = 0; j < nh_; ++j) {
                ...
                for (size_t b = 0; b < npt_; ++b) {
                    ...
                    va += Fa_srg_[nb][nj] * Fa_srg_[ni][na] * d1 *
                          srg_source_->compute_renormalized_denominator(
                              d1 * d1 + d2 * d2);
                    ...
                    for (size_t u = 0; u < na_; ++u) {
                        ...
                        for (size_t v = 0; v < na_; ++v) {
                            ...
                            d1 = Fa_[ni][ni] + Fa_[nv][nv] - Fa_[na][na] -
                                 Fa_[nu][nu];
                            d2 = Fa_[nb][nb] - Fa_[nj][nj];
                            va += Fa_srg_[nb][nj] *
                                  integral_->aptei_aa(ni, nv, na, nu) *
                                  Da_[nu][nv] * d1 *
                                  srg_source_->compute_renormalized_denominator(
                                      d1 * d1 + d2 * d2);
                            d1 = Fa_[ni][ni] + Fb_[nv][nv] - Fa_[na][na] -
                                 Fb_[nu][nu];
                            va += Fa_srg_[nb][nj] *
                                  integral_->aptei_ab(ni, nv, na, nu) *
                                  Db_[nu][nv] * d1 *
                                  srg_source_->compute_renormalized_denominator(
                                      d1 * d1 + d2 * d2);

                            d1 = Fa_[nv][nv] + Fb_[ni][ni] - Fa_[nu][nu] -
                                 Fb_[na][na];
                            d2 = Fb_[nb][nb] - Fb_[nj][nj];
                            vb += Fb_srg_[nb][nj] *
                                  integral_->aptei_ab(nv, ni, nu, na) *
                                  Da_[nu][nv] * d1 *
                                  srg_source_->compute_renormalized_denominator(
                                      d1 * d1 + d2 * d2);
                            d1 = Fb_[nv][nv] + Fb_[ni][ni] - Fb_[nu][nu] -
                                 Fb_[na][na];
                            vb += Fb_srg_[nb][nj] *
                                  integral_->aptei_bb(nv, ni, nu, na) *
                                  Db_[nu][nv] * d1 *
                                  srg_source_->compute_renormalized_denominator(
                                      d1 * d1 + d2 * d2);

                            d1 = Fa_[ni][ni] - Fa_[na][na];
                            d2 = Fa_[nb][nb] + Fa_[nv][nv] - Fa_[nj][nj] -
                                 Fa_[nu][nu];
                            va += Fa_srg_[ni][na] *
                                  integral_->aptei_aa(nb, nv, nj, nu) *
                                  Da_[nu][nv] * d1 *
                                  srg_source_->compute_renormalized_denominator(
                                      d1 * d1 + d2 * d2);
                            d2 = Fa_[nb][nb] + Fb_[nv][nv] - Fa_[nj][nj] -
                                 Fb_[nu][nu];
                            va += Fa_srg_[ni][na] *
                                  integral_->aptei_ab(nb, nv, nj, nu) *
                                  Db_[nu][nv] * d1 *
                                  srg_source_->compute_renormalized_denominator(
                                      d1 * d1 + d2 * d2);

                            d1 = Fb_[ni][ni] - Fb_[na][na];
                            d2 = Fa_[nv][nv] + Fb_[nb][nb] - Fa_[nu][nu] -
                                 Fb_[nj][nj];
                            vb += Fb_srg_[ni][na] *
                                  integral_->aptei_ab(nv, nb, nu, nj) *
                                  Da_[nu][nv] * d1 *
                                  srg_source_->compute_renormalized_denominator(
                                      d1 * d1 + d2 * d2);
                            d2 = Fb_[nv][nv] + Fb_[nb][nb] - Fb_[nu][nu] -
                                 Fb_[nj][nj];
                            vb += Fb_srg_[ni][na] *
                                  integral_->aptei_bb(nv, nb, nu, nj) *
                                  Db_[nu][nv] * d1 *
                                  srg_source_->compute_renormalized_denominator(
                                      d1 * d1 + d2 * d2);
                            ...
                            for (size_t x = 0; x < na_; ++x) {
                                ...
                                for (size_t y = 0; y < na_; ++y) {
                                    ...
                                    d1 = Fa_[ni][ni] + Fa_[nv][nv] -
                                         Fa_[na][na] - Fa_[nu][nu];
                                    d2 = Fa_[nb][nb] + Fa_[ny][ny] -
                                         Fa_[nj][nj] - Fa_[nx][nx];
                                    va +=
                                        integral_->aptei_aa(nb, ny, nj, nx) *
                                        integral_->aptei_aa(ni, nv, na, nu) *
                                        Da_[nx][ny] * Da_[nu][nv] * d1 *
                                        srg_source_
                                            ->compute_renormalized_denominator(
                                                d1 * d1 + d2 * d2);

                                    d1 = Fa_[ni][ni] + Fb_[nv][nv] -
                                         Fa_[na][na] - Fb_[nu][nu];
                                    va +=
                                        integral_->aptei_aa(nb, ny, nj, nx) *
                                        integral_->aptei_ab(ni, nv, na, nu) *
                                        Da_[nx][ny] * Db_[nu][nv] * d1 *
                                        srg_source_
                                            ->compute_renormalized_denominator(
                                                d1 * d1 + d2 * d2);
                                    ...
                                    d1 = Fa_[ni][ni] + Fa_[nv][nv] -
                                         Fa_[na][na] - Fa_[nu][nu];
                                    d2 = Fa_[nb][nb] + Fb_[ny][ny] -
                                         Fa_[nj][nj] - Fb_[nx][nx];
                                    va +=
                                        integral_->aptei_ab(nb, ny, nj, nx) *
                                        integral_->aptei_aa(ni, nv, na, nu) *
                                        Db_[nx][ny] * Da_[nu][nv] * d1 *
                                        srg_source_
                                            ->compute_renormalized_denominator(
                                                d1 * d1 + d2 * d2);
                                    ...
                                    va +=
                                        integral_->aptei_ab(nb, ny, nj, nx) *
                                        integral_->aptei_ab(ni, nv, na, nu) *
                                        Db_[nx][ny] * Db_[nu][nv] * d1 *
                                        srg_source_
                                            ->compute_renormalized_denominator(
                                                d1 * d1 + d2 * d2);

                                    d1 = Fb_[ni][ni] + Fb_[nv][nv] -
                                         Fb_[na][na] - Fb_[nu][nu];
                                    d2 = Fb_[nb][nb] + Fb_[ny][ny] -
                                         Fb_[nj][nj] - Fb_[nx][nx];
                                    vb +=
                                        integral_->aptei_bb(nb, ny, nj, nx) *
                                        integral_->aptei_bb(ni, nv, na, nu) *
                                        Db_[nx][ny] * Db_[nu][nv] * d1 *
                                        srg_source_
                                            ->compute_renormalized_denominator(
                                                d1 * d1 + d2 * d2);

                                    d1 = Fa_[nv][nv] + Fb_[ni][ni] -
                                         Fb_[nu][nu] - Fb_[na][na];
                                    vb +=
                                        integral_->aptei_bb(nb, ny, nj, nx) *
                                        integral_->aptei_ab(nv, ni, nu, na) *
                                    ...
                                    d1 = Fb_[ni][ni] + Fb_[nv][nv] -
                                         Fb_[na][na] - Fb_[nu][nu];
                                    d2 = Fa_[ny][ny] + Fb_[nb][nb] -
                                         Fa_[nx][nx] - Fb_[nj][nj];
                                    vb +=
                                        integral_->aptei_ab(ny, nb, nx, nj) *
                                        integral_->aptei_bb(ni, nv, na, nu) *
                                        Da_[nx][ny] * Db_[nu][nv] * d1 *
                                        srg_source_
                                            ->compute_renormalized_denominator(
            ...
            for (size_t x = 0; x < na_; ++x) {
                ...
                for (size_t y = 0; y < na_; ++y) {
                    ...
                    for (size_t e = 0; e < nv_; ++e) {
                        ...
                        double d2 = Fa_[ne][ne] + Fa_[nv][nv] - Fa_[nx][nx] -
                                    Fa_[ny][ny];
                        ...
                        d2 = Fb_[ne][ne] + Fb_[nv][nv] - Fb_[nx][nx] -
                             Fb_[ny][ny];
                        ...
                        d2 = Fa_[ne][ne] + Fb_[nv][nv] - Fa_[nx][nx] -
                             Fb_[ny][ny];
                        ...
                        d2 = Fa_[nu][nu] + Fb_[ne][ne] - Fa_[nx][nx] -
                             Fb_[ny][ny];
                        ...
                        for (size_t w = 0; w < na_; ++w) {
                            ...
                            for (size_t z = 0; z < na_; ++z) {
                                ...
                                d1 = Fa_[nu][nu] + Fa_[nw][nw] - Fa_[ne][ne] -
                                     Fa_[nz][nz];
                                d2 = Fa_[ne][ne] + Fa_[nv][nv] - Fa_[nx][nx] -
                                     Fa_[ny][ny];
                                vaa += integral_->aptei_aa(ne, nv, nx, ny) *
                                       integral_->aptei_aa(nu, nw, ne, nz) *
                                       Da_[nz][nw] * d1 *
                                       srg_source_
                                           ->compute_renormalized_denominator(
                                               d1 * d1 + d2 * d2);
                                d1 = Fa_[nu][nu] + Fb_[nw][nw] - Fa_[ne][ne] -
                                     Fb_[nz][nz];
                                vaa += integral_->aptei_aa(ne, nv, nx, ny) *
                                       integral_->aptei_ab(nu, nw, ne, nz) *
                                       Db_[nz][nw] * d1 *
                                       srg_source_
                                           ->compute_renormalized_denominator(
                                               d1 * d1 + d2 * d2);

                                d1 = Fb_[nu][nu] + Fb_[nw][nw] - Fb_[ne][ne] -
                                     Fb_[nz][nz];
                                d2 = Fb_[ne][ne] + Fb_[nv][nv] - Fb_[nx][nx] -
                                     Fb_[ny][ny];
                                vbb += integral_->aptei_bb(ne, nv, nx, ny) *
                                       integral_->aptei_bb(nu, nw, ne, nz) *
                                       Db_[nz][nw] * d1 *
                                       srg_source_
                                           ->compute_renormalized_denominator(
                                               d1 * d1 + d2 * d2);
                                d1 = Fa_[nw][nw] + Fb_[nu][nu] - Fa_[nz][nz] -
                                     Fb_[ne][ne];
                                vbb += integral_->aptei_bb(ne, nv, nx, ny) *
                                       integral_->aptei_ab(nw, nu, nz, ne) *
                                       Da_[nz][nw] * d1 *
                                       srg_source_
                                           ->compute_renormalized_denominator(
                                               d1 * d1 + d2 * d2);
                                ...
                                d1 = Fa_[nw][nw] + Fb_[nv][nv] - Fa_[nz][nz] -
                                     Fb_[ne][ne];
                                d2 = Fa_[nu][nu] + Fb_[ne][ne] - Fa_[nx][nx] -
                                     Fb_[ny][ny];
                                vab += integral_->aptei_ab(nu, ne, nx, ny) *
                                       integral_->aptei_ab(nw, nv, nz, ne) *
                                       Da_[nz][nw] * d1 *
                                       srg_source_
                                           ->compute_renormalized_denominator(
                                               d1 * d1 + d2 * d2);
                                d1 = Fb_[nw][nw] + Fb_[nv][nv] - Fb_[nz][nz] -
                                     Fb_[ne][ne];
                                vab += integral_->aptei_ab(nu, ne, nx, ny) *
                                       integral_->aptei_bb(nw, nv, nz, ne) *
                                       Db_[nz][nw] * d1 *
                                       srg_source_
                                           ->compute_renormalized_denominator(
                                               d1 * d1 + d2 * d2);
                    ...
                    for (size_t m = 0; m < nc_; ++m) {
                        ...
                        double d2 = Fa_[nu][nu] + Fa_[nv][nv] - Fa_[nm][nm] -
                                    Fa_[ny][ny];
                        ...
                        d2 = Fb_[nu][nu] + Fb_[nv][nv] - Fb_[nm][nm] -
                             Fb_[ny][ny];
                        ...
                        d2 = Fa_[nu][nu] + Fb_[nv][nv] - Fa_[nm][nm] -
                             Fb_[ny][ny];
                        ...
                        d2 = Fa_[nu][nu] + Fb_[nv][nv] - Fa_[nx][nx] -
                             Fb_[nm][nm];
                        ...
                        for (size_t w = 0; w < na_; ++w) {
                            ...
                            for (size_t z = 0; z < na_; ++z) {
                                ...
                                d1 = Fa_[nm][nm] + Fa_[nw][nw] - Fa_[nx][nx] -
                                     Fa_[nz][nz];
                                d2 = Fa_[nu][nu] + Fa_[nv][nv] - Fa_[nm][nm] -
                                     Fa_[ny][ny];
                                vaa -= integral_->aptei_aa(nu, nv, nm, ny) *
                                       integral_->aptei_aa(nm, nw, nx, nz) *
                                       Da_[nw][nz] * d1 *
                                       srg_source_
                                           ->compute_renormalized_denominator(
                                               d1 * d1 + d2 * d2);
                                d1 = Fa_[nm][nm] + Fb_[nw][nw] - Fa_[nx][nx] -
                                     Fb_[nz][nz];
                                vaa -= integral_->aptei_aa(nu, nv, nm, ny) *
                                       integral_->aptei_ab(nm, nw, nx, nz) *
                                       Db_[nw][nz] * d1 *
                                       srg_source_
                                           ->compute_renormalized_denominator(
                                               d1 * d1 + d2 * d2);

                                d1 = Fb_[nm][nm] + Fb_[nw][nw] - Fb_[nx][nx] -
                                     Fb_[nz][nz];
                                d2 = Fb_[nu][nu] + Fb_[nv][nv] - Fb_[nm][nm] -
                                     Fb_[ny][ny];
                                vbb -= integral_->aptei_bb(nu, nv, nm, ny) *
                                       integral_->aptei_bb(nm, nw, nx, nz) *
                                       Db_[nw][nz] * d1 *
                                       srg_source_
                                           ->compute_renormalized_denominator(
                                               d1 * d1 + d2 * d2);
                                d1 = Fa_[nw][nw] + Fb_[nm][nm] - Fa_[nz][nz] -
                                     Fb_[nx][nx];
                                vbb -= integral_->aptei_bb(nu, nv, nm, ny) *
                                       integral_->aptei_ab(nw, nm, nz, nx) *
                                       Da_[nw][nz] * d1 *
                                       srg_source_
                                           ->compute_renormalized_denominator(
                                               d1 * d1 + d2 * d2);
                                ...
                                d2 = Fa_[nu][nu] + Fb_[nv][nv] - Fa_[nx][nx] -
                                     Fb_[nm][nm];
                                vab -= integral_->aptei_ab(nu, nv, nx, nm) *
                                       integral_->aptei_ab(nw, nm, nz, ny) *
                                       Da_[nw][nz] * d1 *
                                       srg_source_
                                           ->compute_renormalized_denominator(
                                               d1 * d1 + d2 * d2);
                                d1 = Fb_[nw][nw] + Fb_[nm][nm] - Fb_[nz][nz] -
                                     Fb_[ny][ny];
                                vab -= integral_->aptei_ab(nu, nv, nx, nm) *
                                       integral_->aptei_bb(nw, nm, nz, ny) *
                                       Db_[nw][nz] * d1 *
                                       srg_source_
                                           ->compute_renormalized_denominator(
                                               d1 * d1 + d2 * d2);
            ...
            for (size_t x = 0; x < na_; ++x) {
                ...
                for (size_t y = 0; y < na_; ++y) {
                    ...
                    for (size_t e = 0; e < nv_; ++e) {
                        ...
                        double d1 = Fa_[nu][nu] + Fa_[nv][nv] - Fa_[ne][ne] -
                                    Fa_[ny][ny];
                        ...
                        d1 = Fb_[nu][nu] + Fb_[nv][nv] - Fb_[ne][ne] -
                             Fb_[ny][ny];
                        d2 = Fb_[ne][ne] - Fb_[nx][nx];
                        vbb += Fb_srg_[ne][nx] *
                               integral_->aptei_bb(nu, nv, ne, ny) * d1 *
                               srg_source_->compute_renormalized_denominator(
                                   d1 * d1 + d2 * d2);

                        d1 = Fa_[nu][nu] + Fb_[nv][nv] - Fa_[ne][ne] -
                             Fb_[ny][ny];
                        d2 = Fa_[ne][ne] - Fa_[nx][nx];
                        vab += Fa_srg_[ne][nx] *
                               integral_->aptei_ab(nu, nv, ne, ny) * d1 *
                               srg_source_->compute_renormalized_denominator(
                                   d1 * d1 + d2 * d2);

                        d1 = Fa_[nu][nu] + Fb_[nv][nv] - Fa_[nx][nx] -
                             Fb_[ne][ne];
                        ...
                        for (size_t w = 0; w < na_; ++w) {
                            ...
                            for (size_t z = 0; z < na_; ++z) {
                                ...
                                d1 = Fa_[nu][nu] + Fa_[nv][nv] - Fa_[ne][ne] -
                                     Fa_[ny][ny];
                                d2 = Fa_[ne][ne] + Fa_[nw][nw] - Fa_[nx][nx] -
                                     Fa_[nz][nz];
                                vaa += integral_->aptei_aa(nu, nv, ne, ny) *
                                       integral_->aptei_aa(ne, nw, nx, nz) *
                                       Da_[nz][nw] * d1 *
                                       srg_source_
                                           ->compute_renormalized_denominator(
                                               d1 * d1 + d2 * d2);
                                d2 = Fa_[ne][ne] + Fb_[nw][nw] - Fa_[nx][nx] -
                                     Fb_[nz][nz];
                                vaa += integral_->aptei_aa(nu, nv, ne, ny) *
                                       integral_->aptei_ab(ne, nw, nx, nz) *
                                       Db_[nz][nw] * d1 *
                                       srg_source_
                                           ->compute_renormalized_denominator(
                                               d1 * d1 + d2 * d2);
                                ...
                                d1 = Fb_[nu][nu] + Fb_[nv][nv] - Fb_[ne][ne] -
                                     Fb_[ny][ny];
                                d2 = Fb_[ne][ne] + Fb_[nw][nw] - Fb_[nx][nx] -
                                     Fb_[nz][nz];
                                vbb += integral_->aptei_bb(nu, nv, ne, ny) *
                                       integral_->aptei_bb(ne, nw, nx, nz) *
                                       Db_[nz][nw] * d1 *
                                       srg_source_
                                           ->compute_renormalized_denominator(
                                               d1 * d1 + d2 * d2);
                                d2 = Fa_[nw][nw] + Fb_[ne][ne] - Fa_[nz][nz] -
                                     Fb_[nx][nx];
                                vbb += integral_->aptei_bb(nu, nv, ne, ny) *
                                       integral_->aptei_ab(nw, ne, nz, nx) *
                                       Da_[nz][nw] * d1 *
                                       srg_source_
                                           ->compute_renormalized_denominator(
                                               d1 * d1 + d2 * d2);
                                ...
                                vab += integral_->aptei_ab(nu, nv, nx, ne) *
                                       integral_->aptei_ab(nw, ne, nz, ny) *
                                       Da_[nz][nw] * d1 *
                                       srg_source_
                                           ->compute_renormalized_denominator(
                                               d1 * d1 + d2 * d2);
                                d2 = Fb_[nw][nw] + Fb_[ne][ne] - Fb_[nz][nz] -
                                     Fb_[ny][ny];
                                vab += integral_->aptei_ab(nu, nv, nx, ne) *
                                       integral_->aptei_bb(nw, ne, nz, ny) *
                                       Db_[nz][nw] * d1 *
                                       srg_source_
                                           ->compute_renormalized_denominator(
                                               d1 * d1 + d2 * d2);
                    ...
                    for (size_t m = 0; m < nc_; ++m) {
                        ...
                        double d1 = Fa_[nu][nu] + Fa_[nm][nm] - Fa_[nx][nx] -
                                    Fa_[ny][ny];
                        double d2 = Fa_[nv][nv] - Fa_[nm][nm];
                        vaa -= Fa_srg_[nv][nm] *
                               integral_->aptei_aa(nu, nm, nx, ny) * d1 *
                               srg_source_->compute_renormalized_denominator(
                                   d1 * d1 + d2 * d2);

                        d1 = Fb_[nu][nu] + Fb_[nm][nm] - Fb_[nx][nx] -
                             Fb_[ny][ny];
                        d2 = Fb_[nv][nv] - Fb_[nm][nm];
                        vbb -= Fb_srg_[nv][nm] *
                               integral_->aptei_bb(nu, nm, nx, ny) * d1 *
                               srg_source_->compute_renormalized_denominator(
                                   d1 * d1 + d2 * d2);

                        d1 = Fa_[nu][nu] + Fb_[nm][nm] - Fa_[nx][nx] -
                             Fb_[ny][ny];
                        d2 = Fb_[nv][nv] - Fb_[nm][nm];
                        vab -= Fb_srg_[nv][nm] *
                        ...
                        d1 = Fa_[nm][nm] + Fb_[nv][nv] - Fa_[nx][nx] -
                             Fb_[ny][ny];
                        d2 = Fa_[nu][nu] - Fa_[nm][nm];
                        vab -= Fa_srg_[nu][nm] *
                               integral_->aptei_ab(nm, nv, nx, ny) * d1 *
                               srg_source_->compute_renormalized_denominator(
                                   d1 * d1 + d2 * d2);
                        ...
                        for (size_t w = 0; w < na_; ++w) {
                            ...
                            for (size_t z = 0; z < na_; ++z) {
                                ...
                                d1 = Fa_[nu][nu] + Fa_[nm][nm] - Fa_[nx][nx] -
                                     Fa_[ny][ny];
                                d2 = Fa_[nv][nv] + Fa_[nw][nw] - Fa_[nm][nm] -
                                     Fa_[nz][nz];
                                vaa -= integral_->aptei_aa(nu, nm, nx, ny) *
                                       integral_->aptei_aa(nv, nw, nm, nz) *
                                       Da_[nw][nz] * d1 *
                                       srg_source_
                                           ->compute_renormalized_denominator(
                                               d1 * d1 + d2 * d2);
                                d2 = Fa_[nv][nv] + Fb_[nw][nw] - Fa_[nm][nm] -
                                     Fb_[nz][nz];
                                vaa -= integral_->aptei_aa(nu, nm, nx, ny) *
                                       integral_->aptei_ab(nv, nw, nm, nz) *
                                       Db_[nw][nz] * d1 *
                                       srg_source_
                                           ->compute_renormalized_denominator(
                                               d1 * d1 + d2 * d2);
                                ...
                                d1 = Fb_[nu][nu] + Fb_[nm][nm] - Fb_[nx][nx] -
                                     Fb_[ny][ny];
                                d2 = Fb_[nv][nv] + Fb_[nw][nw] - Fb_[nm][nm] -
                                     Fb_[nz][nz];
                                vbb -= integral_->aptei_bb(nu, nm, nx, ny) *
                                       integral_->aptei_bb(nv, nw, nm, nz) *
                                       Db_[nw][nz] * d1 *
                                       srg_source_
                                           ->compute_renormalized_denominator(
                                               d1 * d1 + d2 * d2);
                                d2 = Fa_[nw][nw] + Fb_[nv][nv] - Fa_[nz][nz] -
                                     Fb_[nm][nm];
                                vbb -= integral_->aptei_bb(nu, nm, nx, ny) *
                                       integral_->aptei_ab(nw, nv, nz, nm) *
                                       Da_[nw][nz] * d1 *
                                       srg_source_
                                           ->compute_renormalized_denominator(
                                               d1 * d1 + d2 * d2);
                                ...
                                vab -= integral_->aptei_ab(nu, nm, nx, ny) *
                                       ...
                                       srg_source_
                                           ->compute_renormalized_denominator(
                                               d1 * d1 + d2 * d2);
                                d2 = Fb_[nw][nw] + Fb_[nv][nv] - Fb_[nz][nz] -
                                     Fb_[nm][nm];
                                vab -= integral_->aptei_ab(nu, nm, nx, ny) *
                                       integral_->aptei_bb(nw, nv, nz, nm) *
                                       Db_[nw][nz] * d1 *
                                       srg_source_
                                           ->compute_renormalized_denominator(
                                               d1 * d1 + d2 * d2);
                                ...
                                d1 = Fa_[nm][nm] + Fb_[nv][nv] - Fa_[nx][nx] -
                                     Fb_[ny][ny];
                                d2 = Fa_[nu][nu] + Fa_[nw][nw] - Fa_[nm][nm] -
                                     Fa_[nz][nz];
                                vab -= integral_->aptei_ab(nm, nv, nx, ny) *
                                       integral_->aptei_aa(nu, nw, nm, nz) *
                                       Da_[nw][nz] * d1 *
                                       srg_source_
                                           ->compute_renormalized_denominator(
                                               d1 * d1 + d2 * d2);
                                d2 = Fa_[nu][nu] + Fb_[nw][nw] - Fa_[nm][nm] -
                                     Fb_[nz][nz];
                                vab -= integral_->aptei_ab(nm, nv, nx, ny) *
                                       integral_->aptei_ab(nu, nw, nm, nz) *
                                       Db_[nw][nz] * d1 *
                                       srg_source_
                                           ->compute_renormalized_denominator(
                                               d1 * d1 + d2 * d2);
                        ...
                        for (size_t l = 0; l < nh_; ++l) {
                            ...
                            for (size_t c = 0; c < npt_; ++c) {
                                ...
                                for (size_t d = 0; d < npt_; ++d) {
                                    ...
                                    double d1 = Fa_[ni][ni] + Fa_[nj][nj] -
                                                Fa_[na][na] - Fa_[nb][nb];
                                    double d2 = Fa_[nc][nc] + Fa_[nd][nd] -
                                                Fa_[nk][nk] - Fa_[nl][nl];
                                    E += 0.5 *
                                         integral_->aptei_aa(nc, nd, nk, nl) *
                                         integral_->aptei_aa(na, nb, ni, nj) *
                                         Da_[nk][ni] * Da_[nl][nj] *
                                         (Delta(na, nc) - Da_[na][nc]) *
                                    ...
                                    d1 = Fa_[ni][ni] + Fb_[nj][nj] -
                                         Fa_[na][na] - Fb_[nb][nb];
                                    d2 = Fa_[nc][nc] + Fb_[nd][nd] -
                                         Fa_[nk][nk] - Fb_[nl][nl];
                                    E += 2.0 *
                                         integral_->aptei_ab(nc, nd, nk, nl) *
                                         integral_->aptei_ab(na, nb, ni, nj) *
                                         Da_[nk][ni] * Db_[nl][nj] *
                                         (Delta(na, nc) - Da_[na][nc]) *
                                    ...
                                    d1 = Fb_[ni][ni] + Fb_[nj][nj] -
                                         Fb_[na][na] - Fb_[nb][nb];
                                    d2 = Fb_[nc][nc] + Fb_[nd][nd] -
                                         Fb_[nk][nk] - Fb_[nl][nl];
                                    E += 0.5 *
                                         integral_->aptei_bb(nc, nd, nk, nl) *
                                         integral_->aptei_bb(na, nb, ni, nj) *
                                         Db_[nk][ni] * Db_[nl][nj] *
                                         (Delta(na, nc) - Db_[na][nc]) *
                                         (Delta(nb, nd) - Db_[nb][nd]) * d1 *
                                         srg_source_
                        ...
                        for (size_t j = 0; j < nh_; ++j) {
                            ...
                            for (size_t k = 0; k < nh_; ++k) {
                                ...
                                for (size_t l = 0; l < nh_; ++l) {
                                    ...
                                    double d1 = Fa_[ni][ni] + Fa_[nj][nj] -
                                                Fa_[nx][nx] - Fa_[ny][ny];
                                    double d2 = Fa_[nu][nu] + Fa_[nv][nv] -
                                                Fa_[nk][nk] - Fa_[nl][nl];
                                    vaa +=
                                        0.25 *
                                        integral_->aptei_aa(nu, nv, nk, nl) *
                                        integral_->aptei_aa(ni, nj, nx, ny) *
                                    ...
                                    d1 = Fb_[ni][ni] + Fb_[nj][nj] -
                                         Fb_[nx][nx] - Fb_[ny][ny];
                                    d2 = Fb_[nu][nu] + Fb_[nv][nv] -
                                         Fb_[nk][nk] - Fb_[nl][nl];
                                    vbb +=
                                        0.25 *
                                        integral_->aptei_bb(nu, nv, nk, nl) *
                                        integral_->aptei_bb(ni, nj, nx, ny) *
                                    ...
                                    d1 = Fa_[ni][ni] + Fb_[nj][nj] -
                                         Fa_[nx][nx] - Fb_[ny][ny];
                                    d2 = Fa_[nu][nu] + Fb_[nv][nv] -
                                         Fa_[nk][nk] - Fb_[nl][nl];
                                    vab +=
                                        2.0 *
                                        integral_->aptei_ab(nu, nv, nk, nl) *
                                        integral_->aptei_ab(ni, nj, nx, ny) *
                        ...
                        for (size_t b = 0; b < npt_; ++b) {
                            ...
                            for (size_t c = 0; c < npt_; ++c) {
                                ...
                                for (size_t d = 0; d < npt_; ++d) {
                                    ...
                                    double d1 = Fa_[nu][nu] + Fa_[nv][nv] -
                                                Fa_[na][na] - Fa_[nb][nb];
                                    double d2 = Fa_[nc][nc] + Fa_[nd][nd] -
                                                Fa_[nx][nx] - Fa_[ny][ny];
                                    vaa +=
                                        0.25 *
                                        integral_->aptei_aa(nc, nd, nx, ny) *
                                    ...
                                    d1 = Fb_[nu][nu] + Fb_[nv][nv] -
                                         Fb_[na][na] - Fb_[nb][nb];
                                    d2 = Fb_[nc][nc] + Fb_[nd][nd] -
                                         Fb_[nx][nx] - Fb_[ny][ny];
                                    vbb +=
                                        0.25 *
                                        integral_->aptei_bb(nc, nd, nx, ny) *
                                    ...
                                    d1 = Fa_[nu][nu] + Fb_[nv][nv] -
                                         Fa_[na][na] - Fb_[nb][nb];
                                    d2 = Fa_[nc][nc] + Fb_[nd][nd] -
                                         Fa_[nx][nx] - Fb_[ny][ny];
                                    vab +=
                                        2.0 *
                                        integral_->aptei_ab(nc, nd, nx, ny) *
                        ...
                        for (size_t a = 0; a < npt_; ++a) {
                            ...
                            for (size_t j = 0; j < nh_; ++j) {
                                ...
                                for (size_t b = 0; b < npt_; ++b) {
                                    ...
                                    double d1 = Fa_[ni][ni] + Fa_[nv][nv] -
                                                Fa_[na][na] - Fa_[ny][ny];
                                    double d2 = Fa_[nb][nb] + Fa_[nu][nu] -
                                                Fa_[nj][nj] - Fa_[nx][nx];
                                    vaa +=
                                        2.0 *
                                        integral_->aptei_aa(nb, nu, nj, nx) *
                                        integral_->aptei_aa(ni, nv, na, ny) *
                                        Da_[nj][ni] *
                                    ...
                                    d1 = Fa_[nv][nv] + Fb_[ni][ni] -
                                         Fa_[ny][ny] - Fb_[na][na];
                                    d2 = Fa_[nu][nu] + Fb_[nb][nb] -
                                         Fa_[nx][nx] - Fb_[nj][nj];
                                    vaa +=
                                        2.0 *
                                        integral_->aptei_ab(nu, nb, nx, nj) *
                                        integral_->aptei_ab(nv, ni, ny, na) *
                                        Db_[nj][ni] *
                                    ...
                                    d1 = Fb_[ni][ni] + Fb_[nv][nv] -
                                         Fb_[na][na] - Fb_[ny][ny];
                                    d2 = Fb_[nb][nb] + Fb_[nu][nu] -
                                         Fb_[nj][nj] - Fb_[nx][nx];
                                    vbb +=
                                        2.0 *
                                        integral_->aptei_bb(nb, nu, nj, nx) *
                                        integral_->aptei_bb(ni, nv, na, ny) *
                                        Db_[nj][ni] *
                                        (Delta(na, nb) - Db_[na][nb]) * d1 *
                                        srg_source_
                                            ->compute_renormalized_denominator(
                                    ...
                                    d1 = Fa_[ni][ni] + Fb_[nv][nv] -
                                         Fa_[na][na] - Fb_[ny][ny];
                                    d2 = Fa_[nb][nb] + Fb_[nu][nu] -
                                         Fa_[nj][nj] - Fb_[nx][nx];
                                    vbb +=
                                        2.0 *
                                        integral_->aptei_ab(nb, nu, nj, nx) *
                                        integral_->aptei_ab(ni, nv, na, ny) *
                                        Da_[nj][ni] *
                                        (Delta(na, nb) - Da_[na][nb]) * d1 *
                                        srg_source_
                                            ->compute_renormalized_denominator(
                                    ...
                                    d1 = Fa_[nu][nu] + Fb_[ni][ni] -
                                         Fa_[na][na] - Fb_[ny][ny];
                                    d2 = Fa_[nb][nb] + Fb_[nv][nv] -
                                         Fa_[nx][nx] - Fb_[nj][nj];
                                    vab -=
                                        2.0 *
                                        integral_->aptei_ab(nb, nv, nx, nj) *
                                        integral_->aptei_ab(nu, ni, na, ny) *
                                        Db_[nj][ni] *
                                        (Delta(na, nb) - Da_[na][nb]) * d1 *
                                        srg_source_
                                            ->compute_renormalized_denominator(
                                    ...
                                    d1 = Fa_[ni][ni] + Fb_[nv][nv] -
                                         Fa_[na][na] - Fb_[ny][ny];
                                    d2 = Fa_[nu][nu] + Fa_[nb][nb] -
                                         Fa_[nx][nx] - Fa_[nj][nj];
                                    vab +=
                                        2.0 *
                                        integral_->aptei_aa(nu, nb, nx, nj) *
                                        integral_->aptei_ab(ni, nv, na, ny) *
                                        Da_[nj][ni] *
                                        (Delta(na, nb) - Da_[na][nb]) * d1 *
                                        srg_source_
                                    ...
                                    vab -=
                                        2.0 *
                                        integral_->aptei_ab(nu, nb, nj, ny) *
                                        integral_->aptei_ab(ni, nv, nx, na) *
                                        Da_[nj][ni] *
                                        (Delta(na, nb) - Db_[na][nb]) * d1 *
                                        srg_source_
                                            ->compute_renormalized_denominator(
                                    ...
                                    d1 = Fa_[ni][ni] + Fa_[nu][nu] -
                                         Fa_[na][na] - Fa_[nx][nx];
                                    d2 = Fa_[nb][nb] + Fb_[nv][nv] -
                                         Fa_[nj][nj] - Fb_[ny][ny];
                                    vab +=
                                        2.0 *
                                        integral_->aptei_ab(nb, nv, nj, ny) *
                                        integral_->aptei_aa(ni, nu, na, nx) *
                                        Da_[nj][ni] *
                                        (Delta(na, nb) - Da_[na][nb]) * d1 *
                                        srg_source_
                                            ->compute_renormalized_denominator(
                                    ...
                                    d1 = Fa_[nu][nu] + Fb_[ni][ni] -
                                         Fa_[nx][nx] - Fb_[na][na];
                                    d2 = Fb_[nb][nb] + Fb_[nv][nv] -
                                         Fb_[nj][nj] - Fb_[ny][ny];
                                    vab +=
                                        2.0 *
                                        integral_->aptei_bb(nb, nv, nj, ny) *
                                        integral_->aptei_ab(nu, ni, nx, na) *
                                        Db_[nj][ni] *
                                        (Delta(na, nb) - Db_[na][nb]) * d1 *
                                        srg_source_
                                            ->compute_renormalized_denominator(
                ...
                for (size_t x = 0; x < na_; ++x) {
                    ...
                    for (size_t y = 0; y < na_; ++y) {
                        ...
                        for (size_t z = 0; z < na_; ++z) {
                            ...
                            double vaaa = 0.0, vaab = 0.0, vabb = 0.0,
                                   vbbb = 0.0;
                            ...
                            for (size_t m = 0; m < nc_; ++m) {
                                ...
                                double d1 = Fa_[nm][nm] + Fa_[nw][nw] -
                                            Fa_[nx][nx] - Fa_[ny][ny];
                                double d2 = Fa_[nu][nu] + Fa_[nv][nv] -
                                            Fa_[nm][nm] - Fa_[nz][nz];
                                vaaa +=
                                    0.5 * integral_->aptei_aa(nu, nv, nm, nz) *
                                ...
                                d2 = Fb_[nu][nu] + Fb_[nv][nv] - Fb_[nm][nm] -
                                     Fb_[nz][nz];
                                vbbb +=
                                    0.5 * integral_->aptei_bb(nu, nv, nm, nz) *
                                    integral_->aptei_bb(nm, nw, nx, ny) * d1 *
                                    srg_source_
                                        ->compute_renormalized_denominator(
                                            d1 * d1 + d2 * d2);

                                d1 = Fb_[nm][nm] + Fa_[nv][nv] - Fa_[nx][nx] -
                                     Fb_[nz][nz];
                                d2 = Fa_[nu][nu] + Fb_[nw][nw] - Fb_[nm][nm] -
                                     Fa_[ny][ny];
                                vaab +=
                                    2.0 * integral_->aptei_ab(nu, nw, ny, nm) *
                                ...
                                d1 = Fa_[nm][nm] + Fa_[nv][nv] - Fa_[nx][nx] -
                                     Fa_[ny][ny];
                                d2 = Fa_[nu][nu] + Fb_[nw][nw] - Fa_[nm][nm] -
                                     Fb_[nz][nz];
                                vaab -= integral_->aptei_ab(nu, nw, nm, nz) *
                                        integral_->aptei_aa(nm, nv, nx, ny) *
                                        d1 *
                                        srg_source_
                                            ->compute_renormalized_denominator(
                                                d1 * d1 + d2 * d2);
                                ...
                                d2 = Fa_[nu][nu] + Fb_[nv][nv] - Fa_[nm][nm] -
                                     Fb_[nz][nz];
                                vabb +=
                                    2.0 * integral_->aptei_ab(nu, nv, nm, nz) *
                                    integral_->aptei_ab(nm, nw, nx, ny) * d1 *
                                    srg_source_
                                        ->compute_renormalized_denominator(
                                            d1 * d1 + d2 * d2);

                                d1 = Fb_[nm][nm] + Fa_[nu][nu] - Fa_[nx][nx] -
                                     Fb_[ny][ny];
                                d2 = Fb_[nw][nw] + Fb_[nv][nv] - Fb_[nm][nm] -
                                     Fb_[nz][nz];
                                vabb -= integral_->aptei_bb(nv, nw, nm, nz) *
                                        integral_->aptei_ab(nu, nm, nx, ny) *
                                        d1 *
                                        srg_source_
                                            ->compute_renormalized_denominator(
                                                d1 * d1 + d2 * d2);

                                d1 = Fb_[nm][nm] + Fb_[nw][nw] - Fb_[nz][nz] -
                                     Fb_[ny][ny];
                                d2 = Fa_[nu][nu] + Fb_[nv][nv] - Fb_[nm][nm] -
                                     Fa_[nx][nx];
                                vabb -= integral_->aptei_ab(nu, nv, nx, nm) *
                                        integral_->aptei_bb(nm, nw, ny, nz) *
                                        d1 *
                                        srg_source_
                                            ->compute_renormalized_denominator(
                                                d1 * d1 + d2 * d2);
                            ...
                            for (size_t e = 0; e < nv_; ++e) {
                                ...
                                double d1 = Fa_[nu][nu] + Fa_[nv][nv] -
                                            Fa_[ne][ne] - Fa_[nz][nz];
                                double d2 = Fa_[nw][nw] + Fa_[ne][ne] -
                                            Fa_[nx][nx] - Fa_[ny][ny];
                                vaaa +=
                                    0.5 * integral_->aptei_aa(nw, ne, nx, ny) *
                                ...
                                d1 = Fb_[nu][nu] + Fb_[nv][nv] - Fb_[ne][ne] -
                                     Fb_[nz][nz];
                                d2 = Fb_[nw][nw] + Fb_[ne][ne] - Fb_[nx][nx] -
                                     Fb_[ny][ny];
                                vbbb +=
                                    0.5 * integral_->aptei_bb(nw, ne, nx, ny) *
                                    integral_->aptei_bb(nu, nv, ne, nz) * d1 *
                                    srg_source_
                                        ->compute_renormalized_denominator(
                                            d1 * d1 + d2 * d2);

                                d1 = Fa_[nu][nu] + Fb_[nw][nw] - Fa_[ny][ny] -
                                     Fb_[ne][ne];
                                d2 = Fa_[nv][nv] + Fb_[ne][ne] - Fa_[nx][nx] -
                                     Fb_[nz][nz];
                                vaab -=
                                    2.0 * integral_->aptei_ab(nv, ne, nx, nz) *
                                ...
                                vaab += integral_->aptei_ab(ne, nw, nx, nz) *
                                        integral_->aptei_aa(nu, nv, ne, ny) *
                                        d1 *
                                        srg_source_
                                            ->compute_renormalized_denominator(
                                                d1 * d1 + d2 * d2);
                                ...
                                d1 = Fa_[nu][nu] + Fb_[nw][nw] - Fa_[ne][ne] -
                                     Fb_[nz][nz];
                                d2 = Fa_[nv][nv] + Fa_[ne][ne] - Fa_[nx][nx] -
                                     Fa_[ny][ny];
                                ...
                                d2 = Fa_[ne][ne] + Fb_[nw][nw] - Fa_[nx][nx] -
                                     Fb_[ny][ny];
                                vabb -=
                                    2.0 * integral_->aptei_ab(ne, nw, nx, ny) *
                                ...
                                d1 = Fa_[nu][nu] + Fb_[nv][nv] - Fa_[nx][nx] -
                                     Fb_[ne][ne];
                                d2 = Fb_[nw][nw] + Fb_[ne][ne] - Fb_[ny][ny] -
                                     Fb_[nz][nz];
                                vabb -= integral_->aptei_bb(nw, ne, ny, nz) *
                                        integral_->aptei_ab(nu, nv, nx, ne) *
                                        d1 *
                                        srg_source_
                                            ->compute_renormalized_denominator(
                                                d1 * d1 + d2 * d2);

                                d1 = Fb_[nv][nv] + Fb_[nw][nw] - Fb_[ne][ne] -
                                     Fb_[nz][nz];
                                d2 = Fa_[nu][nu] + Fb_[ne][ne] - Fa_[nx][nx] -
                                     Fb_[ny][ny];
                                vabb += integral_->aptei_ab(nu, ne, nx, ny) *
                                        integral_->aptei_bb(nv, nw, ne, nz) *
                                        d1 *
                                        srg_source_
                                            ->compute_renormalized_denominator(
                                                d1 * d1 + d2 * d2);

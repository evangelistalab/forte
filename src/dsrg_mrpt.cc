...
void DSRG_MRPT::read_options(){
    ...
    if(corr_lv_ != "PT2" && corr_lv_ != "PT3"){
        outfile->Printf("\n  Warning: CORR_LEVEL option \"%s\" is not available in DSRG_MRPT. Changed to PT2.", corr_lv_.c_str());
        corr_lv_ = "PT2";
    }

    ref_relax_ = options_.get_str("RELAX_REF");
    if(ref_relax_ != "NONE" && ref_relax_ != "ONCE"){
        outfile->Printf("\n  Warning: RELAX_REF option \"%s\" is not available in DSRG_MRPT. Changed to ONCE", ref_relax_.c_str());
        ref_relax_ = "ONCE";
    }
    ...
    if(taylor_threshold_ <= 0){
        outfile->Printf("\n  Error: Threshold for Taylor expansion must be an integer greater than 0!");
...
void DSRG_MRPT::startup(){
    ...
    od_blocks.erase(std::remove_if(od_blocks.begin(), od_blocks.end(),
                                 [&](std::string i) -> bool
    {return std::find(throw_blocks.begin(), throw_blocks.end(), i) != throw_blocks.end();}),
            od_blocks.end());
//    od_blocks.erase(std::remove(od_blocks.begin(), od_blocks.end(), "ccvv"), od_blocks.end());
...
void DSRG_MRPT::build_density(){
    ...
    diff("pqrs") = reference_.L2aa()("pqrs")
            - reference_.L2ab()("pqrs") + reference_.L2ab()("pqsr");
    if(diff.norm() > 1.0e-8){
        outfile->Printf("\n  Error: two-particle density cumulant cannot be spin-adapted!");
        outfile->Printf("\n  |L2[pqrs] - (L2[pQrS] - L2[pQsR])| = %20.15f  <== This should be 0.0.", diff.norm());
    ...
    if(options_.get_str("THREEPDC") != "ZERO"){
        ...
        diff("pqrstu") += reference_.L3aab()("pqrstu") - reference_.L3aab()("pqrsut") + reference_.L3aab()("pqrtus");
        diff("pqrstu") -= reference_.L3aab()("prqstu") - reference_.L3aab()("prqsut") + reference_.L3aab()("prqtus");
        diff("pqrstu") += reference_.L3aab()("qrpstu") - reference_.L3aab()("qrpsut") + reference_.L3aab()("qrptus");
        diff.scale(1.0/3.0);
        diff("pqrstu") -= reference_.L3aaa()("pqrstu");
        if(diff.norm() > 1.0e-8){
            outfile->Printf("\n  Error: three-particle density cumulant cannot be spin-adapted!");
            outfile->Printf("\n  |L3aaa - 1/3 * P(L3aab)| = %20.15f  <== This should be 0.0.", diff.norm());
...
void DSRG_MRPT::build_fock(){
    ...
    ambit::BlockedTensor VFock = ambit::BlockedTensor::build(tensor_type_,"VFock",{"caga","aapa","vava"});
...
bool DSRG_MRPT::check_semicanonical(){
    ...
    ambit::BlockedTensor diff = ambit::BlockedTensor::build(tensor_type_,"F - H0",{"cc","aa","vv"});
    diff["mn"] = F_["mn"];
    diff["uv"] = F_["uv"];
    diff["ef"] = F_["ef"];
    ...
    for(auto& block: {"cc","aa","vv"}){
        ...
        auto iter = std::max_element(data.begin(), data.end(),[&](double x, double y) {return std::fabs(x) < std::fabs(y);});
    ...
    if(Fd_od_sum > threshold){
        ...
        outfile->Printf("\n    Off-diagonal elements of the core, active, virtual blocks of Fock matrix");
        outfile->Printf("\n         %15s %15s %15s", "core", "active", "virtual");
        outfile->Printf("\n    %s", sep.c_str());
        outfile->Printf("\n    Max  %15.10f %15.10f %15.10f", Fd_od_max[0], Fd_od_max[1], Fd_od_max[2]);
        outfile->Printf("\n    Norm %15.10f %15.10f %15.10f", Fd_od_norm[0], Fd_od_norm[1], Fd_od_norm[2]);
        outfile->Printf("\n    %s\n", sep.c_str());
        outfile->Printf("\n    Warning: these elements above will be ignored!");
        outfile->Printf("\n    The DSRG-MRPT energy will make sense only when these elements are small (< 1.0e-6).");
    }else{
        outfile->Printf("     OK.");
...
void DSRG_MRPT::test_memory(const size_t& c, const size_t& a, const size_t& v){
    ...
    // We store H[gg*], F[gg*], T1[hp*], V[hhpp_], T2[hhpp_], L1[aa], LH1[aa], L2[aaaa], L3[aaaaaa]
    ...
    auto converter = [&](size_t numel, bool mem = false){
        size_t bytes;
        if(mem) bytes = numel;
        else bytes = numel * sizeof(double);
        std::string out;
        for(auto& XB: to_XB){
    ...
    if(leftover < 0){
        outfile->Printf("\n  Error: Not enough memory! Need %s of memory.",converter(required).c_str());

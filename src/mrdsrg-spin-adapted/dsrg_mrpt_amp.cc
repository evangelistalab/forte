...
void DSRG_MRPT::compute_T_1st(BlockedTensor& V, BlockedTensor& T2,
                              BlockedTensor& F, BlockedTensor& T1) {
...
void DSRG_MRPT::compute_T2_1st(BlockedTensor& V, BlockedTensor& T2) {
    ...
    T2.iterate([&](const std::vector<size_t>& i, const std::vector<SpinType>&,
                   double& value) {
...
void DSRG_MRPT::compute_T1_1st(BlockedTensor& F, BlockedTensor& T2,
                               BlockedTensor& T1) {
    Timer timer;
    ...
    T1.iterate([&](const std::vector<size_t>& i, const std::vector<SpinType>&,
                   double& value) {
        if (std::fabs(value) > 1.0e-15) {
            value *= dsrg_source_->compute_renormalized_denominator(
...
void DSRG_MRPT::analyze_amplitudes(BlockedTensor& T1, BlockedTensor& T2,
                                   std::string name) {
    if (!name.empty())
        name += " ";
...
void DSRG_MRPT::print_amp_summary(

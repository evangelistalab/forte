...
std::vector<std::pair<std::string,double>> MRDSRG::compute_energy_pt2_FdiagV(){
    ...
    if (max_diis_vectors > 0){
        diis_manager = std::shared_ptr<DIISManager>(new DIISManager(max_diis_vectors, "MRPT2 DIIS T", DIISManager::LargestError, DIISManager::InCore));
...
//    }
    ...
    do{
        ...
        outfile->Printf("\n    %5d  %10.3e %10.3e  %10.3e %10.3e  %8.3f %8.3f  ",
                        cycle, Hbar1od, Hbar2od, T1rms_, T2rms_, time_hbar, time_amp);

 ...
 */
void Explorer::explore_singles_sequential(psi::Options& options)
{
    ...
    std::vector<double> fock_diagonal_beta(nmo_,0.0);
    ...
    selected_determinants_alpha.push_back(boost::make_tuple(ref_energy_,wavefunction_symmetry_,zero_ex));
    ...
    while(iterate){
        ...
        for (int n = 0; n < maxn; ++n){
            ...
            ints_->make_alpha_fock_diagonal(str_ex.get_alfa_bits(),str_ex.get_beta_bits(),fock_diagonal_alpha);
            ...
            for (int i = maxai - 1; i >= minai; --i){
                for (int a = minaa; a < maxaa; ++a){
                    ...
                    if (relative_energy < denominator_threshold_){
                        ...
                        selected_determinants_alpha.push_back(boost::make_tuple(absolute_energy,irrep_ia_ex,ia_ex));
    ...
    while(iterate){
        ...
        for (int n = 0; n < maxn; ++n){
            ...
            ints_->make_beta_fock_diagonal(str_ex.get_alfa_bits(),str_ex.get_beta_bits(),fock_diagonal_beta);

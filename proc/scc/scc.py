...
def make_forte_objs(psi4_wfn,mo_spaces,localize=False):
    ...
    if localize:
        ...
        forte_objs = forte.utils.prepare_forte_objects(psi4_wfn,mo_spaces=mo_spaces, active_space = 'CORRELATED',core_spaces = [],localize = True,localize_spaces = ['RESTRICTED_DOCC','RESTRICTED_UOCC'])
    else:
...
def solve_selected_cc_equations(cc_type,t1,op,selected_op,op_pool,denominators,ref,as_ints,compute_threshold,e_convergence = 1.0e-10,r_convergence = 1.0e-10, maxiter = 100):
    ...
    for micro_iter in range(maxiter):
        ...
        print(f'    {micro_iter:4d} {e:20.12f}   {delta_e_micro:+6e}   {residual_norm:+6e}   {time.time() - micro_start:8.3f}', flush=True)
...
def run_selected_ucc(forte_objs,psi4_wfn, max_exc, omega, ordering = 4,e_convergence = 1.0e-12,r_convergence = 1.0e-10,compute_threshold = 1.0e-10, selection_threshold = 1.0e-10):

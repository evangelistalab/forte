...
// #pragma omp critical
//         {
//             Bm_vec[thread].data() = ints_->three_integral_block(aux_mos_, virt_mos_,
//             {im}).data(); if (!semi_canonical_) {
...
double SA_MRPT2::E_V_T2_CAVV() {
    ...
    // auto D1 = ambit::Tensor::build(tensor_type_, "C1 VT2 CAVV", {na, na});
    // compute_Hbar1V_diskDF(D1, true);
    // double Eold = D1("vu") * L1_.block("aa")("uv");
...
void SA_MRPT2::compute_Hbar1V_DF(ambit::Tensor& Hbar1, bool Vr) {
    ...
    // temp tensors for each thread
    std::vector<ambit::Tensor> C_vec(nthreads);
    std::vector<ambit::Tensor> J1(nthreads), J2(nthreads), JK(nthreads);

    for (int i = 0; i < nthreads; ++i) {
        std::string t = std::to_string(i);
        C_vec[i] = ambit::Tensor::build(CoreTensor, "C1uv_thread" + t, {na, na});
...
#pragma omp parallel for num_threads(nthreads)
        for (size_t p = 0; p < ic_pairs_size; ++p) {
            ...
            C_vec[thread]("vu") += JK[thread]("ev") * J1[thread]("eu");
        }
        i_shift += i_nocc;
    }
    ...
    //     int n_threads = static_cast<size_t>(n_threads_) < max_n_threads ? n_threads_ :
    //     max_n_threads; if (n_threads != n_threads_) {
    ...
    //             double denom =
    //                 Fm + Fdiag_[actv_mos_[i[2]]] - Fdiag_[virt_mos_[i[0]]] -
    //                 Fdiag_[virt_mos_[i[1]]];
    //             value *= dsrg_source_->compute_renormalized_denominator(denom);
    //         });
...
#pragma omp parallel for num_threads(nthreads)
        for (size_t c = 0; c < c_nvir; ++c) {
            ...
            C_vec[thread]("vu") += J[thread]("vmn") * JK[thread]("umn");
        }
        c_shift += c_nvir;
    }
    ...
    // #pragma omp critical
    //         {
    //             Be_vec[thread].data() = ints_->three_integral_block(aux_mos_, core_mos_,
    //             {ie}).data(); if (!semi_canonical_) {
    ...
    //         S_vec[thread].iterate([&](const std::vector<size_t>& i, double& value) {
    //             double denom =
    //                 Fdiag_[core_mos_[i[1]]] + Fdiag_[core_mos_[i[2]]] - Fe -
    //                 Fdiag_[actv_mos_[i[0]]];
    //             value *= dsrg_source_->compute_renormalized_denominator(denom);
    //         });

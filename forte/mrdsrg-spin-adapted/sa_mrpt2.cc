...
void SA_MRPT2::compute_Hbar1V_DF(ambit::Tensor& Hbar1, bool Vr) {
    ...
    ambit::Tensor Bu; // euQ
    if (!semi_v or !semi_a)
        Bu = read_Bcanonical("va", {0, nv}, {0, na}, ThreeIntsBlockOrder::pqQ);
    else
        Bu = ints_->three_integral_block(aux_mos_, virt_mos_, actv_mos_, ThreeIntsBlockOrder::pqQ);
...
void SA_MRPT2::compute_Hbar1C_DF(ambit::Tensor& Hbar1, bool Vr) {
    ...
    ambit::Tensor Bu; // umQ
    if (!semi_c or !semi_a)
        Bu = read_Bcanonical("ac", {0, na}, {0, nc}, ThreeIntsBlockOrder::pqQ);
    else
        Bu = ints_->three_integral_block(aux_mos_, actv_mos_, core_mos_, ThreeIntsBlockOrder::pqQ);
    auto& Bu_vec = Bu.data();

...
class SADSRG : public DynamicCorrelationSolver {
  ...
  protected:
     ...
     */
    void canonicalize_B(const std::vector<std::string>& blocks);
    /**
     ...
     * @param mos1_range the range of the 1st MO index (0-based)
     * @param mos2_range the range of the 2nd MO index (0-based)
     ...
     *
     * auto B = read_Bcanonical("cv", {1,1}, {0,10});
     * loads a slice of the "cv" block of B on disk, where the "c" index is the 2nd core MO
     * and the "v" indices contain the first 11 virtual MOs.
     * The output is a Tensor of dimension nQ x 1 x 11.
     *
     * auto C = read_Bcanonical("vc", {0,10}, {1,1});
     ...
     * C("Qpq") = B("Qqp");
     *
     * auto D = read_Bcanonical("cv", {1,1}, {0,10});
     * outputs a Tensor of dimension 1 x 11 x nQ and it is equivalent to
     * D("pqQ") = B("pqQ");
     */

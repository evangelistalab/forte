...
//     mcscf_debug_print_ = options_->get_bool("CASSCF_DEBUG_PRINTING");
...
//     int mcscf_freq = options_->get_int("CASSCF_CI_FREQ");
...
//     E_mcscf_ = 0.0;
//     double E_mcscf_old = 0.0, Ediff = 0.0;
...
//         // If mcscf_freq is on, do the CI step every mcscf_iteration
//         // for example, every 5 iterations, run a CASCI.
//         if (ci_step) {
//             if ((iter < mcscf_freq) || (iter % mcscf_freq) == 0) {
...
//                         E_mcscf_, mcscf_total_iter.get(), diis_start_label.c_str());
//     }
...
//     psi::Process::environment.globals["CURRENT ENERGY"] = E_mcscf_;
//     psi::Process::environment.globals["CASSCF_ENERGY"] = E_mcscf_;
...
//     std::string mcscf_ci_type = options_->get_str("ACTIVE_SPACE_SOLVER");

//     auto state_map = to_state_nroots_map(state_weights_map_);
//     auto active_space_solver = make_active_space_solver(mcscf_ci_type, state_map, scf_info_,
...
//     E_mcscf_ = compute_average_state_energy(state_energies_map, state_weights_map_);

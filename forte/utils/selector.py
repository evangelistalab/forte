...
def run_sci(ints, scf_info, wfn, selector):
    ...
    for k in range(2):
        ...
        num_dets = min(3, int(scipy.special.binom(nactv, na_act) * scipy.special.binom(nactv, nb_act)))
        #         print(k, num_dets)
        twice_ms = na - nb + 2 * k
        multiplicity = twice_ms + 1
        state = forte.StateInfo(na=na + k, nb=nb - k, multiplicity=multiplicity, twice_ms=twice_ms, irrep=0)
    ...
    as_solver = forte.make_active_space_solver("ASCI", state_map, scf_info, mo_space_info, forte.forte_options, as_ints)
...
def make_mo_plot(energies, occs):
    ...
    ax.tick_params(direction="out", width=1.0, length=3.0, axis="x", which="both", bottom=False, top=False)
...
class Selector:
            ...
            mo_list.append(
                {
                    ...
                    "file_name": file,
                    "occ": self.get_mo_occ(na, nb, int(mo_num) - 1),
    ...
    def make_mo_widget(self, mo_list):
        ...
        box_layout = widgets.Layout(border="0px solid black", width="", height="", flex_direction="row", display="flex")
        ...
        for mo in mo_list:
            ...
            mol_renderer = FortePy3JSRenderer(width=self.box_width, height=self.box_height)
            cubefile = CubeFile(cube_file_name)
            mol_renderer.add_cubefile(cubefile)
            image = mol_renderer.renderer

            label = "{}) {} {} (eps={:.3f},occ={})".format(
                mo["mo_num"] + 1, mo["symmetry"][0], mo["symmetry"][1], mo["epsilon"], mo["occ"]
            )
            button = widgets.ToggleButton(value=False, description=label, layout={"width": "200px"})
        ...
        if self.debug:
            self.occ_w = widgets.Textarea(
                ...
                disabled=True,
                layout={"height": "100px", "width": "800px", "margin": "0px 0px 0px 0px"},
            )
    ...
    def generate_occupations(self, change):
        ...
        s = "mo_reordering = {}\nrestricted_docc = {}\nactive = {}\nrestricted_uocc = {}".format(
            self.mo_reorder, self.restricted_docc, self.active, self.restricted_uocc
        )
        if self.run:

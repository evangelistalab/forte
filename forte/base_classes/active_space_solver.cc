...
const std::map<StateInfo, std::vector<double>>& ActiveSpaceSolver::compute_energy() {
    ...
    for (const auto& state_nroot : state_nroots_map_) {
        ...
        //            psi::outfile->Printf("\n  Continue to the next symmetry block: No need to find
        //            the "
        ...
        //        // save energies for ms < 0 states (same in energy as ms > 0) to ensure correct
        //        averaging if (twice_ms > 0 and ms_avg_) {
...
void ActiveSpaceSolver::compute_fosc_same_orbs() {
    ...
    for (size_t M = 0, n_entries = states.size(); M < n_entries; ++M) {
        ...
        for (size_t N = M; N < n_entries; ++N) {
            ...
            //            } else {
            //                if (M != N and ms_avg_) {
            //                    // skip same multiplicity but different Ms (no spin-orbit
            //                    coupling) std::tuple<int, int, int, std::vector<size_t>,
            //                    std::vector<size_t>> set1{
            //                        state1.na(), state1.nb(), state1.irrep(), state1.gas_min(),
            //                        state1.gas_max()};
            //                    std::tuple<int, int, int, std::vector<size_t>,

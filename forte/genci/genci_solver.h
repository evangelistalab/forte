...
class GenCISolver : public ActiveSpaceMethod {
  ...
  public:
    ...
    /// @brief Construct a GenCISolver object
    ...
    GenCISolver(StateInfo state, size_t nroot, std::shared_ptr<MOSpaceInfo> mo_space_info,
    ...
    /// Return string lists
    std::shared_ptr<GenCIStringLists> lists();
  ...
  private:
    ...
    std::shared_ptr<GenCIVector> C_;

    /// A temporary wave function
    std::shared_ptr<GenCIVector> T_;
    ...
    void copy_state_into_fci_vector(int root, std::shared_ptr<GenCIVector> C);
};
} // namespace forte

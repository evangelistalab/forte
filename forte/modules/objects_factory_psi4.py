...
class ObjectsFromPsi4(Module):
    ...
    def _run(self, data: ForteData = None) -> ForteData:
        data = prepare_forte_objects(data, **self.kwargs)
...
def prepare_forte_objects(data, **kwargs):
    ...
    add_and_orthonormalize_orbitals(data, is_psi_wfn_fresh, Ca, temp_mo_space_info, **kwargs)
    ...
    copy_state_info_options_from_wfn(data)

    # Step 8: Prepare Forte objects from the wavefunction, including a new MOSpaceInfo object
    prepare_forte_objects_from_wavefunction(data, temp_mo_space_info)
...
def get_psi4_wavefunction(data, **kwargs):
    """
    ...
    ref_wfn = run_psi4(ref_type, data.molecule, not do_mcscf, **kwargs)
    return ref_wfn, True


def run_psi4(ref_type, molecule, print_warning=False, **kwargs):
    """
...
def create_mo_space_info(data, **kwargs):
    """
...
def add_and_orthonormalize_orbitals(data, is_psi_wfn_fresh, Ca, temp_mo_space_info, **kwargs):
    """
    ...
    if check_mo_orthonormality(new_S, Ca):
        ...
        # if the psi_wfn is not fresh, we need to perform a new SCF and orthonormalize the orbitals
            ...
            # run a new SCF
            wfn_new = run_psi4("scf", data.molecule, False, **kwargs)
...
def copy_state_info_options_from_wfn(data):
...
def prepare_forte_objects_from_wavefunction(data, temp_mo_space_info):
    """
...
def prepare_forte_objects_old(data, **kwargs):
    """

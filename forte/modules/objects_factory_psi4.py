...
class ObjectsFromPsi4(Module):
    ...
    def prepare_forte_objects(self, data, **kwargs):
        ...
        #  ref_wfn  | Ca   | Basis mismatch | Action
        #  -----------------------------------------------------------------------------------------------
        #  None     | None | N/A            | Run new SCF
        #  None     | Read | N/A            | Run new SCF, inject Ca, and orthonormalize MOs
        #  Passed   | None | No             | Run new SCF, inject Ca, and orthonormalize MOs
        #  Passed   | None | Yes            | Run new SCF, project and orthonormalize old Ca, and inject them in the new wfn
        #  Passed   | Read | No             | Not supported because of potential redundancy in the input
        #  Passed   | Read | Yes            | Not supported because of potential redundancy in the input
        ...
        # Step 2. Run an SCF computation and get the reference wavefunction with the current molecule and basis set
        data.psi_wfn = self.get_psi4_wavefunction(data, **kwargs)

        # Step 3. Optionally either read the orbitals or get them from a ref_wfn object passed by the user.     
        Ca = self.get_orbitals_from_file(data)
        ref_wfn = kwargs.get("ref_wfn", None)
        if Ca is None and ref_wfn is not None:


        if ref_wfn:

        else:
            # compute a fresh psi4 wavefunction with the molecule and basis set given in the input
            psi_wfn = self.get_new_psi4_wavefunction(data, **kwargs)
            is_psi_wfn_fresh = True
        ...
        if not ref_wfn:
            ...
            bs_name = data.options.get_str("BASIS")
            if bs_name != ref_wfn.basisset().name():
    ...
    def get_psi4_wavefunction(self, data, **kwargs):
        ...
        # # check if ref_wfn is provided in kwargs
        # ref_wfn = kwargs.get("ref_wfn", None)
        # if ref_wfn:
        #     # if the basis requested is not consisten with the one in ref_wnf run a new SCF
        #     bs_name = data.options.get_str("BASIS")
        #     if bs_name != ref_wfn.basisset().name():
        #         p4print(f"\n\n  Different basis sets between option ({bs_name}) and ref_wfn ({ref_wfn.basisset().name()})!")
        #         p4print(f"\n  Perform new SCF using target basis set {bs_name} ...\n\n")
        #         kwargs_copy = {k: v for k, v in kwargs.items() if k != "ref_wfn"}
        #         wfn_new = this.run_psi4("scf",data.molecule, False, **kwargs_copy)

...
def prepare_summary(jobdir, test_results, test_time, summary, color):
    ...
    summary.append("    {0}{1}{2}{3:7.1f}".format(jobdir.upper(), filler, msg, duration))
    return duration
...
def setup_argument_parser():
    ...
    parser.add_argument("--psi4_exec", help="the location of the psi4 executable")
    parser.add_argument(
        "--file", help="the yaml file containing the list of tests (default: tests.yaml)", default="tests.yaml"
    )
    parser.add_argument("--failed", help="run only failed tests (listed in the file failed_tests)", action="store_true")
    parser.add_argument("--bw", help="print the summary in black and white? (default: color)", action="store_true")
    parser.add_argument("--failed_dump", help="dump the output of the failed tests to stdout?", action="store_true")
    parser.add_argument(
        "--type",
    ...
    parser.add_argument("--group", help="which group of tests to run? (default: None)", default=None)
...
def main():
    ...
    for test_group, test_levels in test_dict.items():
        if test_group in tested_groups:
            print(f"Test group {test_group.upper()}")
            ...
            for test_level, tests in test_levels.items():
                ...
                if test_level in TEST_LEVELS[args.type]:
                    for test in tests:
                        print(f"    Running test {test.upper()}")
                        successful = run_job(test, psi4command, test_results, test_time)
                        if not successful:
                            local_failed_tests.append(test)
                            nfailed += 1
                        total_time += prepare_summary(test, test_results, test_time, summary, not args.bw)
                    ntests += len(tests)
    ...
    # check if the folder "test_results" exists, if not create it
    if not os.path.exists("test_results"):
        os.makedirs("test_results")

    # save the summary to a file in the folder "test_results"
    file_name = "test_results/test_results_%s.txt" % now.strftime("%Y-%m-%d-%H%M")
    ...
    if nfailed == 0:
        print(f"Tests: All passed ({ntests} tests)\n")
    else:
        print(f"Tests: {ntests - nfailed} passed and {nfailed} failed\n")
        ...
        if args.failed_dump:
            for test_group, test_levels in failed_tests.items():
                for test_level, tests in test_levels.items():
                    for test in tests:
                        print(f"\n\n==> {test.upper()} TEST OUTPUT <==\n")
                        subprocess.call(f"cat {test}/output.dat", shell=True)
                        print("\n")
        exit(1)

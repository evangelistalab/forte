...
# def test_ndarray_invalid_einsum_expression():
#     """Test einsum with an invalid expression."""
#     tensor = ndarray([2, 2])
#     with pytest.raises(ValueError):
#         ndarray.einsum("invalid_expression", [tensor])

# def test_ndarray_einsum_multiple_tensors():
#     """Test einsum with multiple tensors."""
#     tensor_a = ndarray([2, 2])
#     tensor_a.fill(1.0)
#     tensor_b = ndarray([2, 2])
#     tensor_b.fill(2.0)
#     tensor_c = ndarray([2, 2])
#     tensor_c.fill(3.0)
#     result = ndarray.einsum("ij,jk,kl->il", [tensor_a, tensor_b, tensor_c])
#     expected = np.einsum("ij,jk,kl->il", tensor_a.numpy_array(), tensor_b.numpy_array(), tensor_c.numpy_array())
#     assert np.array_equal(result.numpy_array(), expected)

# def test_ndarray_memoryview():
#     """Test that the ndarray supports the buffer protocol."""
#     tensor = ndarray([2, 2])
#     memory_view = memoryview(tensor)
#     assert memory_view.shape == (2, 2)
#     assert memory_view.format == "d"  # Format character for double


# def test_ndarray_einsum():
#     """Test the einsum operation."""
#     tensor_a = ndarray([2, 3])
#     tensor_a.set_to([1, 2, 3, 4, 5, 6])  # Data: [1, 2, 3, 4, 5, 6]
#     tensor_b = ndarray([3, 2])
#     tensor_b.set_to([7, 8, 9, 10, 11, 12])  # Data: [7, 8, 9, 10, 11, 12]

#     # Perform einsum operation
#     result = ndarray.einsum("ij,jk->ik", [tensor_a, tensor_b])
#     assert result.shape == [2, 2]

#     # Verify against NumPy's einsum
#     expected = np.einsum("ij,jk->ik", tensor_a.numpy_array(), tensor_b.numpy_array())
#     assert np.allclose(result.numpy_array(), expected)

# def test_ndarray_einsum_no_tensors():
#     """Test einsum with no tensors provided."""
#     with pytest.raises(RuntimeError):
#         ndarray.einsum("i->", [])


# def test_ndarray_einsum_incorrect_shapes():
#     """Test einsum with tensors of incompatible shapes."""
#     tensor_a = ndarray([2, 3])
#     tensor_b = ndarray([4, 2])
#     with pytest.raises(ValueError):
#         ndarray.einsum("ij,jk->ik", [tensor_a, tensor_b])
...
if __name__ == "__main__":
    ...
    # test_ndarray_invalid_einsum_expression()  # 29
    # test_ndarray_einsum_multiple_tensors()  # 30
    # test_ndarray_memoryview()  # 31
    # test_ndarray_einsum()  # 32
    # test_ndarray_einsum_no_tensors()  # 33
    # test_ndarray_einsum_incorrect_shapes()  # 34

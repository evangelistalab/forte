...
def test_sparse_ci3():
    ...
    gop.add_term_from_str('[0a+ 0b+ 0b- 0a-]',1.0)
    dtest = det("20")
    ...
    gop = forte.GeneralOperator()
    gop.add_term_from_str('[0a+ 0b+ 0a- 0b-]',1.0)
    dtest = det("20")
    ref = forte.StateVector({ dtest: 1.0})
    wfn = forte.apply_operator(gop,ref)
    assert wfn[dtest] == pytest.approx(-1.0, abs=1e-9)
    ...
    gop = forte.GeneralOperator()
    gop.add_term_from_str('[0b+ 0a+ 0b- 0a-]',1.0)
    dtest = det("20")
    ref = forte.StateVector({ dtest: 1.0})
    wfn = forte.apply_operator(gop,ref)
    assert wfn[dtest] == pytest.approx(-1.0, abs=1e-9)
    ...
    gop = forte.GeneralOperator()
    gop.add_term_from_str('[0b+ 0a+ 0a- 0b-]',1.0)
    dtest = det("20")
    ref = forte.StateVector({ dtest: 1.0})
    wfn = forte.apply_operator(gop,ref)
    assert wfn[dtest] == pytest.approx(1.0, abs=1e-9)
    ...
    #                             = + 3a+ |-+-++>
    gop = forte.GeneralOperator()
    gop.add_term_from_str('[3a+ 4a+ 2b+ 1b- 0a-]',1.0)
    ref = forte.StateVector({ det("22"): 1.0})
    wfn = forte.apply_operator(gop,ref)
    assert wfn[det("-+-++")] == pytest.approx(1.0, abs=1e-9)

    gop = forte.GeneralOperator()
    gop.add_term_from_str('[0a+]',1.0)
    wfn = forte.apply_operator(gop,forte.StateVector({ det(""): 1.0}))
    assert wfn[det("+")] == pytest.approx(1.0, abs=1e-9)

    gop = forte.GeneralOperator()
    gop.add_term_from_str('[0a+ 1a+]',1.0)
    wfn = forte.apply_operator(gop,forte.StateVector({ det(""): 1.0}))
    assert wfn[det("++")] == pytest.approx(1.0, abs=1e-9)

    gop = forte.GeneralOperator()
    gop.add_term_from_str('[0a+ 1a+ 2a+]',1.0)
    wfn = forte.apply_operator(gop,forte.StateVector({ det(""): 1.0}))
    assert wfn[det("+++")] == pytest.approx(1.0, abs=1e-9)

    gop = forte.GeneralOperator()
    gop.add_term_from_str('[0a+ 1a+ 2a+ 3a+]',1.0)
    wfn = forte.apply_operator(gop,forte.StateVector({ det(""): 1.0}))
    assert wfn[det("++++")] == pytest.approx(1.0, abs=1e-9)

    gop = forte.GeneralOperator()
    gop.add_term_from_str('[0a+ 1a+ 2a+ 3a+ 4a+]',1.0)
    wfn = forte.apply_operator(gop,forte.StateVector({ det(""): 1.0}))
    assert wfn[det("+++++")] == pytest.approx(1.0, abs=1e-9)

    gop = forte.GeneralOperator()
    gop.add_term_from_str('[2a+ 0a+ 3a+ 1a+ 4a+]',1.0)
    wfn = forte.apply_operator(gop,forte.StateVector({ det(""): 1.0}))
    assert wfn[det("+++++")] == pytest.approx(-1.0, abs=1e-9)

    gop = forte.GeneralOperator()
    gop.add_term_from_str('[0a-]',1.0)
    wfn = forte.apply_operator(gop,forte.StateVector({ det("22222"): 1.0}))
    ...
    gop = forte.GeneralOperator()
    gop.add_term_from_str('[2a- 1a- 0a-]',1.0)
    wfn = forte.apply_operator(gop,forte.StateVector({ det("22222"): 1.0}))
    assert wfn[det("---22")] == pytest.approx(1.0, abs=1e-9)

    gop = forte.GeneralOperator()
    gop.add_term_from_str('[3a- 2a- 1a- 0a-]',1.0)
    wfn = forte.apply_operator(gop,forte.StateVector({ det("22222"): 1.0}))
    assert wfn[det("----2")] == pytest.approx(1.0, abs=1e-9)

    gop = forte.GeneralOperator()
    gop.add_term_from_str('[4a- 3a- 2a- 1a- 0a-]',1.0)
    wfn = forte.apply_operator(gop,forte.StateVector({ det("22222"): 1.0}))
    assert wfn[det("-----")] == pytest.approx(1.0, abs=1e-9)

    gop = forte.GeneralOperator()
    gop.add_term_from_str('[1a- 3a- 2a- 4a- 0a-]',1.0)
    wfn = forte.apply_operator(gop,forte.StateVector({ det("22222"): 1.0}))
    assert wfn[det("-----")] == pytest.approx(-1.0, abs=1e-9)
    ...
    # test destroying empty orbitals: (0a+ 0b+ 0b- 0a-) |0> = 0
    gop = forte.GeneralOperator()
    gop.add_term_from_str('[0a+ 0b+ 0b-]',1.0)
    dtest = det("00")
    ref = forte.StateVector({ dtest: 1.0})
    wfn = forte.apply_operator(gop,ref)
    assert dtest not in wfn
    ...
    gop = forte.GeneralOperator()
    gop.add_term_from_str('[1a+ 0a+ 0a-]',1.0)
    dtest = det("+")
    ref = forte.StateVector({ dtest: 1.0})
    wfn = forte.apply_operator(gop,ref)
    assert dtest not in wfn
    ...
    # test creating in filled orbitals: (0a+ 1a+ 0a-) |22> = 0
    gop = forte.GeneralOperator()
    gop.add_term_from_str('[1b+ 0a+ 0a-]',1.0)
    dtest = det("+")
    ref = forte.StateVector({ dtest: 1.0})
    wfn = forte.apply_operator(gop,ref)
    assert dtest not in wfn
    ...
    gop = forte.GeneralOperator()
    gop.add_term_from_str('[0a+ 0a-]',1.0)
    dtest = det("0")
    ref = forte.StateVector({ dtest: 1.0})
    wfn = forte.apply_operator(gop,ref)
    assert dtest not in wfn
    ...
    gop = forte.GeneralOperator()
    gop.add_term_from_str('[0b+ 0b-]',1.0)
    dtest = det("0")
    ref = forte.StateVector({ dtest: 1.0})
    wfn = forte.apply_operator(gop,ref)
    assert dtest not in wfn
    ...
    gop = forte.GeneralOperator()
    gop.add_term_from_str('[2a+ 2a-] + [2b+ 2b-]',1.0)
    ...
    gop = forte.GeneralOperator()
    gop.add_term_from_str('[3a+ 0a-]',1.0)
    dtest = det("22")
    ref = forte.StateVector({ dtest: 1.0})
    wfn = forte.apply_operator(gop,ref)
    ...
    gop = forte.GeneralOperator()
    gop.add_term_from_str('[0a- 3a+]',1.0)
    dtest = det("22")
    ref = forte.StateVector({ dtest: 1.0})
    wfn = forte.apply_operator(gop,ref)
    assert wfn[det("-20+")] == pytest.approx(1.0, abs=1e-9)
    ...
    gop = forte.GeneralOperator()
    gop.add_term_from_str('[3b+ 0b-]',1.0)
    dtest = det("22")
    ref = forte.StateVector({ dtest: 1.0})
    wfn = forte.apply_operator(gop,ref)
    assert wfn[det("+20-")] == pytest.approx(-1.0, abs=1e-9)

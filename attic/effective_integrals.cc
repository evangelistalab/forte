...
public:
  ...
  public:
    ...
    void operator()(int pabs, int qabs, int rabs, int sabs, int, int, int, int,
...
void EffectiveIntegrals::transform_integrals() {
  ...
  ambit::Tensor Vmo =
      ambit::Tensor::build(ambit::CoreTensor, "Vmo", {nmo_, nmo_, nmo_, nmo_});
  ambit::Tensor Vso =
      ambit::Tensor::build(ambit::CoreTensor, "Vso", {nso_, nso_, nso_, nso_});
  ...
  // Erf(x)/x integrals (long range)
  {
    ...
    SOShellCombinationsIterator shellIter(sobasisset, sobasisset, sobasisset,
                                          sobasisset);
    ...
    for (size_t q = 0; q < ncmo_; ++q) {
      for (size_t r = 0; r < ncmo_; ++r) {
        for (size_t s = 0; s < ncmo_; ++s) {
          size_t pqrs_cmo =
              ncmo_ * ncmo_ * ncmo_ * p + ncmo_ * ncmo_ * q + ncmo_ * r + s;
          size_t pqrs_mo = nmo_ * nmo_ * nmo_ * map[p] + nmo_ * nmo_ * map[q] +
                           nmo_ * map[r] + map[s];

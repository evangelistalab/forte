...
void TensorSRG_ODEInterface::operator() (const odeint_state_type& x,odeint_state_type& dxdt,const double t)
{
    ...
    tensorsrg_obj_.Hbar1.iterate_over_elements(
                [&](std::vector<size_t>& m,std::vector<MOSetSpinType>& spin,double& value){value = x[k]; k++;});
...
double TensorSRG::compute_srg_energy()
{
    ...
    Hbar1.iterate_over_elements(
                [&](std::vector<size_t>& m,std::vector<MOSetSpinType>& spin,double& value){ x[k] = value; k++;});
...
//        commutator_A_B_C(1.0,O1_,O2_,Hbar1_,Hbar2_,eta0,eta1_,eta2_);
        ...
        S1.fill_one_electron_spin([&](size_t i,MOSetSpinType si,size_t a,MOSetSpinType sa){
            if (si  == Alpha){
                size_t ii = mos_to_aocc[i];
                size_t aa = mos_to_avir[a];
                return Hbar1_ov(ii,aa) / (Hbar1_vv(aa,aa) - Hbar1_oo(ii,ii) - Hbar2_vovo(aa,ii,aa,ii));
            }else if (si  == Beta){
        ...
        S2.fill_two_electron_spin([&](size_t i,MOSetSpinType si,
            ...
            if ((si == Alpha) and (sj == Alpha)){
                ...
                return Hbar2_oovv(ii,jj,aa,bb) / (Hbar1_vv(aa,aa) + Hbar1_vv(bb,bb) - Hbar1_oo(ii,ii) - Hbar1_oo(jj,jj) + A);
